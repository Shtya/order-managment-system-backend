 ...
 */
export function dtoToJsonSchema(DtoClass: new (...args: any[]) => object, options?: JsonSchemaOptions): Record<string, unknown> {
	...
	if (Date.now() - start > timeoutMs) {
		throw new Error(`Schema derivation for '${DtoClass.name}' exceeded timeout (${timeoutMs}ms)`);
	}

	return schema as unknown as Record<string, unknown>;
}

function deriveForClass(DtoClass: new (...args: any[]) => object, seen: Set<string>): JsonSchema {
	...
	const required = Array.from(byProperty.keys()).filter((p) => !optionalProps.has(p));
...
function deriveProperty(
	...
	const designType = Reflect.getMetadata('design:type', DtoClass.prototype, propertyName);
	...
	for (const meta of metas) {
		...
		switch (t) {
			...
			case 'isIn':
				types.add('string');
				enumValues = t === 'isEnum' ? extractEnumValues(constraint) : (Array.isArray(meta.constraints?.[0]) ? meta.constraints![0] as unknown[] : undefined);
				break;
			case 'isNumber':
			case 'isInt':
				types.add('number');
	...
	if (types.size === 0) {
		...
		} else if (designType && typeof designType === 'function' && designType !== Object) {
			types.add('object');
	...
	if (isArray) {
	...
	} else if (nestedClass && typeof nestedClass === 'function' && (types.has('object') || designType !== Object)) {
		const nested = deriveForClass(nestedClass, seen);
		schema.type = 'object';
		...
		const primaryType = types.has('string') ? 'string' : types.has('number') ? 'number' : types.has('boolean') ? 'boolean' : 'object';
	...
	if (schemaMeta) {
		if (schemaMeta.description !== undefined) schema.description = schemaMeta.description;
		if (schemaMeta.example !== undefined) schema.example = schemaMeta.example;
		if (schemaMeta.examples !== undefined) schema.examples = schemaMeta.examples;
...
function applyConstraint(schema: JsonSchema, type: string, constraints: unknown[]) {
