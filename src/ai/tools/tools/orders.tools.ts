  ...
  getTools(): AiTool[] {
    return [
      ...
      new AiTool({
        ...
        description:
          "Record an address conflict and stop. This does not message the customer. Use it when the written address contradicts itself (the same part has two alternatives such as \"12 أو 14\" or \"شارع الحجاز أو شارع سوريا\"; a street number plus a building, floor, and apartment is one address, so \"12 شارع عباس العقاد، عمارة 4\" is not a conflict), when the area does not belong to the city (names may be in the written address or in the city and area fields), when the written address disagrees with the selected city or area, or when the written address and the WhatsApp/map location are different places. Do not copy the WhatsApp location onto the order and do not use it to fix these conflicts. For a self-contradiction or a city/area mismatch, send the written address only (label \"العنوان المسجل\", source \"address\"). For a written-address vs WhatsApp-location mismatch, include both candidates (\"العنوان المسجل\" and \"عنوان الواتساب\") so a later step can offer the choice. fullAddress must be plain address text with NO lat/lng. Do NOT call bulk_update_orders_shipping in the same turn. Do NOT invent addresses.",
