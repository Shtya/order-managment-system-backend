  ...
  getTools(): AiTool[] {
    return [
      ...
      new AiTool({
        ...
        description:
          "Record an address conflict and stop. Do not update the order. Call it with both addresses only when the written address and the WhatsApp/map address are each valid and they are completely different places (different city or area, or a large distance). City or area written inside the address still counts when that field is empty, and that does not block this call. Nearby places, a small difference, and addresses that complete each other are not a conflict. Also call it with the written address only when the written address contradicts itself (the same part has two alternatives such as \"12 أو 14\"; \"12 شارع عباس العقاد، عمارة 4\" is one address) or the area does not belong to the city. For those, send the written address only (label \"العنوان المسجل\", source \"address\"). For two valid but completely different places, include both (\"العنوان المسجل\" and \"عنوان الواتساب\", with latitude and longitude). fullAddress is plain text with no coordinates. Do not invent an address.",
