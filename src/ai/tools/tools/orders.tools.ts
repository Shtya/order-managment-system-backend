...
import { AI_PERMISSION_TOOLS_ORDERS_READ, AI_PERMISSION_TOOLS_ORDERS_WRITE } from '../../ai.constants';
...
export class OrdersAiTools {
	...
	getTools(): AiTool[] {
		return [
			...
			new AiTool({
				...
				description:
					`Retrieve the status-change history / action log for an order using the order number. ${ORDER_NUMBER_HELP}`,
			...
			new AiTool({
				...
				description: 'Get aggregate order statistics for the tenant (totals, counts by status, revenue metrics).',
	...
	private wrap<T>(code: string, fn: () => Promise<T>): Promise<AiExecutionResult> {
		return fn().then(
			(data) => ({ ok: true, code, data }),
			(error) => ({ ok: false, code: `${code}_ERROR`, error: error instanceof Error ? error.message : String(error) }),
		);
	}

	private async getOrder(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_FOUND', async () => {
			const order: any = await this.ordersService.get(this.buildMe(ctx), String(args.orderNumber));
			return mapOrderCompact(order, { includeItems: Boolean(args.includeItems) });
		});
	}

	private async getLatestOrder(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_FOUND', async () => {
	...
	private async getLatestOrderByPhone(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_FOUND', async () => {
			const records = await this.ordersService.list(this.buildMe(ctx), {
	...
	private async searchOrders(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDERS_FOUND', async () => {
			...
			return {
				...
				records: records.slice(0, 20).map((o: any) => mapOrderCompact(o, { includeItems: false })),
			};
		});
	}

	private async searchOrdersByPhone(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDERS_FOUND', async () => {
			const result = await this.ordersService.list(this.buildMe(ctx), {
				search: String(args.phoneNumber),
				limit: 20,
			...
			const records = (result as any)?.records ?? [];
			return records.map((o: any) => mapOrderCompact(o, { includeItems: false }));
		});
	}

	private async getOrderStatus(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
	...
	private async summarizeOrder(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
	...
	private async getOrderHistory(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_HISTORY', async () => {
			const order: any = await this.ordersService.get(this.buildMe(ctx), String(args.orderNumber));
			const history = await this.ordersService.getOrderHistory(order.id, this.buildMe(ctx));
			return history;
		});
	}

	private async getOrderItems(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
	...
	private async getOrderShipping(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
	...
	private async addOrderMessage(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_MESSAGE_ADDED', async () => {
			const order: any = await this.ordersService.get(this.buildMe(ctx), String(args.orderNumber));
			const message = await this.ordersService.addMessage(this.buildMe(ctx), order.id, {
	...
	private async bulkUpdateOrdersShipping(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDERS_SHIPPING_UPDATED', async () => {
			const rawItems = Array.isArray(args.items) ? args.items : [];
			const me = this.buildMe(ctx);
			const items: any[] = [];
			for (const item of rawItems) {
				const order: any = await this.ordersService.get(me, String((item as any).orderNumber));
				items.push({ ...(item as any), id: order.id });
			}
			const result = await this.ordersService.bulkUpdateShippingFields(me, { items } as any);
			return result;
		});
	}
}
		...
		replacementResult: order.replacementResult
			? {
				...
				items: (order.replacementResult.items ?? []).map((item: any) => ({
					id: item.id,
					originalOrderItemId:
						item.originalOrderItem?.id ?? null,
					productName:
						item.originalOrderItem?.variant?.product?.name ??
						null,
				})),
			}
			: null,
