...
import { AI_PERMISSION_TOOLS_WHATSAPP_READ, AI_PERMISSION_TOOLS_WHATSAPP_WRITE } from '../../ai.constants';
...
export class WhatsappAiTools {
	...
	private async listTemplates(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('WHATSAPP_TEMPLATES', async () => {
			const result: any = await this.whatsappTemplateService.list(this.buildMe(ctx), {
				search: args.search,
	...
	private async sendText(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('WHATSAPP_MESSAGE_SENT', async () => {
			...
			return {
				messageId: (response as any)?.messageId ?? (response as any)?.id ?? null,
				status: (response as any)?.status ?? 'accepted',
			};
		});
	}

	private async sendTemplate(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('WHATSAPP_TEMPLATE_SENT', async () => {
			...
			const response = await this.whatsappService.sendTemplate(
				this.buildMe(ctx),
				{
					...
					headerVariables: (args.headerVariables as Record<string, any>) ?? undefined,
					bodyVariables: (args.bodyVariables as Record<string, any>) ?? undefined,
					buttonVariables: (args.buttonVariables as Record<string, any>) ?? undefined,
					locationData: { latitude: '0', longitude: '0', address: '', name: '' },
				},
				args.accountId ? String(args.accountId) : undefined,
				undefined,
				{ source: 'ai', orderId },
...
export function whatsappTextDedupKey(args: Record<string, unknown>): string | null {
...
export function whatsappTemplateDedupKey(args: Record<string, unknown>): string | null {
	const phone = args.phoneNumber ? String(args.phoneNumber) : null;
