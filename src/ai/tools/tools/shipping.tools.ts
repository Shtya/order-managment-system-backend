...
import { AI_PERMISSION_TOOLS_SHIPPING_READ, AI_PERMISSION_TOOLS_SHIPPING_WRITE } from '../../ai.constants';
...
export class ShippingAiTools {
	...
	getTools(): AiTool[] {
		return [
			...
			new AiTool({
				...
				description: 'List the districts of a provider city. Returns unified district records (id, nameEn, nameAr).',
			...
			new AiTool({
				...
				description: 'List the zones of a provider district. Returns unified zone records (id, nameEn, nameAr).',
	...
	private async listProviderCities(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('SHIPPING_CITIES', async () => {
			const result: any = await this.shippingService.getCities(this.getTenantId(ctx), String(args.provider));
			return {
	...
	private async listProviderDistricts(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('SHIPPING_DISTRICTS', async () => {
			const result: any = await this.shippingService.getDistricts(this.getTenantId(ctx), String(args.provider), String(args.cityId));
			return {
				provider: result.provider,
				records: (result.records ?? []).map((d: any) => ({ id: d.id, nameEn: d.nameEn, nameAr: d.nameAr })),
			};
		});
	}

	private async listProviderZones(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('SHIPPING_ZONES', async () => {
			const result: any = await this.shippingService.getZones(this.getTenantId(ctx), String(args.provider), String(args.districtId));
			return {
				provider: result.provider,
				records: (result.records ?? []).map((z: any) => ({ id: z.id, nameEn: z.nameEn, nameAr: z.nameAr })),
			};
		});
	}

	private async resolveLocation(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('LOCATION_RESOLVED', async () => {
			...
			const citiesResult: any = await this.shippingService.getCities(this.getTenantId(ctx), provider);
			const city = (citiesResult.records ?? []).find((c: any) => matchName(c, cityName));
			...
			const providerLocation = await this.citiesService.findProviderLocationByProviderCityId(provider, city.id);
			const unifiedCityId = providerLocation?.cityId ?? null;
			const unifiedCityName = providerLocation?.city?.nameEn ? `${providerLocation.city.nameEn} / ${providerLocation.city.nameAr}` : null;
	...
	private async updateOrderShippingLocation(ctx: AiToolContext, args: Record<string, unknown>): Promise<AiExecutionResult> {
		return this.wrap('ORDER_SHIPPING_LOCATION_UPDATED', async () => {
			const provider = String(args.provider);
			const providerCityId = String(args.providerCityId);

			const providerLocation = await this.citiesService.findProviderLocationByProviderCityId(provider, providerCityId);
			const unifiedCityId = providerLocation?.cityId ?? null;
			...
			const result = await this.ordersService.updateOrderShippingLocation(this.buildMe(ctx), String(args.orderId), {
				cityId: unifiedCityId,
				districtId: args.unifiedDistrictId ? String(args.unifiedDistrictId) : undefined,
...
function matchName(record: { nameEn?: string; nameAr?: string }, search: string): boolean {
