import { IsArray, IsBoolean, IsNumber, IsOptional, IsString, Max, MaxLength, Min, MinLength } from 'class-validator';
...
export class GetOrderToolArgsDto {
	...
	@SchemaProperty({ description: 'Set true to include the order items list in the response.', example: false })
	@IsOptional()
	@IsBoolean()
...
export class SearchOrdersToolArgsDto {
	@SchemaProperty({
		description: 'Free-text search across order number (ORD... code), customer name, and phone number.',
		example: 'Ahmed Ali',
	})
	@IsOptional()
	@IsString()
	...
	@SchemaProperty({
		description: 'Exact order number to find. It is a short code starting with "ORD" + 8 letters/digits (e.g. ORD8VVTGTH), NOT the UUID id.',
		example: 'ORD8VVTGTH',
	})
	@IsOptional()
	@IsString()
	...
	@SchemaProperty({ description: 'Filter by order status name.', example: 'New' })
	@IsOptional()
	@IsString()
	status?: string;

	@SchemaProperty({ description: 'Filter by payment status.', example: 'pending' })
	@IsOptional()
	@IsString()
	...
	@SchemaProperty({ description: 'Number of records per page (1-50).', example: 10 })
	@IsOptional()
	@IsNumber()
	@Min(1)
	@Max(50)
...
export class SearchOrdersByPhoneToolArgsDto {
	@SchemaProperty({
		description: 'Customer phone number. Digits only, starting with country code.',
		example: '201000000000',
	})
	@IsString()
	@MinLength(8)
...
export class AddOrderMessageToolArgsDto {
	...
	@SchemaProperty({ description: 'The note/message text to append to the order.', example: 'Customer confirmed by phone.' })
	@IsString()
	@MinLength(1)
	@MaxLength(2000)
	message: string;

	@SchemaProperty({ description: 'Source label of the message (e.g. ai, admin, system).', example: 'ai' })
	@IsOptional()
	@IsString()
	source?: string;
}
