...
export class AiProviderError extends Error {
	...
	constructor(
		message: string,
		options: { kind: AiProviderErrorKind; provider?: string; status?: number; retryable?: boolean; cause?: unknown },
	) {
		super(message);
...
export class AiProviderRateLimitedError extends AiProviderError {
	constructor(message: string, options: { provider?: string; retryAfterMs?: number; status?: number } = {}) {
...
export function toAiProviderError(error: unknown, provider?: string): AiProviderError {
	...
	const err = error as { name?: string; message?: string; status?: number; code?: string; response?: { status?: number } };
	const status = err?.response?.status ?? err?.status;
	const code = String(err?.code ?? '');
	const isTimeout = code === 'ECONNABORTED' || code === 'ETIMEDOUT' || code === 'timeout';
	const isNetwork =
		code === 'ECONNREFUSED' ||
		code === 'ECONNRESET' ||
		code === 'ENOTFOUND' ||
	...
	if (isNetwork) return new AiProviderNetworkError(message, { provider, cause: error });
	if (status === 401 || status === 403) return new AiProviderAuthError(message, { provider, status });
	if (status === 429) return new AiProviderRateLimitedError(message, { provider, status });
	if (status) {
		return new AiProviderError(message, { kind: 'HTTP', provider, status, retryable: status >= 500 || status === 408 || status === 409 });
	}

	return new AiProviderError(message, { kind: 'NETWORK', provider, retryable: true, cause: error });
}
