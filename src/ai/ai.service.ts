...
export class AiService {
	...
	async setDefaultModel(me: any, dto: SetDefaultModelDto) {
		...
		if (!model) throw new NotFoundException(this.translations.t('domains.ai.model_not_found'));

		const provider = model.provider;
		if (!provider || !provider.isActive) {
			throw new BadRequestException(this.translations.t('domains.ai.model_provider_not_active'));
		}

		const isAdminScope = isSuperAdmin ? model.adminId === null : model.adminId === myAdminId || model.adminId === null;
		if (!isAdminScope) {
			throw new ForbiddenException(this.translations.t('domains.ai.model_not_accessible'));
		}
		...
		if (!isSuperAdmin && myAdminId) {
			...
			if (!integration || !integration.encryptedCredentials) {
				throw new BadRequestException(this.translations.t('domains.ai.credentials_not_configured'));
			}
		}
	...
	async resolveDefaultModel(me: any): Promise<{ modelId: string; providerEntityId: string } | null> {
		const myAdminId = tenantId(me);
		const isSuperAdmin = me.role?.name === SystemRole.SUPER_ADMIN;
		...
		if (myAdminId) {
			...
			if (record?.model?.provider?.isActive) {
				return { modelId: record.model.id, providerEntityId: record.model.provider.id };
			}
		}
	...
	async listProviders(
		...
		const isSuperAdmin =
			me.role?.name === SystemRole.SUPER_ADMIN;
		...
		if (isActive !== undefined) {
			if (scope === AiEntityScope.SYSTEM) {
				...
				qb.where('p.adminId = :adminId AND p.isActive = :isActive', { adminId, isActive });
			} else {
				if (isSuperAdmin) {
					qb.where('p.adminId IS NULL AND p.isActive = :isActive', { isActive });
				} else if (adminId) {
					qb.where('(p.adminId IS NULL OR p.adminId = :adminId) AND p.isActive = :isActive', { adminId, isActive });
				} else {
					qb.where('p.adminId IS NULL AND p.isActive = :isActive', { isActive });
				}
			}
			...
			if (scope === AiEntityScope.SYSTEM) {
				...
				if (isSuperAdmin) {
					...
					qb.where('(p.adminId IS NULL AND p.isActive = true) OR (p.adminId = :adminId)', { adminId });
				} else {
					qb.where('p.adminId IS NULL AND p.isActive = true');
				}
			}
		...
		const records = providers.map((provider) => ({
			...provider,
			integration: provider.integrations?.[0] ? this.toIntegrationResponse(provider.integrations[0]) : undefined,
	...
	async getProvider(me: any, id: string) {
		...
		const models = await this.modelRepo.find({ where: { providerId: id }, order: { name: 'ASC' } });
		const integration = (provider as any).integrations?.[0];
		return {
			...this.toProviderResponse(provider, models),
			integration: integration ? this.toIntegrationResponse(integration) : undefined,
		};
	}

	async createProvider(me: any, dto: CreateProviderDto) {
		const myAdminId = tenantId(me);
		if (!myAdminId) throw new ForbiddenException(this.translations.t('domains.ai.provider_not_custom'));

		const existing = await this.providerRepo.findOne({
			where: { name: dto.name, adminId: myAdminId },
		});
		if (existing) throw new BadRequestException(this.translations.t('domains.ai.provider_already_exists'));

		const existingCode = await this.providerRepo.findOne({
			where: { code: dto.code, adminId: myAdminId },
		});
		if (existingCode) throw new BadRequestException(this.translations.t('domains.ai.provider_code_exists'));

		const { baseUrl, credentials, ...providerData } = dto;
		const protocol = dto.protocol ?? AiProviderProtocol.OPENAI_COMPATIBLE;
		const authType = dto.authType ?? PROTOCOL_AUTH_MAP[protocol] ?? AiAuthType.API_KEY;
		...
		return this.dataSource.transaction(async (mgr) => {
			...
			if (baseUrl || credentials) {
				const encrypted = credentials
					? await this.encryptionService.encrypt(JSON.stringify(credentials))
					: undefined;
	...
	async updateProvider(me: any, id: string, dto: UpdateProviderDto) {
		...
		const authType = dto.authType ?? (protocol ? PROTOCOL_AUTH_MAP[protocol] : undefined) ?? provider.authType;
	...
	async toggleProviderActive(me: any, providerId: string, dto: { isActive: boolean }) {
	...
	async listModels(me: any, query: ListModelsQueryDto) {
		const { providerId, providerCode, modelType, tier, isActive, scope } = query;
		const myAdminId = tenantId(me);

		const limit = Number(query.limit) || 50;
		const sortDir: 'ASC' | 'DESC' = String(query.sortDir ?? 'DESC').toUpperCase() === 'ASC' ? 'ASC' : 'DESC';
		...
		if (myAdminId) {
			qb.where('(m.adminId = :adminId OR m.adminId IS NULL)', { adminId: myAdminId });
		}
		if (providerId) qb.andWhere('m.providerId = :providerId', { providerId });
		if (providerCode) qb.andWhere('provider.code = :providerCode', { providerCode });
		...
		if (isActive !== undefined) {
			...
			qb.andWhere(
				'((m.adminId IS NULL AND m.isActive = true) OR (m.adminId IS NOT NULL))',
			);
		}
		if (scope) {
			if (scope === 'system') qb.andWhere('m.adminId IS NULL');
			else if (scope === 'custom' && myAdminId) qb.andWhere('m.adminId = :adminId', { adminId: myAdminId });
		}

		if (cursor) {
			const operator = sortDir === 'DESC' ? '<' : '>';
			qb.andWhere(
				`(m.created_t, m.id) ${operator} (:cursorValue, :cursorId)`,
		...
		return {
			...
			limit,
			nextCursor: hasMore
				? { value: last.created_t, id: last.id }
				: undefined,
			sortBy: 'created_t',
			sortDir,
		};
	...
	async createModel(me: any, dto: CreateModelDto) {
		...
		if (provider.adminId !== myAdminId) {
			throw new ForbiddenException(this.translations.t('domains.ai.provider_not_custom'));
		}

		const existing = await this.modelRepo.findOne({
			where: { providerId: dto.providerId, modelCode: dto.modelCode },
		});
		if (existing) throw new BadRequestException(this.translations.t('domains.ai.model_already_exists'));
		...
		if (integration?.encryptedCredentials) {
			...
			const testResult = await this.testProviderCredentials(provider, credentials as AiProviderCredentials, integration.baseUrl, dto.modelCode);
			if (!testResult.valid) {
				throw new BadRequestException(testResult.message ?? this.translations.t('domains.ai.credentials_invalid_api_key'));
			}
		}
	...
	async listIntegrations(me: any, query?: ListIntegrationsQueryDto) {
		...
		if (!isSuperAdmin && myAdminId) {
			...
			else return { records: [], hasMore: false, limit, nextCursor: undefined, sortBy: 'created_t', sortDir };
		}

		if (providerId) qb.andWhere('i.providerId = :providerId', { providerId });
		if (isActive !== undefined) qb.andWhere('i.isActive = :isActive', { isActive });
		...
		if (cursor) {
			const operator = sortDir === 'DESC' ? '<' : '>';
			qb.andWhere(
				`(i.created_t, i.id) ${operator} (:cursorValue, :cursorId)`,
				{ cursorValue: cursor.value, cursorId: cursor.id },
			);
		}
	...
	async getIntegration(me: any, providerId: string) {
		...
		const integration = await this.integrationRepo.findOne({ where, relations: ['provider'] });
		if (!integration) throw new NotFoundException(this.translations.t('domains.ai.integration_not_found'));
	...
	async setCredentials(me: any, providerId: string, dto: SetCredentialsDto) {
		...
		return this.dataSource.transaction(async (mgr) => {
			...
			const encrypted = dto.credentials
				? await this.encryptionService.encrypt(JSON.stringify(dto.credentials))
				: undefined;
	...
	async testCredentials(me: any, providerId: string, modelCode?: string) {
		...
		const integration = await this.integrationRepo.findOne({
			where,
			select: ['id', 'providerId', 'adminId', 'encryptedCredentials', 'authType', 'baseUrl', 'lastValidatedAt', 'lastError', 'scope', 'created_t', 'updated_ut'],
			relations: ['provider'],
		});

		if (!integration) {
			return { valid: false, message: this.translations.t('domains.ai.credentials_not_configured'), lastValidatedAt: null };
		}
		...
		try {
			...
			const credentials = JSON.parse(decrypted);
			const result = await this.testProviderCredentials(integration.provider, credentials as AiProviderCredentials, integration.baseUrl, modelCode);
			...
			return { valid: result.valid, message: result.message, lastValidatedAt: new Date() };
		} catch (err: any) {
			const errorMsg = err?.message ?? this.translations.t('domains.ai.credentials_test_error');
			await this.integrationRepo.update(integration.id, { lastValidatedAt: new Date(), lastError: errorMsg });
			return { valid: false, message: this.translations.t('domains.ai.credentials_test_failed'), lastValidatedAt: new Date() };
		}
	}

	private resolveBaseProvider(provider: AiProviderEntity): AiProviderAbstract | undefined {
		...
		return baseProviders.find((p) => p.kind === provider.code) ?? baseProviders.find((p) => p.kind === provider.protocol);
	}

	private async testProviderCredentials(provider: AiProviderEntity, credentials: AiProviderCredentials, baseUrl?: string, modelCode?: string): Promise<{ valid: boolean; message?: string }> {
		const baseProvider = this.resolveBaseProvider(provider);
		if (!baseProvider) {
			return { valid: false, message: this.translations.t('domains.ai.provider_class_not_found', { args: { code: provider.code ?? provider.protocol } }) };
		}
		...
		try {
			...
			if (err instanceof AiProviderError && err.kind === 'AUTH') {
				return { valid: false, message: this.translations.t('domains.ai.credentials_invalid_api_key') };
			}
			return { valid: false, message: err?.message ?? this.translations.t('domains.ai.credentials_test_error') };
		}
	}
	...
	async deleteCustomModel(me: any, id: string) {
		const myAdminId = tenantId(me);
		const model = await this.modelRepo.findOne({ where: { id, adminId: myAdminId } });
		if (!model) throw new NotFoundException(this.translations.t('domains.ai.model_not_found'));
		await this.modelRepo.remove(model);
	}
	...
	async listRequestSummaries(me: any, query: ListRequestSummariesQueryDto) {
		const { page = 1, limit = 20, status, providerId, modelId, startDate, endDate, adminId } = query;
	...
	async getRequestSummary(me: any, id: string) {
		const summary = await this.summaryRepo.findOne({ where: { id }, relations: ['provider', 'model'] });
		if (!summary) throw new NotFoundException(this.translations.t('domains.ai.audit_request_not_found'));
		return this.toSummaryResponse(summary);
	}

	async getRequestSummaryProgress(me: any, id: string) {
		const summary = await this.summaryRepo.findOne({ where: { id }, select: ['id', 'progress'] });
		if (!summary) throw new NotFoundException(this.translations.t('domains.ai.audit_request_not_found'));
	...
	async getWriteToolCall(me: any, id: string) {
		const call = await this.writeCallRepo.findOne({ where: { id }, relations: ['provider', 'model'] });
		if (!call) throw new NotFoundException(this.translations.t('domains.ai.write_call_not_found'));
	...
	async retryWriteToolCall(me: any, id: string) {
		...
		if (call.status !== AiWriteToolCallStatus.FAILED && call.status !== AiWriteToolCallStatus.STALE) {
			throw new BadRequestException(
				this.translations.t('domains.ai.write_call_not_retryable', { args: { status: call.status } }),
			);
		}
	...
	updateConfig(dto: { enabled?: boolean; defaultProvider?: string; maxRoundtrips?: number; piiMaskingEnabled?: boolean }) {
		if (dto.enabled !== undefined) this.config.enabled = dto.enabled;
		if (dto.defaultProvider !== undefined) this.config.defaultProvider = dto.defaultProvider;
		if (dto.maxRoundtrips !== undefined) this.config.maxProviderRoundtrips = dto.maxRoundtrips;
		if (dto.piiMaskingEnabled !== undefined) this.config.piiMaskingEnabled = dto.piiMaskingEnabled;
	...
	async testProviderHealth(providerCode: string) {
		...
		if (!provider) throw new NotFoundException(this.translations.t('domains.ai.provider_not_found'));
		...
		try {
			...
			const latencyMs = Date.now() - start;
			return { healthy: false, latencyMs, error: err?.message ?? this.translations.t('domains.ai.health_check_failed', { args: { provider: providerCode } }) };
		}
	}
	...
	async exportRequestSummaries(me: any, query: ExportRequestSummariesQueryDto) {
		const data = await this.listRequestSummaries(me, { ...query, limit: 100000 });
		return data.records;
	}
	...
	private async findProviderWithAccess(me: any, id: string): Promise<AiProviderEntity> {
		const myAdminId = tenantId(me);
		...
		const provider = await this.providerRepo.findOne({ where: { id }, relations: ['integrations'] });
		if (!provider) throw new NotFoundException(this.translations.t('domains.ai.provider_not_found'));
		return provider;
	}

	private async findCustomProviderWithAccess(me: any, id: string): Promise<AiProviderEntity> {
		const myAdminId = tenantId(me);
		if (!myAdminId) throw new ForbiddenException(this.translations.t('domains.ai.provider_not_custom'));

		const provider = await this.providerRepo.findOne({ where: { id, adminId: myAdminId }, relations: ['integrations'] });
		if (!provider) throw new NotFoundException(this.translations.t('domains.ai.provider_not_found'));
		return provider;
	}

	private ensureWritable(resource: { adminId?: string | null; scope?: string }, me: any): void {
		const myAdminId = tenantId(me);
		const isSuperAdmin = me.role?.name === SystemRole.SUPER_ADMIN;
		const isSystem = resource.scope === AiEntityScope.SYSTEM || resource.scope === 'system';

		if (isSystem && !isSuperAdmin) {
			throw new ForbiddenException(this.translations.t('domains.ai.provider_is_system'));
		}
	...
	private async findModelWithAccess(me: any, id: string): Promise<AiModelEntity> {
		const myAdminId = tenantId(me);
	...
	private toProviderResponse(entity: AiProviderEntity, models?: AiModelEntity[]): ProviderResponseDto {
		return {
			id: entity.id,
			code: entity.code,
			name: entity.name,
			...
			models: models
				? models.filter((m) => m.isActive).map((m) => ({ id: m.id, modelCode: m.modelCode }))
				: undefined,
	...
	private toIntegrationResponse(entity: AiIntegrationEntity, models?: ModelResponseDto[]): IntegrationResponseDto {
		return {
			...entity,
			adminId: entity.adminId,
			providerId: entity.providerId,
	...
	private toSummaryResponse(entity: AiRequestSummaryEntity): RequestSummaryResponseDto {
		return {
			id: entity.id,
			adminId: entity.adminId,
	...
	private toWriteCallResponse(entity: AiWriteToolCallEntity): WriteToolCallResponseDto {
		return {
			id: entity.id,
			adminId: entity.adminId,
			sessionId: entity.sessionId,
