...
class PhaseTimer {
	private readonly phases: Array<{ name: string; ms: number; detail?: Record<string, unknown> }> = [];
	private readonly startedAt = performance.now();
	private active?: { name: string; startedAt: number; detail?: Record<string, unknown> };
	...
	summarize(): { totalMs: number; phases: Array<{ name: string; ms: number; pct: string; detail?: Record<string, unknown> }> } {
...
export class AiOrchestratorService {
	...
	async chat(me: any, userMessage: string, options: AiChatOptions = {}): Promise<AiOrchestrationResult> {
		...
		messages.push({ role: 'system', content: this.systemPromptService.build(ctx) });
		timer.stop({ systemPromptLen: messages[0].content?.length ?? 0 });

		let historyCount = 0;
		timer.start('buildMessages.history', { historyProvided: options.history?.length ?? 0 });
		if (options.history?.length) {
			const sanitisedHistory = options.history.filter((m) => m.role !== 'system');
		...
		try {
			...
			timer.stop({ rounds: execution.currentRound, outcome: result.error ? 'error' : 'ok' });

			timer.start('finalize');
			const finalized = this.finalize(ctx, execution, result, masked.pairs, timer);
			...
			timer.stop({ rounds: execution.currentRound, outcome: 'exception', errorKind: errorDetails?.kind });
			timer.start('finalize.errorPath');
			const finalized = this.finalize(ctx, execution, {
		...
		const isDevEnv = nodeEnv === 'development' || nodeEnv === 'dev' || nodeEnv === 'local';
		if (isDevEnv) {
	...
	private async runLoop(
		...
		const toolSpecs = ctx.session.allowedToolNames?.length
			? allToolSpecs.filter((t) => ctx.session.allowedToolNames!.includes(t.name))
			: allToolSpecs;
		timer.stop({ allCount: allToolSpecs.length, allowedCount: toolSpecs.length });

		const seenToolCalls = new Set<string>();
		timer.start('resolveProviders');
		const { candidates, userExplicitChoice, primary } = await this.resolveProviders(ctx);
		timer.stop({
		...
		for (let round = 1; round <= maxRounds; round++) {
			...
			timer.start(`callProvider.r${round}`, { round, candidates: candidates.length, isLastRound });
			...
			timer.stop({ provider: provider.kind, role: result.role, hasTools: !!result.toolCalls?.length, hasContent: typeof result.content === 'string' });
			...
			if (result.role === 'assistant' && result.toolCalls?.length) {
				...
				timer.start(`dedupTools.r${round}`, { toolCalls: result.toolCalls.length });
				...
				timer.stop({ kept: newToolCalls.length, skipped: result.toolCalls.length - newToolCalls.length });
				...
				timer.start(`executeTools.r${round}`, { count: newToolCalls.length, tools: toolNames });
				const toolMessages = await Promise.all(
					newToolCalls.map((toolCall) => this.executeToolCall(ctx, execution, provider, toolCall, round)),
				);
				timer.stop();
	...
	private async resolveProviders(ctx: AiToolContext): Promise<{ primary: AiProviderAbstract; candidates: AiProviderAbstract[]; userExplicitChoice: boolean }> {
		...
		if (userExplicitChoice) {
			this.logger.debug('[perf] resolveProviders — user explicit choice, no failovers', {
				requestId: ctx.requestId,
				primary: primary.kind,
				selectMs,
				usedDefaultModel,
		...
		const failovers = await this.providerSelector.failoverCandidates(excludeKey, ctx.session.tenantId);
	...
	private async callProviderWithFailover(
		...
		const attemptTimes: Array<{ provider: string; ms: number; ok: boolean; errorCode?: string }> = [];
		...
		for (const candidate of candidates) {
			...
			try {
				...
				const toolCallsLen = 'toolCalls' in result ? result.toolCalls.length : 0;
				...
				attemptTimes.push({ provider: candidate.kind, ms, ok: false, errorCode: code });
	...
	private async executeToolCall(
		...
		if (!tool) {
			const ms = performance.now() - t0;
			this.logger.debug('[perf] tool.skip.UNKNOWN_TOOL', { requestId: ctx.requestId, round, tool: toolCall.name, ms });
			return {
				role: 'tool',
				toolCallId: toolCall.id,
		...
		if (!tool.canRunFor(ctx)) {
			const ms = performance.now() - t0;
			this.logger.debug('[perf] tool.skip.TOOL_NOT_ALLOWED', { requestId: ctx.requestId, round, tool: tool.name, ms });
			return {
				role: 'tool',
				toolCallId: toolCall.id,
		...
		if (tool.isWrite && !ctx.session.acceptWriteOperations) {
			const ms = performance.now() - t0;
			this.logger.debug('[perf] tool.skip.WRITE_NOT_ACCEPTED', { requestId: ctx.requestId, round, tool: tool.name, ms });
			return {
				role: 'tool',
				toolCallId: toolCall.id,
		...
		if (tool.isWrite && this.config.writeToolDedup.enabled) {
			result = await this.executeWriteToolIdempotently(ctx, execution, tool, toolCall, perToolTimer);
		} else {
			execution.emit({ type: 'tool_start', round, provider: provider.getConfig().name, toolName: tool.name, toolCallId: toolCall.id });
			perToolTimer.start('tool.execute', { tool: tool.name, round, isWrite: tool.isWrite });
		...
		return { role: 'tool', toolCallId: toolCall.id, content: JSON.stringify(result).slice(0, 100_000) };
	}
	...
	private async executeWriteToolIdempotently(
		...
		const pending = await this.auditService.findWriteCall(adminId, tool.name, dedupKey);
		...
		if (pending) {
			switch (pending.status) {
				case AiWriteToolCallStatus.COMPLETED:
					...
					this.logger.debug('[perf] writeTool.hit.COMPLETED_dedup', { requestId: ctx.requestId, tool: tool.name, totalMs: performance.now() - t0 });
				...
				case AiWriteToolCallStatus.PENDING: {
					...
					if (ageMs <= this.config.writeToolDedup.pendingTtlMs) {
						this.logger.debug('[perf] writeTool.hit.STALE_PENDING_young', { requestId: ctx.requestId, tool: tool.name, ageMs });
						return {
							ok: false,
							code: 'STALE_PENDING',
		...
		if (!claimed) {
			this.logger.debug('[perf] writeTool.race.another_claim_won', { requestId: ctx.requestId, tool: tool.name, totalMs: performance.now() - t0 });
			return {
				ok: false,
				code: 'STALE_PENDING',
		...
		try {
			...
			await this.auditService.failWriteCall(adminId, tool.name, dedupKey, message);
			timer.stop();
		...
		execution.emit({ type: 'tool_result', round: execution.currentRound, toolName: tool.name, toolCallId: toolCall.id, result: cappedResult });
		await this.auditService.completeWriteCall(adminId, tool.name, dedupKey, cappedResult);
		timer.stop();
	...
	private finalize(
		...
		if (typeof result.content === 'string') {
			timer.start('finalize.piiUnmask', { enabled: ctx.session.enforcePiiMasking });
			content = ctx.session.enforcePiiMasking ? this.piiMasker.unmask(result.content, pairs) : result.content;
			timer.stop({ contentLen: content?.length ?? 0, pairsCount: pairs.length });
		}
		...
		timer.stop({ eventsCount: progress.length, providersUsed: providersUsed.length, modelsUsed: modelsUsed.length });

		timer.start('finalize.persistSummary');
		const persistDone = this.persistSummary(ctx, execution, result, usage, ok, progress, providersUsed, modelsUsed, rounds);
		persistDone
			.then((ms) => this.logger.debug('[perf] persistSummary', {
				requestId: ctx.requestId,
				ok: true,
				ms,
			}))
			.catch((err) => this.logger.error('[finalize] persistSummary failed', err));
		timer.stop();
