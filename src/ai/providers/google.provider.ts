...
import { AiProviderAbstract, boolEnv, intEnv, floatEnv, strEnv } from './ai-provider.abstract';
...
export class GoogleProvider extends AiProviderAbstract {
	...
	protected buildClient(): GoogleGenAI {
		if (!this.apiKey) {
			throw new Error(`Missing API key for provider '${this.kind}'(env or integration)`);
		}
	...
	constructor() {
		...
		this.apiKey = strEnv(process.env[`${prefix}_API_KEY`], process.env.GOOGLE_API_KEY ?? '');
