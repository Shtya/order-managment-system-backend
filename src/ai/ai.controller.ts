import { Body, Controller, Delete, ForbiddenException, Get, Param, ParseUUIDPipe, Patch, Post, Put, Query, Req, Res, UseGuards } from '@nestjs/common';
...
export class AiController {
	...
	@ApiOperation({ summary: 'List all providers visible to the current tenant with lightweight models' })
	...
	async exportProviders(@Req() req: any, @Query() query: ExportProvidersQueryDto, @Res() res: Response) {
		...
		res.setHeader('Content-Disposition', `attachment; filename=ai_providers_export_${Date.now()}.xlsx`);
	...
	@Permissions(AI_MANAGE)
	@Patch('providers/:id')
	async updateProvider(@Req() req: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateProviderDto) {
	...
	@Permissions(AI_MANAGE)
	@Delete('providers/:id')
	async deleteProvider(@Req() req: any, @Param('id', ParseUUIDPipe) id: string) {
	...
	@Permissions(AI_MANAGE)
	@Get('models/export')
	async exportModels(@Req() req: any, @Query() query: ExportModelsQueryDto, @Res() res: Response) {
		...
		res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		res.setHeader('Content-Disposition', `attachment; filename=ai_models_export_${Date.now()}.xlsx`);
		return res.send(buffer);
	}
	...
	@Permissions(AI_MANAGE)
	@Patch('models/:id')
	async updateModel(@Req() req: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateModelDto) {
	...
	@Permissions(AI_PERMISSION_CHAT)
	@Get('integrations')
	async listIntegrations(@Req() req: any, @Query() query: ListIntegrationsQueryDto) {
	...
	async getIntegration(@Req() req: any, @Param('providerId', ParseUUIDPipe) providerId: string) {
	...
	async testCredentials(@Req() req: any, @Param('providerId', ParseUUIDPipe) providerId: string, @Query('modelCode') modelCode?: string) {
	...
	async deleteIntegration(@Req() req: any, @Param('providerId', ParseUUIDPipe) providerId: string) {
	...
	@Permissions(AI_MANAGE)
	@Get('audit/requests')
	async listRequestSummaries(@Req() req: any, @Query() query: ListRequestSummariesQueryDto) {
		// this.ensureSuperAdmin(req.user);
		return this.svc.listRequestSummaries(req.user, query);
	}

	@ApiOperation({ summary: 'Get request summary with full progress (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Get('audit/requests/:id')
	async getRequestSummary(@Req() req: any, @Param('id', ParseUUIDPipe) id: string) {
		this.ensureSuperAdmin(req.user);
		return this.svc.getRequestSummary(req.user, id);
	}

	@ApiOperation({ summary: 'Get only progress events for a request (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Get('audit/requests/:id/progress')
	async getRequestSummaryProgress(@Req() req: any, @Param('id', ParseUUIDPipe) id: string) {
		this.ensureSuperAdmin(req.user);
		return this.svc.getRequestSummaryProgress(req.user, id);
	}

	@ApiOperation({ summary: 'Export request summaries as Excel (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Get('audit/requests/export')
	async exportRequestSummaries(@Req() req: any, @Query() query: ExportRequestSummariesQueryDto, @Res() res: Response) {
		this.ensureSuperAdmin(req.user);
		const data = await this.svc.exportRequestSummaries(req.user, query);
		const buffer = await this.exportSvc.requestSummaries(data);
		res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		res.setHeader('Content-Disposition', `attachment; filename=ai_request_summaries_export_${Date.now()}.xlsx`);
		return res.send(buffer);
	}
	...
	async listWriteToolCalls(@Req() req: any, @Query() query: ListWriteToolCallsQueryDto) {
		this.ensureSuperAdmin(req.user);
		return this.svc.listWriteToolCalls(req.user, query);
	}

	@ApiOperation({ summary: 'Get a single write tool call record (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Get('audit/write-calls/:id')
	async getWriteToolCall(@Req() req: any, @Param('id', ParseUUIDPipe) id: string) {
		this.ensureSuperAdmin(req.user);
		return this.svc.getWriteToolCall(req.user, id);
	}

	@ApiOperation({ summary: 'Retry a failed/stale write tool call (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Post('audit/write-calls/:id/retry')
	async retryWriteToolCall(@Req() req: any, @Param('id', ParseUUIDPipe) id: string) {
		this.ensureSuperAdmin(req.user);
		return this.svc.retryWriteToolCall(req.user, id);
	}

	@ApiOperation({ summary: 'Export write tool calls as Excel (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Get('audit/write-calls/export')
	async exportWriteToolCalls(@Req() req: any, @Query() query: ExportWriteToolCallsQueryDto, @Res() res: Response) {
		this.ensureSuperAdmin(req.user);
		const data = await this.svc.exportWriteToolCalls(req.user, query);
		const buffer = await this.exportSvc.writeToolCalls(data);
		res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		res.setHeader('Content-Disposition', `attachment; filename=ai_write_tool_calls_export_${Date.now()}.xlsx`);
		return res.send(buffer);
	}
	...
	@ApiOperation({ summary: 'Update AI module configuration (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Patch('config')
	async updateConfig(
		@Req() req: any,
		@Body() dto: { enabled?: boolean; defaultProvider?: string; maxRoundtrips?: number; piiMaskingEnabled?: boolean },
	) {
		this.ensureSuperAdmin(req.user);
	...
	@ApiOperation({ summary: 'Report AI module status and enabled providers (super_admin only)' })
	@Permissions(AI_PERMISSION_CHAT)
	@Get('status')
	...
	@ApiOperation({ summary: 'List the tools the current user may invoke (super_admin only)' })
	@Permissions(AI_PERMISSION_CHAT)
	@Get('tools')
	...
	@ApiOperation({ summary: 'Force a health check on a specific provider (super_admin only)' })
	@Permissions(AI_MANAGE)
	@Post('health/:providerCode/test')
	async testProviderHealth(@Req() req: any, @Param('providerCode') providerCode: string) {
		this.ensureSuperAdmin(req.user);
