...
export class ShippingService {
  ...
  async printMassAWB(me: any, providerCode: string, dto: PrintMassAWBDto) {
    ...
    for (const order of orders) {
      ...
      if (
        ...
        throw new BadRequestException(
          this.translations.t("domains.shipping.order_invalid_awb_status", {
            args: {
              orderNumber: order.orderNumber,
  ...
  private getProvider(provider: string | ProviderCode): ShippingProvider {
    ...
    if (!p) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.unsupported_provider", {
          args: {
            provider,
          },
        }),
  ...
  private async getCompanyByProviderForAdmin(
    _adminId: string,
    provider: string,
  ) {
    ...
    if (!company) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.company_not_found", {
          args: {
            provider,
          },
        }),
  ...
  private async getOrCreateIntegration(adminId: string, companyId: string) {
    let integ = await this.integrationsRepo.findOne({
      where: { adminId, shippingCompanyId: companyId },
    });
    if (!integ) {
  ...
  private async requireApiKey(
    ...
    companyId: string;
    integId: string;
    integ: ShippingIntegrationEntity;
  }> {
    ...
    if (!integ.isActive) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.integration_disabled"),
      );
    }
    ...
    if (!apiKey) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.credentials_not_configured"),
      );
    }
  ...
  private async validateProviderConnection(
    ...
    const accountId = integ?.credentials?.accountId
      ? String(integ?.credentials?.accountId).trim()
      : integ?.credentials?.accountId || undefined;
    const apiKey = integ?.credentials?.apiKey;
    ...
    if (!valid) {
      throw new BadRequestException(
        message ||
          this.translations.t("domains.shipping.connection_failed", {
            args: { provider },
          }),
      );
    }
  ...
  async setCredentials(adminId: string, provider: string, credentials: any) {
    ...
    const next = {
      ...
      apiKey: credentials.apiKey
        ...
        : integ.credentials?.apiKey || undefined,
      accountId: credentials.accountId
        ? String(credentials.accountId).trim()
        : integ.credentials?.accountId || undefined,

      // optional webhook config persistence
      webhookHeaderName: credentials.webhookHeaderName
        ? String(credentials.webhookHeaderName).trim()
        : integ.credentials?.webhookHeaderName || undefined,
      webhookSecret: credentials.webhookSecret
        ? String(credentials.webhookSecret).trim()
        : integ.credentials?.webhookSecret || undefined,
    };

    if (!next.apiKey) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.api_key_missing"),
      );
    }
  ...
  async setActive(adminId: string, provider: string, isActive: boolean) {
    ...
    if (isActive) {
      ...
      if (!apiKey) {
        throw new BadRequestException(
          this.translations.t("domains.shipping.api_key_not_found", {
            args: { provider },
          }),
        );
      }
  ...
  async createShipment(
    ...
    try {
      ...
      if (!orderId) {
        throw new BadRequestException(
          this.translations.t("domains.shipping.order_required"),
        );
      }
      ...
      if (!order) {
        throw new BadRequestException(
          this.translations.t("domains.shipping.order_not_found"),
        );
      }

      // Validation: Order status
      if (
        ![OrderStatus.CONFIRMED, OrderStatus.FAILED_DELIVERY].includes(
          order.status?.code as OrderStatus,
        )
      ) {
        throw new BadRequestException(
          this.translations.t("domains.shipping.invalid_order_status", {
            args: { status: order.status?.code },
          }),
        );
      }
        ...
        try {
          if (!prevShipment.shippingCompanyId) {
            ...
            await this.cancelShipment(
              { id: adminId, adminId, role: { name: "admin" } },
              prevShipment.shippingCompany.code,
              prevShipment.id,
            );
          }
        } catch (e: any) {
          const prevTracking =
            prevShipment?.trackingNumber || prevShipment?.providerShipmentId;
          throw new BadRequestException(
            this.translations.t(
              "domains.shipping.previous_shipment_not_cancelled",
              { args: { prevTracking } },
            ),
          );
        }
      ...
      const result = await this.dataSource.transaction(async (manager) => {
        ...
        const newStatusCode =
          settings.orderFlowPath === OrderFlowPath.SHIPPING
            ? OrderStatus.SHIPPED
            : OrderStatus.DISTRIBUTED;
        ...
        if (isNoneProvider) {
          const status = await this.ordersService.findStatusByCode(
            newStatusCode,
            adminId,
            manager,
          );

          const trackingNumber = await this.generateUniqueManualTrackingNumber(
            adminId,
            manager,
          );
          ...
          await this.ordersService.logOrderAction({
            ...
            details: await this.requestTranslations.tAsync(
              "domains.shipping.courier_assigned_manual",
              adminId,
              { args: { trackingNumber } },
            ),
          });
          ...
          if (queryRunner) {
            ...
            queryRunner.data.postCommitTasks.push(() =>
              dispatchShipmentCreated(manualShipment.id),
            );
          } else {
        ...
        await this.ordersService.logOrderAction({
          ...
          shippingCompanyId: companyId,
          details: await this.requestTranslations.tAsync(
            "domains.shipping.courier_assigned_provider",
            adminId,
            { args: { provider, trackingNumber: shipment.trackingNumber } },
          ),
        });
        ...
        if (queryRunner) {
          ...
          queryRunner.data.postCommitTasks.push(() =>
            dispatchShipmentCreated(shipment.id),
          );
        } else {
        ...
        title:
          provider === "none"
            ? await this.requestTranslations.tAsync(
                "domains.shipping.order_distributed_title",
                adminId,
              )
            : await this.requestTranslations.tAsync(
                "domains.shipping.shipment_created_title",
                adminId,
              ),
        message:
          provider === "none"
            ? await this.requestTranslations.tAsync(
                "domains.shipping.manual_shipping_assigned",
                adminId,
                {
                  args: {
                    orderNumber: order.orderNumber,
            ...
            : await this.requestTranslations.tAsync(
                "domains.shipping.shipment_created_successfully",
                adminId,
                {
                  args: {
                    orderNumber: order.orderNumber,
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_creation_failed",
          adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_failed_message",
          adminId,
          { args: { orderNumber: order?.orderNumber, erorr: errorMessage } },
        ),
        relatedEntityType: "order",
        relatedEntityId: String(order?.id),
      });
  ...
  async cancelShipment(me, provider: string, shipmentId: string) {
    ...
    if (!shipment) {
      throw new NotFoundException(
        this.translations.t("domains.shipping.shipment_not_found"),
      );
    }
    if (shipment.status === ShipmentStatus.CANCELLED) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.shipment_already_cancelled"),
      );
    }

    return await this.dataSource.transaction(async (manager) => {
      try {
        const isCancelled = await p.cancelShipment(
          apiKey,
          shipment.providerShipmentId || shipment.trackingNumber,
        );

        if (!isCancelled) {
          throw new Error(
            this.translations.t("domains.shipping.provider_cancel_failed"),
          );
        }
        ...
        const result = {
          ...
          message: this.translations.t(
            "domains.shipping.shipment_cancel_success",
          ),
          shipmentId: shipment.id,
          status: shipment.unifiedStatus,
        };
        ...
        await this.notificationService.create({
          ...
          title: await this.requestTranslations.tAsync(
            "domains.shipping.shipment_cancelled_title",
            adminId,
          ),
          message: await this.requestTranslations.tAsync(
            "domains.shipping.shipment_cancelled_message",
            adminId,
            { args: { orderNumber: shipment.order.orderNumber } },
          ),
          relatedEntityType: "order",
  ...
  async assignOrder(
    ...
    const adminId = tenantId(me);
    return this.createShipment(me, provider, dto, orderId, {
      emitSocket: false,
    });
  }
  ...
  async bulkAssignOrders(
    ...
    const adminId = tenantId(me);
    await this.orderSyncQueueService.enqueueBulkShippingTasks(
      adminId,
      provider,
      dto,
    );
    return {
      success: true,
      message: await this.translations.t(
        "domains.shipping.bulk_shipping_enqueued",
        { args: { count: dto.items.length } },
      ),
  ...
  async getShipmentByTrackingNumber(adminId: string, trackingNumber: string) {
    const s = await this.shipmentsRepo.findOne({
  ...
  async listExternalShipmentLogs(me: any, q?: any) {
    ...
    const qb = this.externalShipmentLogsRepo
  ...
  async exportExternalShipmentLogs(me: any, q?: any) {
    const { records } = await this.listExternalShipmentLogs(me, {
      ...q,
      limit: 1000,
      page: 1,
    });
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.shipping.external_shipment_logs"),
    );
  ...
  async trackShipment(me: any, trackingNumber: string) {
    ...
    if (!shipment.shippingCompany) {
      throw new BadRequestException(
        this.translations.t("domains.shipping.shipment_company_not_found"),
      );
    }

    const provider = this.getProvider(shipment.shippingCompany.code);
    const integ = await this.getOrCreateIntegration(
      adminId,
    ...
    const { unifiedStatus, providerShipmentId, rawState } =
      await provider.getShipmentStatus(
        apiKey,
    ...
    const getDistributedBaseQuery = () =>
      this.ordersRepo
        ...
        .leftJoin(
          ...
          "integ",
          "integ.shippingCompanyId = order.shippingCompanyId AND integ.adminId = :adminId AND integ.isActive = true",
          { adminId },
        )
        .leftJoinAndSelect(
  ...
  async getWebhookSetup(adminId: string, provider: string) {
    ...
    const headerName = (
      integ.credentials?.webhookHeaderName || "Authorization"
    ).trim();
  ...
  async handleWebhook(
    ...
    try {
      ...
      if (!company) {
        return { ok: true, ignored: true, reason: "company_not_found" };
      }

      const integ = await this.integrationsRepo.findOne({
        where: { adminId: shipment.adminId, shippingCompanyId: company.id },
      });
      ...
      if (savedSecret) {
        ...
        const isAuthed = p.verifyWebhookAuth(
          headers,
          body,
          savedSecret,
          customHeaderName,
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        if (!s) return;
        await this.applyMappedUnifiedStatusInTransaction(
          manager,
          s as any,
          mapped,
          {
  ...
  private async generateUniqueManualTrackingNumber(
    adminId: string,
    ...
    throw new BadRequestException(
      this.translations.t(
        "domains.shipping.could_not_allocate_unique_manual_tracking_number",
      ),
    );
  }
  ...
  async cancelManualShipment(me: any, shipmentId: string) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (shipment.status === ShipmentStatus.CANCELLED) {
        return {
          ok: true,
          message: this.translations.t("domains.shipping.already_cancelled"),
          shipmentId: shipment.id,
        };
      }
      ...
      return {
        ...
        shipmentId: shipment.id,
        message: this.translations.t(
          "domains.shipping.manual_shipment_cancelled",
        ),
      };
    });
  }
  ...
  private async applyMappedUnifiedStatusInTransaction(
    ...
    const returnStock = async () => {
      const itemsToRestock = (order.items ?? []).filter(
        (item) => item.variantId && getEffectiveDeductedQuantity(item) > 0,
      );
      if (itemsToRestock.length > 0) {
        ...
        itemsToRestock.forEach((item) => {
          ...
          restockMap.set(
            item.variantId,
            current + getEffectiveDeductedQuantity(item),
          );
        });
        const restockUpdates = Array.from(restockMap.entries()).map(
          ([id, qty]) =>
            manager
              .createQueryBuilder()
    ...
    if (mapped.unifiedStatus === UnifiedShippingStatus.DELIVERED) {
      const deliveredStatus = await manager.findOne(OrderStatusEntity, {
      ...
      await manager.save(order);
      await this.ordersService.deductStockForOrder(
        manager,
        order?.id,
        shipment?.adminId,
        { skipValidation: true },
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_delivered_title",
          shipment.adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_delivered_message",
          shipment.adminId,
          {
            args: {
              orderNumber: order.orderNumber,
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_delivary_failed_title",
          shipment.adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.shipping.shipment_delivary_failed_message",
          shipment.adminId,
          {
            args: {
              orderNumber: order.orderNumber,
    ...
    if (statusChanged && oldStatusId !== order.statusId) {
      await this.ordersService.logStatusChange({
        ...
        notes: await this.requestTranslations.tAsync(
          "domains.shipping.automated_status_update_from_provider",
          shipment.adminId,
          {
            args: {
  ...
  async updateShipmentStatusManually(
    me: any,
    shipmentId: string,
    dto: ManualUpdateShipmentStatusDto,
  ) {
    const adminId = tenantId(me);
    const shipment = await this.shipmentsRepo.findOne({
      where: { id: shipmentId, adminId },
    ...
    if (shipment.unifiedStatus === UnifiedShippingStatus.DELIVERED) {
      throw new BadRequestException(
        "Cannot change shipment status after it has been delivered.",
      );
    }
  ...
  private mapUnifiedToLegacy(u: UnifiedShippingStatus): ShipmentStatus {
    ...
    if (u === UnifiedShippingStatus.CUSTOMER_DATA_WRONG) {
      return ShipmentStatus.CUSTOMER_DATA_WRONG;
    }
    if (u === UnifiedShippingStatus.CUSTOMER_NOT_RESPOND) {
      return ShipmentStatus.CUSTOMER_NOT_RESPOND;
    }
    if (u === UnifiedShippingStatus.CUSTOMER_REFUSED) {
      return ShipmentStatus.CUSTOMER_REFUSED;
    }

    if (
      [
        UnifiedShippingStatus.EXCEPTION,
        UnifiedShippingStatus.LOST,
        UnifiedShippingStatus.DAMAGED,
        UnifiedShippingStatus.TERMINATED,
      ].includes(u)
    ) {
