...
export class ShipmentSubscriber implements EntitySubscriberInterface<ShipmentEntity> {
  ...
  async afterUpdate(event: UpdateEvent<ShipmentEntity>) {
    ...
    this.logger.log(
      ...
      JSON.stringify(previousShipment),
    );
    ...
    const hasStatusChanged =
      !previousShipment || (newStatus && oldStatus !== newStatus);
  ...
  private queueShipmentTrigger(
    ...
    const runAfterCommit = async () => {
      try {
        ...
        this.logger.error(
          "[ShipmentSubscriber] Error dispatching SHIPMENT_UPDATED trigger:",
          error,
        );
      }
    };
    ...
    if (tasks && tasks.length > 0) {
      for (const task of tasks) {
        try {
          ...
          this.logger.error(
            "[ShipmentSubscriber] Post-commit task failed:",
            error,
          );
        }
      }
