...
export class SafesService {
  ...
  async listAccounts(me: any, q: AccountFilterDto) {
    ...
    qb.addSelect((subQuery) => {
      return subQuery
        ...
        .from(FinancialTransaction, "t")
        .where("t.accountId = a.id")
        .andWhere("t.direction = :outDir", {
          outDir: TransactionDirection.OUT,
        });
    }, "totalOut");
  ...
  async listTransactions(me: any, q: TransactionFilterDto) {
    ...
    if (q.accountId) {
      qb.andWhere("t.accountId = :accountId", { accountId: q.accountId });
    }
    if (q.direction) {
      qb.andWhere("t.direction = :direction", { direction: q.direction });
    }
    if (q.referenceType) {
      qb.andWhere("t.referenceType = :referenceType", {
        referenceType: q.referenceType,
      });
    }
    if (q.startDate) {
      qb.andWhere("t.transactionDate >= :start", { start: q.startDate });
    }
  ...
  private async createTransaction(
    ...
    const run = async (em: EntityManager) => {
      ...
      if (
        ...
        throw new BadRequestException(
          this.translations.t("domains.safe.insufficient_account_balance", {
            args: {
      ...
      const balanceChange =
        data.direction === TransactionDirection.IN ? amount : -totalDeduction;
  ...
  async listTransfers(me: any, q: TransferFilterDto) {
    ...
    if (q.fromAccountId) {
      qb.andWhere("tr.fromAccountId = :fromId", { fromId: q.fromAccountId });
    }
    if (q.toAccountId) {
      qb.andWhere("tr.toAccountId = :toId", { toId: q.toAccountId });
    }
    if (q.startDate) {
      qb.andWhere("tr.createdAt >= :start", { start: q.startDate });
    }
  ...
  async getStats(me: any) {
    ...
    transactionStats.forEach((row) => {
      if (row.direction === TransactionDirection.IN) {
        stats.totalIn = Number(row.total);
      }
      if (row.direction === TransactionDirection.OUT) {
        stats.totalOut = Number(row.total);
      }
    });
  ...
  private async generateTrxNumber(em: EntityManager): Promise<string> {
    ...
    const lastTrx = await em
      .createQueryBuilder(FinancialTransaction, "t")
