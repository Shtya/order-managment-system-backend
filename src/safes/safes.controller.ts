...
export class SafesController {
  ...
  async exportAccounts(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=accounts-${Date.now()}.xlsx`,
    );
    res.end(buffer);
  }
  ...
  async exportTransfers(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=transfers-${Date.now()}.xlsx`,
    );
    res.end(buffer);
  }
}
