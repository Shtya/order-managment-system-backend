...
const ACHIEVEMENT_TRANSLATION_KEYS: Record<
  GettingStartedAchievementType,
  I18nKey
> = {
  [GettingStartedAchievementType.FIRST_PRODUCT_CREATED]:
    "domains.getting_started.achievements.first_product_created",
  [GettingStartedAchievementType.FIRST_WAREHOUSE_CREATED]:
    "domains.getting_started.achievements.first_warehouse_created",
  [GettingStartedAchievementType.FIRST_ORDER_CREATED]:
    "domains.getting_started.achievements.first_order_created",
  [GettingStartedAchievementType.SHIPPING_INTEGRATION_CONNECTED]:
    "domains.getting_started.achievements.shipping_integration_connected",
  [GettingStartedAchievementType.WHATSAPP_CONNECTED]:
    "domains.getting_started.achievements.whatsapp_connected",
  [GettingStartedAchievementType.STORE_CONNECTED]:
    "domains.getting_started.achievements.store_connected",
  [GettingStartedAchievementType.FIRST_TEAM_MEMBER_CREATED]:
    "domains.getting_started.achievements.first_team_member_created",
  [GettingStartedAchievementType.FIRST_AUTOMATION_CREATED]:
    "domains.getting_started.achievements.first_automation_created",
  [GettingStartedAchievementType.FIRST_SAFE_CREATED]:
    "domains.getting_started.achievements.first_safe_created",
  [GettingStartedAchievementType.FIRST_PURCHASE_ACCEPTED]:
    "domains.getting_started.achievements.first_purchase_accepted",
  [GettingStartedAchievementType.FIRST_SUPPLIER_CREATED]:
    "domains.getting_started.achievements.first_supplier_created",
  [GettingStartedAchievementType.FIRST_ORDER_ASSIGNMENT_AUTOMATION_RULE_CREATED]:
    "domains.getting_started.achievements.first_order_assignment_automation_rule_created",
  [GettingStartedAchievementType.FIRST_ORDER_BUNDLE_CREATED]:
    "domains.getting_started.achievements.first_order_bundle_created",
  [GettingStartedAchievementType.FIRST_CUSTOM_ROLE_CREATED]:
    "domains.getting_started.achievements.first_custom_role_created",
};
...
export class OnboardingAchievementProcessor extends WorkerHost {
  ...
  async process(job: Job<ProcessAchievementJobDto>): Promise<any> {
    ...
    this.logger.log(
      `Processing achievement job ${job.id} | Admin: ${adminId} | Type: ${type}`,
    );

    try {
      const result = await this.dataSource.transaction(async (manager) => {
        const achievementRepo = manager.getRepository(
          GettingStartedAchievementEntity,
        );
        ...
        if (existing) {
          this.logger.log(
            `Achievement already recorded, skipping | Admin: ${adminId} | Type: ${type}`,
          );
        ...
        this.logger.log(
          `Achievement recorded | Admin: ${adminId} | Type: ${type}`,
        );

        const totalTypes = Object.keys(GettingStartedAchievementType).length;
        const completedCount = await achievementRepo.count({
          where: { adminId },
        });
        const allComplete = completedCount >= totalTypes;

        return { recorded: true, reason: "recorded", achievement, allComplete };
      });
      ...
      if (result.allComplete) {
        await this.sendAllCompleteNotification(adminId);
      }
  ...
  private async sendAchievementNotification(
    ...
    try {
      ...
      this.logger.log(
        `Achievement notification sent | Admin: ${adminId} | Type: ${type}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async sendAllCompleteNotification(adminId: string) {
    try {
      const title = await this.requestTranslations.tAsync(
        "domains.getting_started.all_complete_title",
        adminId,
      );
      const message = await this.requestTranslations.tAsync(
        "domains.getting_started.all_complete_message",
        adminId,
      );
      ...
      if (notification) {
        this.appGateway.emitNewNotification(adminId, notification);
      }

      this.logger.log(`All-complete notification sent | Admin: ${adminId}`);
    } catch (error) {
      this.logger.error(
        `Failed to send all-complete notification | Admin: ${adminId}`,
        error instanceof Error ? error.stack : error,
      );
    }
  }
