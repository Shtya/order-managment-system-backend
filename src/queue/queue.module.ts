...
} from "./queues/auto-assignment.queue";
...
} from "./queues/product-sync.queue";
import {
  OrderSyncQueueService,
  OrderSyncWorkerService,
} from "./queues/order-sync.queue";
import {
...
const registeredQueues = Object.values(QueueNames).map((queueName) => ({
  ...
  metrics: {
    ...
    maxDataPoints:
      queueName === QueueNames.PRODUCT_SYNC ? MetricsTime.ONE_WEEK * 2 : 60,
  },
}));
