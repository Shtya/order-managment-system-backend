...
export class CollectionService {
  ...
  async getCollectionStatistics(me: any) {
    ...
    const otherLocalization = await this.translations.t(
      "domains.collections.direct_other",
    );
    ...
    const [generalStats, shippingBreakdownRaw] = await Promise.all([
      ...
      this.ordersRepo
        ...
        .addSelect(
          "SUM(CASE WHEN COALESCE(o.collectedAmount,0) = 0 THEN 1 ELSE 0 END)",
          "nonCollectedOrdersCount",
        )
        .addSelect(
          "SUM(CASE WHEN COALESCE(o.collectedAmount,0) >= (o.finalTotal) THEN 1 ELSE 0 END)",
          "collectedOrdersCount",
        )
        .addSelect(
          "SUM(CASE WHEN (o.finalTotal) > COALESCE(o.collectedAmount,0) THEN (o.finalTotal) - COALESCE(o.collectedAmount,0) ELSE 0 END)",
          "totalNonCollectedMoney",
        )
    ...
    const stats = {
      ...
      totalPartialCollectedMoney:
        parseFloat(generalStats?.totalPartialCollectedMoney) || 0,
      ...
      totalNonCollectedMoney:
        parseFloat(generalStats?.totalNonCollectedMoney) || 0,
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!order) {
        throw new NotFoundException(
          this.translations.t("domains.collections.order_not_found", {
            args: { orderId: dto.orderId },
          }),
        );
      }
      ...
      if (!safe) {
        throw new BadRequestException(
          this.translations.t("domains.collections.safe_not_found", {
            args: { safeId: dto.safeId },
          }),
        );
      }
      if (safe.status !== AccountStatus.ACTIVE) {
        throw new BadRequestException(
          this.translations.t("domains.collections.safe_not_active", {
            args: { safeId: dto.safeId },
          }),
        );
      }
      ...
      if (dto.shippingCompanyId) {
        ...
        if (!shippingIntegration) {
          throw new NotFoundException(
            this.translations.t(
              "domains.collections.shipping_integration_required",
              { args: { shippingCompanyId: dto.shippingCompanyId } },
            ),
          );
        }
      }
      ...
      order.collectedAmount =
        (Number(order.collectedAmount) || 0) + Number(dto.amount);
        ...
        me,
        {
          ...
          referenceMeta: {
            shippingCompanyProvider:
              shippingIntegration?.shippingCompany?.code || null,
            trackingNumber: order.trackingNumber || null,
            orderNumber: order.orderNumber || null,
          },
          notes: await this.requestTranslations.tAsync(
            "domains.collections.collection_deposit_notes",
            adminId,
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.collections.collection_created_title",
          adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.collections.collection_created_message",
          adminId,
          {
            args: {
              amount: dto.amount,
  ...
  async listCollections(me: any, q?: any) {
    ...
    if (q?.shippingCompanyId) {
      qb.andWhere("order.shippingCompanyId = :shippingId", {
        shippingId: q.shippingCompanyId,
      });
    }
    ...
    if (q?.collectionStatus) {
      ...
      if (q.collectionStatus === "not_collected") {
        ...
        qb.andWhere(`${amt} > 0 AND ${amt} < ${collectible}`).andWhere(
          deliveredCondition,
        );
      } else if (q.collectionStatus === "fully_collected") {
        qb.andWhere(`${amt} >= ${collectible}`).andWhere(`${amt} > 0`);
      } else if (q.collectionStatus === "pending") {
        // لم يكتمل التحصيل + يجب أن يكون مستلم
        qb.andWhere(`${amt} < ${collectible}`).andWhere(deliveredCondition);
      }
    }
    ...
    const records = orders.map((order) => {
      ...
      const isFullyCollected =
        (order.collectedAmount || 0) >= collectibleAmount;
      ...
      return {
        ...
        remainingBalance: Math.max(
          0,
          collectibleAmount - (order.collectedAmount || 0),
        ),
  ...
  async exportCollections(me: any, q?: any) {
    ...
    if (search) {
      qb.andWhere(
        new Brackets((sq) => {
          sq.where("order.orderNumber ILIKE :s", { s: `%${search}%` }).orWhere(
            "order.customerName ILIKE :s",
            { s: `%${search}%` },
          );
        }),
      );
    ...
    if (statusFilter) {
      ...
      if (statusFilter === "not_collected") {
        qb.andWhere(`${amt} = 0`).andWhere(deliveredCondition);
      } else if (statusFilter === "partial") {
        qb.andWhere(`${amt} > 0 AND ${amt} < ${collectible}`).andWhere(
          deliveredCondition,
        );
      } else if (statusFilter === "fully_collected") {
    ...
    if (isFullyCollected) {
      ...
      columns = [
        {
          header: this.translations.t(
            "domains.collections.export_order_number",
          ),
          key: "orderNumber",
          width: 15,
        },
        {
          header: this.translations.t(
            "domains.collections.export_shipping_company",
          ),
          key: "shippingCompany",
          width: 20,
        },
        {
          header: this.translations.t(
            "domains.collections.export_last_collection_date",
          ),
        ...
        {
          header: this.translations.t(
            "domains.collections.export_shipping_cost",
          ),
          key: "shippingCost",
          width: 15,
        },
        {
          header: this.translations.t(
            "domains.collections.export_total_amount",
          ),
          key: "finalTotal",
          width: 15,
        },
        {
          header: this.translations.t(
            "domains.collections.export_collectible_amount",
          ),
        ...
        {
          header: this.translations.t(
            "domains.collections.export_collected_amount",
          ),
          key: "collectedAmount",
          width: 15,
        },
        {
          header: this.translations.t(
            "domains.collections.export_remaining_balance",
          ),
      ...
      columns = [
        ...
        {
          header: this.translations.t(
            "domains.collections.export_collection_method",
          ),
        ...
        {
          header: this.translations.t(
            "domains.collections.export_delivered_at",
          ),
          key: "deliveredAt",
          width: 18,
        },
        ...
        {
          header: this.translations.t("domains.collections.export_delay_days"),
          key: "delayDays",
          width: 12,
        },
      ];
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.collections.export_collections_sheet"),
    );
    ...
    const rows = orders.map((order) => {
          ...
          ? new Date(
              Math.max(
                ...order.collections.map((c) =>
                  new Date(c.collectedAt).getTime(),
                ),
              ),
            ).toLocaleDateString()
          : "—";
      ...
      const methods =
        [...new Set(order.collections?.map((c) => c.source))].join(", ") || "—";
      ...
      if (order.deliveredAt) {
        ...
        delay = Math.floor(
          (end.getTime() - new Date(order.deliveredAt).getTime()) / 86400000,
        );
      }
      ...
      return {
        ...
        collectionStatus:
          remaining > 0
            ? collected > 0
              ? this.translations.t("domains.collections.status_partial")
              : this.translations.t("domains.collections.status_pending")
            : this.translations.t("domains.collections.status_fully_collected"),
