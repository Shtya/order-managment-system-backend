...
import {
  ...
  ProductSyncErrorLogEntity,
  ProductSyncAction,
  ProductSyncStatusDto,
  SyncEntityType,
...
export class ProductSyncStateService {
  ...
  async list(me: any, q?: any) {
    ...
    if (q?.storeId) {
      qb.andWhere("syncState.storeId = :storeId", { storeId: q.storeId });
    }
    if (q?.status) {
      qb.andWhere("syncState.status = :status", { status: q.status });
    }
  ...
  async getById(me: any, id: string) {
    ...
    if (!record) {
      throw new NotFoundException(
        this.translations.t("domains.product_sync.state_not_found"),
      );
    }
  ...
  async export(me: any, q?: any) {
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.product_sync.state_sheet"),
    );
    ...
    worksheet.columns = [
      ...
      {
        header: this.translations.t("domains.product_sync.product_name"),
        key: "productName",
        width: 30,
      },
      {
        header: this.translations.t("domains.product_sync.product_slug"),
        key: "productSlug",
        width: 20,
      },
      {
      ...
      {
        header: this.translations.t("domains.product_sync.remote_id"),
        key: "remoteId",
        width: 20,
      },
      {
        header: this.translations.t("domains.product_sync.status"),
        key: "status",
        width: 15,
      },
      {
        header: this.translations.t("domains.product_sync.last_error"),
        key: "lastError",
        width: 40,
      },
      {
        header: this.translations.t("domains.product_sync.last_synced_at"),
        key: "lastSyncedAt",
        width: 20,
      },
      {
    ...
    records.forEach((r) => {
      worksheet.addRow({
        ...
        lastSyncedAt: r.lastSynced_at
          ? r.lastSynced_at.toLocaleString()
          : naText,
  ...
  async listLogs(me: any, q?: any) {
    ...
    if (q?.productId) {
      qb.andWhere("log.productId = :productId", { productId: q.productId });
    }
    if (q?.storeId) {
      qb.andWhere("log.storeId = :storeId", { storeId: q.storeId });
    }
  ...
  async getLogById(me: any, id: string) {
    ...
    if (!record) {
      throw new NotFoundException(
        this.translations.t("domains.product_sync.error_log_not_found"),
      );
    }
  ...
  async exportLogs(me: any, q?: any) {
    ...
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.product_sync.error_logs_sheet"),
    );
    const naText = this.translations.t("domains.product_sync.n_a");
    ...
    worksheet.columns = [
      ...
      {
        header: this.translations.t("domains.product_sync.bundle_name"),
        key: "bundleName",
        width: 30,
      },
      {
        header: this.translations.t("domains.product_sync.store_name"),
        key: "storeName",
        width: 30,
      },
      {
      ...
      {
        header: this.translations.t("domains.product_sync.action"),
        key: "action",
        width: 15,
      },
      {
        header: this.translations.t("domains.product_sync.error_message"),
        key: "error",
        width: 50,
      },
      {
  ...
  async upsertSyncErrorLog(
    ...
    if (!adminId || !storeId) {
      throw new Error(
        this.translations.t(
          "domains.product_sync.admin_id_and_store_id_required",
        ),
      );
    }
    ...
    if (entityType === SyncEntityType.PRODUCT && !productId) {
      throw new Error(
        this.translations.t(
          "domains.product_sync.product_id_required_for_product_entity",
        ),
      );
    }
    ...
    if (entityType === SyncEntityType.BUNDLE && !bundleId) {
      throw new Error(
        this.translations.t(
          "domains.product_sync.bundle_id_required_for_bundle_entity",
        ),
      );
    }
