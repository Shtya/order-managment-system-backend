...
async function seedCitiesFromCsv(cityRepo: Repository<CityEntity>) {
  ...
  const existingById = new Map(
    existingCities.filter((city) => city.id).map((city) => [city.id, city]),
  );
  const existingByNameEn = new Map(
    existingCities
      .filter((city) => city.nameEn)
      .map((city) => [city.nameEn.toLowerCase(), city]),
  );
  ...
  for (const row of cityRows) {
    ...
    const existingCity =
      ...
      existingByNameEn.get(row.nameEn.toLowerCase());
...
async function seedProviderLocationsFromCsv(
  ...
  if (!fs.existsSync(csvPath)) {
    console.log(
      "âš ï¸ cities-provider.csv not found, skipping provider location seeding",
    );
    return;
  }
  ...
  const existingKeyMap = new Map(
    existingProviderLocations.map((location) => [
      `${location.provider}:${location.providerCityId}`,
      location,
    ]),
  );
  const cityMap = new Map(
    (await cityRepo.find()).map((city) => [city.id, city]),
  );
  ...
  for (const columns of rows) {
    ...
    const existingLocation = existingKeyMap.get(
      `${provider}:${providerCityId}`,
    );
    const locationEntity = existingLocation
      ? existingLocation
      : providerLocationRepo.create();
    ...
    locationEntity.providerCityNameEn =
      providerCityNameEn || providerCityNameAr || providerCityId;
...
async function seedAreasFromCsv(
  areaRepo: Repository<AreaEntity>,
  ...
  const existingById = new Map(
    existingAreas.filter((area) => area.id).map((area) => [area.id, area]),
  );
  ...
  for (const row of areaRows) {
    ...
    if (!row.cityId || !cityMap.has(row.cityId)) {
      console.log(
        `âš ï¸ Skipping area ${row.nameEn} without a seeded city id (${row.cityId || "missing"})`,
      );
      continue;
    }
...
async function seedOrderStatuses() {
  ...
  const systemStatuses = [
    ...
    },
    {
      ...
      isDefault: false,
      order: 4,
      color: "#4CAF50", // Ø£Ø®Ø¶Ø± (Ù†Ø¬Ø§Ø­ Ø§Ù„ØªØ£ÙƒÙŠØ¯)
    },
    {
      name: "No Answer",
      ...
      isDefault: false,
      order: 5,
      color: "#FF5722", // Ø¨Ø±ØªÙ‚Ø§Ù„ÙŠ Ù…Ø­Ø±ÙˆÙ‚ (ØªØ­Ø°ÙŠØ±)
    },
    {
      name: "No Answer - Follow Up",
      code: OrderStatus.NO_ANSWER_FOLLOW_UP,
      isDefault: false,
      order: 6,
      color: "#FF5722", // Same as No Answer
    },
    {
      name: "Wrong Number",
    ...
    },
    {
      ...
      isDefault: false,
      order: 8,
      color: "#673AB7", // Ø¨Ù†ÙØ³Ø¬ÙŠ ØºØ§Ù…Ù‚
    },
    {
      name: "Duplicate",
      code: OrderStatus.DUPLICATE,
      isDefault: false,
      order: 9,
      color: "#E91E63", // ÙˆØ±Ø¯ÙŠ (ØªÙ†Ø¨ÙŠÙ‡ ØªÙƒØ±Ø§Ø±)
    },
    {
      name: "Rejected",
      code: OrderStatus.REJECTED,
      isDefault: false,
      order: 10,
      color: "#F44336", // Red (Matches stats.cancelledShipping)
    },
    {
      name: "Cancelled",
    ...
    {
      name: "Cancelled - Follow Up",
      code: OrderStatus.CANCELLED_FOLLOW_UP,
      isDefault: false,
      order: 12,
      color: "#F44336", // Same as Cancelled
    },
    {
      ...
      isDefault: false,
      order: 13,
      color: "#E91E63", // Pink (Alert for delivery issues)
    },
    {
      name: "Distributed",
      code: OrderStatus.DISTRIBUTED,
      isDefault: false,
      order: 14,
      color: "#9C27B0", // Purple (Indicates it's out for handling)
    },
    {
      name: "Printed",
...
interface GettingStartedChecklistStep {
  ...
  actionConfig?: {
    trigger?: "click";
    openFromPreviousStep?: GettingStartedStepOpenFromPrevious;
  } | null;
  sortOrder: number;
}
  ...
  },
  {
    ...
    steps: [
...

      {
        ...
        title: {
          ar: "Ø£Ø¯Ø®Ù„ Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ø±Ø¨Ø·",
          en: "Enter the integration settings",
        },
        description: {
...
async function seedGettingStarted() {
  ...
  for (const checklistItem of gettingStartedChecklist) {
    ...
    const definedStepKeys = new Set(
      checklistItem.steps.map((step) => step.key),
    );
    ...
    const staleSteps = existingSteps.filter(
      (step) => !definedStepKeys.has(step.key),
    );
    if (staleSteps.length > 0) {
      await stepRepo.remove(staleSteps);
      console.log(
        `ðŸ—‘ï¸  Removed ${staleSteps.length} stale step(s) from checklist item "${checklistItem.key}"`,
      );
    }
  }
  ...
  const definedItemKeys = new Set(
    gettingStartedChecklist.map((item) => item.key),
  );
  const staleItems = existingItems.filter(
    (item) => !definedItemKeys.has(item.key),
  );
  if (staleItems.length > 0) {
    await itemRepo.remove(staleItems);
    console.log(
      `ðŸ—‘ï¸  Removed ${staleItems.length} stale getting started checklist item(s)`,
    );
  }

  console.log(
    `âœ… Seeded ${gettingStartedChecklist.length} getting started checklist items`,
  );
}
export const AI_MODEL_SEEDS = [
  {
    ...
    descriptionAr:
      "كود 3 هايكو هو أسرع وأصغر نموذج من أنثروبك لاستجابة سريعة جداً. أداء مستهدف سريع ودقيق. انظر الإعلان الرسمي ونتائج المعايير [هنا](https://www.anthropic.com/news/claude-3-haiku). #متعدد_الوسائط",
  ...
  },
  {
    ...
    descriptionAr:
      "كود فايبل 5 هو نموذج من فئة ميثوس من أنثروبك، مبني للعمل المعرفي المستقل والبرمجة. يدعم مدخلات النص والصورة والملفات مع مخرجات نصية، مع دعم التفكير و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Claude Haiku 4.5 is Anthropicâ€™s fastest and most efficient model, delivering near-frontier intelligence at a fraction of the cost and latency of larger Claude models. Matching Claude Sonnet 4â€™s performance...",
    descriptionAr:
      "كود هايكو 4.5 هو أسرع وأكثر كفاءة نماذج أنثروبك، يقدم ذكاءً قريباً من الحدود العليا بتكلفة وكمون جزء من نماذج كود الأكبر. يطابق أداء كود سونيت 4...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "Claude Opus 4",
    description:
      "Claude Opus 4 is benchmarked as the worldâ€™s best coding model, at time of release, bringing sustained performance on complex, long-running tasks and agent workflows. It sets new benchmarks in...",
    descriptionAr:
      "تم تصنيف كود أوبوس 4 كأفضل نموذج برمجة في العالم عند إصداره، يقدم أداءً مستداماً في المهام المعقدة طويلة الأمد وسير عمل الوكلاء. يضع معايير جديدة في...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Claude Opus 4.1 is an updated version of Anthropicâ€™s flagship model, offering improved performance in coding, reasoning, and agentic tasks. It achieves 74.5% on SWE-bench Verified and shows notable gains...",
    descriptionAr:
      "كود أوبوس 4.1 هو نسخة محدثة من النموذج الرائد لشركة أنثروبك، يوفر أداءً محسناً في البرمجة والاستدلال والمهام الوكيلة. يحقق 74.5% في اختبار SWE-bench Verified ويظهر مكاسب ملحوظة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Claude Opus 4.5 is Anthropicâ€™s frontier reasoning model optimized for complex software engineering, agentic workflows, and long-horizon computer use. It offers strong multimodal capabilities, competitive performance across real-world coding and...",
    descriptionAr:
      "كود أوبوس 4.5 هو نموذج الاستدلال الحدودي من أنثروبك، محسّن للهندسة البرمجية المعقدة وسير العمل الوكيل واستخدام الكمبيوتر طويل الأمد. يوفر قدرات متعددة الوسائط قوية، وأداءً تنافسياً عبر البرمجة الواقعية و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Opus 4.6 is Anthropicâ€™s strongest model for coding and long-running professional tasks. It is built for agents that operate across entire workflows rather than single prompts, making it especially effective...",
    descriptionAr:
      "أوبوس 4.6 هو أقوى نموذج من أنثروبك للبرمجة والمهام المهنية طويلة التشغيل. مبني للوكلاء الذين يعملون عبر سير العمل الكاملة بدلاً من المطالبات المنفردة، مما يجعله فعالاً بشكل خاص...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أوبوس 4.7 هو الجيل التالي من عائلة أوبوس من أنثروبك، مبني للوكلاء غير المتزامنين طويلي التشغيل. بناءً على نقاط القوة في البرمجة والوكالة من أوبوس 4.6، يقدم أداءً أقوى على...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "نسخة الوضع السريع من [أوبوس 4.7](/anthropic/claude-opus-4.7) - قدرات متطابقة مع سرعة إخراج أعلى بتسعير ممتاز 6 أضعاف. تعلم المزيد في وثائق أنثروبك: https://platform.claude.com/docs/en/build-with-claude/fast-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "كود أوبوس 4.8 هو أكثر نماذج عائلة أوبوس من أنثروبك قدرةً المتاحة عموماً. يدعم مدخلات النص والصورة والملفات مع مخرجات نصية، مع دعم التفكير ونافذة سياق بـ 1 مليون رموز...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "نسخة الوضع السريع من [أوبوس 4.8](/anthropic/claude-opus-4.8) - قدرات متطابقة مع سرعة إخراج أعلى بتسعير 2 أضعاف مقارنة بأوبوس 4.8 العادي. تعلم المزيد في وثائق أنثروبك: https://platform.claude.com/docs/en/build-with-claude/fast-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يعزز كود سونيت 4 قدرات سابقه سونيت 3.7 بشكل كبير، ويتفوق في كل من مهام البرمجة والاستدلال بدقة وتحكم محسنين. يحقق أداءً متطوراً في SWE-bench (72.7%)،...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Claude Sonnet 4.5 is Anthropicâ€™s most advanced Sonnet model to date, optimized for real-world agents and coding workflows. It delivers state-of-the-art performance on coding benchmarks such as SWE-bench Verified, with...",
    descriptionAr:
      "كود سونيت 4.5 هو أكثر نماذج سونيت تقدماً من أنثروبك حتى الآن، محسّن للوكلاء الواقعيين وسير عمل البرمجة. يقدم أداءً متطوراً في معايير البرمجة مثل SWE-bench Verified، مع...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "سونيت 4.6 هو أكثر نماذج فئة سونيت من أنثروبك قدرةً حتى الآن، بأداء حدودي عبر البرمجة والوكلاء والعمل المهني. يمتاز بالتطوير التكراري، والتنقل في قواعد الكود المعقدة، وإدارة المشاريع الشاملة مع...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "سونيت 5 هو أكثر نماذج فئة سونيت من أنثروبك قدرةً، بأداء حدودي عبر البرمجة والوكلاء والعمل المهني. يدعم التفكير التكيفي مع مستويات جهد استدلال قابلة للاختيار (منخفض، متوسط، عالي، أقصى،...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "Claude Opus 5",
    description:
      "Claude Opus 5 is Anthropicâ€™s flagship model for demanding reasoning, coding, and long-horizon agentic work. It is particularly strong at end-to-end software tasks, code review and bug finding, visual analysis...",
    descriptionAr:
      "كود أوبوس 5 هو النموذج الرائد من أنثروبك للاستدلال والبرمجة والعمل الوكيل طويل الأمد المتطلب. قوي بشكل خاص في المهام البرمجية الشاملة، ومراجعة الكود والعثور على الأخطاء، والتحليل البصري...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "نسخة الوضع السريع من [أوبوس 5](/anthropic/claude-opus-5) - قدرات متطابقة مع سرعة إخراج أعلى بتسعير 2 أضعاف مقارنة بأوبوس 5 العادي. تعلم المزيد في وثائق أنثروبك: https://platform.claude.com/docs/en/build-with-claude/fast-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك-V3 هو أحدث نموذج من فريق ديبسيك، يبنى على اتباع التعليمات وقدرات البرمجة من الإصدارات السابقة. تم التدريب المسبق على ما يقرب من 15 تريليون رمز، والتقييمات المبلغ عنها...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك V3، نموذج مزيج من الخبراء (MoE) بمعامل 685 مليار، هو التكرار الأحدث من عائلة نماذج الدردشة الرائدة من فريق ديبسيك. يخلف نموذج [ديبسيك V3](/deepseek/deepseek-chat-v3) ويعمل بشكل جيد جداً...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك-V3.1 هو نموذج استدلال هجين كبير (671 مليار معامل، 37 مليار نشط) يدعم كلاً من وضعي التفكير وغير التفكير عبر قوالب المطالبة. يمد أساس ديبسيك-V3 بسياق طويل من مرحلتين...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك-V3.1 تيرمينوس هو تحديث لـ [ديبسيك V3.1](/deepseek/deepseek-chat-v3.1) يحافظ على القدرات الأصلية للنموذج بينما يعالج المشكلات التي أبلغ عنها المستخدمون، بما في ذلك اتساق اللغة وقدرات الوكيل، مما يحسن أكثر...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك-V3.2 هو نموذج لغوي كبير مصمم لتنسيق الكفاءة الحسابية العالية مع أداء قوي في الاستدلال واستخدام الأدوات الوكيلة. يقدم انتباه ديبسيك المتناثر (DSA)، آلية انتباه متناثرة دقيقة الحبيبات...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك-V3.2-Exp هو نموذج لغوي كبير تجريبي أصدره ديبسيك كخطوة وسيطة بين V3.1 والبنيات المستقبلية. يقدم انتباه ديبسيك المتناثر (DSA)، آلية انتباه متناثرة دقيقة الحبيبات...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك V4 فلاش هو نموذج مزيج الخبراء المحسن للكفاءة من ديبسيك بإجمالي 284 مليار معامل و13 مليار معامل مفعل، يدعم نافذة سياق بـ 1 مليون رمز. مصمم للاستدلال السريع و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك V4 فلاش 0731 هو نموذج مزيج خبراء متناثر من ديبسيك، بـ 13 مليار معامل نشط من إجمالي 284 مليار. هذه النسخة المعاد تدريبها بعد النشر مناسبة للبرمجة والاستدلال وسير عمل الوكيل....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك V4 برو هو نموذج مزيج خبراء واسع النطاق من ديبسيك بـ 1.6 تريليون معامل إجمالي و49 مليار معامل مفعل، يدعم نافذة سياق بـ 1 مليون رمز. مصمم للاستدلال المتقدم، والبرمجة،...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك V4 برو 0813 هو نموذج مزيج خبراء واسع النطاق من ديبسيك. هذا هو الإصدار المتاح عموماً (GA) من ديبسيك V4 برو.",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك R1 هنا: أداء مماثل لـ [أوبن إي آي o1](/openai/o1)، ولكنه مفتوح المصدر برموز استدلال مفتوحة تماماً. حجمه 671 مليار معامل، مع 37 مليار نشط في تمرير الاستدلال....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "تحديث 28 مايو لـ [ديبسيك R1 الأصلي](/deepseek/deepseek-r1). أداء مماثل لـ [أوبن إي آي o1](/openai/o1)، ولكنه مفتوح المصدر برموز استدلال مفتوحة تماماً. حجمه 671 مليار معامل، مع 37 مليار نشط...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "ديبسيك R1 دستيل لاما 70B هو نموذج لغوي كبير دستيل مبني على [لاما-3.3-70B-إنستراكت](/meta-llama/llama-3.3-70b-instruct)، باستخدام مخرجات من [ديبسيك R1](/deepseek/deepseek-r1). يجمع النموذج تقنيات التقطير المتقدمة لتحقيق أداء عالي عبر...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      'جيميني 2.5 فلاش هو نموذج العمل المتطور من جوجل، مصمم خصيصاً للاستدلال المتقدم والبرمجة والرياضيات والمهام العلمية. يتضمن قدرات "تفكير" مدمجة، تمكنه من تقديم استجابات بمزيد من...',
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 2.5 فلاش-لايت هو نموذج استدلال خفيف في عائلة جيميني 2.5، محسّن لكمون فائق الانخفاض وكفاءة التكلفة. يوفر معدل نقل بيانات محسن، وتوليد رموز أسرع، وأداء أفضل...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "Gemini 2.5 Pro",
    description:
      "Gemini 2.5 Pro is Googleâ€™s state-of-the-art AI model designed for advanced reasoning, coding, mathematics, and scientific tasks. It employs â€œthinkingâ€ capabilities, enabling it to reason through responses with enhanced accuracy...",
    descriptionAr:
      'جيميني 2.5 برو هو نموذج الذكاء الاصطناعي المتطور من جوجل، مصمم للاستدلال المتقدم والبرمجة والرياضيات والمهام العلمية. يستخدم قدرات "التفكير"، تمكنه من الاستدلال عبر الاستجابات بدقة معززة...',
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3 فلاش معاينة هو نموذج تفكير عالي السرعة والقيمة مصمم لسير العمل الوكيل، والمحادثة متعددة الدورات، ومساعدة البرمجة. يقدم استدلالاً قريباً من مستوى برو وأدوات...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Gemini 3.1 Flash Lite is Googleâ€™s GA high-efficiency multimodal model optimized for low-latency, high-volume workloads. It supports text, image, video, audio, and PDF inputs, and is designed for lightweight agentic...",
    descriptionAr:
      "جيميني 3.1 فلاش لايت هو نموذج متعدد الوسائط عالي الكفاءة المتاح عموماً من جوجل، محسّن لأحمال العمل عالية الحجم ذات الكمون المنخفض. يدعم مدخلات النص والصورة والفيديو والصوت وملفات PDF، ومصمم للوكيل الخفيف...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.1 فلاش لايت معاينة هو نموذج جوجل عالي الكفاءة محسّن لحالات الاستخدام عالية الحجم. يتفوق على جيميني 2.5 فلاش لايت في الجودة العامة ويقترب من أداء جيميني 2.5 فلاش عبر...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Gemini 3.1 Pro Preview is Googleâ€™s frontier reasoning model, delivering enhanced software engineering performance, improved agentic reliability, and more efficient token usage across complex workflows. Building on the multimodal foundation...",
    descriptionAr:
      "جيميني 3.1 برو معاينة هو نموذج الاستدلال الحدودي من جوجل، يوفر أداءً هندسياً برمجياً محسناً، وموثوقية وكيلة محسنة، واستخدام رموز أكثر كفاءة عبر سير العمل المعقد. بناءً على الأساس متعدد الوسائط...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.1 برو معاينة أدوات مخصصة هو متغير من جيميني 3.1 برو يحسن سلوك اختيار الأدوات من خلال منع الإفراط في استخدام أداة باش العامة عندما تكون أدوات الطرف الثالث الأكثر كفاءة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.5 فلاش هو نموذج متعدد الوسائط عالي الكفاءة من جوجل، يجلب برمجة واستدلالاً قريباً من مستوى برو بتكلفة وسرعة فئة فلاش. محسن بشكل كبير لكفاءة البرمجة وتنفيذ الوكيل المتوازي...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.5 فلاش لايت هو نموذج عالي الكفاءة من جوجل بقدرات وكيلية محسنة. مناسب للوكلاء الفرعيين الذين ينفذون مهام مركزة داخل سير العمل المعقد متعدد الوكلاء.",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.6 فلاش هو نموذج عالي الكفاءة من جوجل للبرمجة، وسير العمل الوكيل، وتطوير الويب والتطبيقات. مصمم لإنتاج مخرجات مصقولة مع تعديلات غير ضرورية أقل و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيميني 3.7 فلاش هو نموذج متعدد الوسائط من جوجل لسير العمل الوكيل السريع، والبرمجة، والاستدلال المعقد متعدد الخطوات. مصمم للمهام التي تتطلب أداءً متجاوباً ومتعدد الخطوات الموثوق...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيما 2 27B من جوجل هو نموذج مفتوح مبني من نفس البحث والتكنولوجيا المستخدمة لإنشاء [نماذج جيميني](/models?q=gemini). نماذج جيما مناسبة لمجموعة متنوعة من...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "تقدم جيما 3 تعدد الوسائط، وتدعم إدخال الرؤية واللغة والمخرجات النصية. تتعامل مع نوافذ السياق حتى 128 ألف رمز، وتفهم أكثر من 140 لغة، وتوفر قدرات رياضيات واستدلال ودردشة محسنة،...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "Gemma 3n 4B",
    description:
      "Gemma 3n E4B-it is optimized for efficient execution on mobile and low-resource devices, such as phones, laptops, and tablets. It supports multimodal inputsâ€”including text, visual data, and audioâ€”enabling diverse tasks...",
    descriptionAr:
      "جيما 3n E4B-it محسّن للتنفيذ الفعال على الأجهزة المحمولة والأجهزة منخفضة الموارد، مثل الهواتف وأجهزة الكمبيوتر المحمولة والأجهزة اللوحية. يدعم مدخلات متعددة الوسائط - بما في ذلك النص والبيانات البصرية والصوت - مما يتيح مهام متنوعة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "Gemma 4 26B A4B IT is an instruction-tuned Mixture-of-Experts (MoE) model from Google DeepMind. Despite 25.2B total parameters, only 3.8B activate per token during inference â€” delivering near-31B quality at...",
    descriptionAr:
      "جيما 4 26B A4B IT هو نموذج مزيج الخبراء (MoE) المعد للتعليمات من جوجل ديب مايند. على الرغم من إجمالي 25.2 مليار معامل، يتم تنشيط 3.8 مليار فقط لكل رمز أثناء الاستدلال - مما يوفر جودة قريبة من 31 مليار بـ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "جيما 4 31B إنستراكت هو النموذج المتعدد الوسائط الكثيف بـ 30.7 مليار من جوجل ديب مايند، يدعم إدخال النص والصورة مع مخرجات نصية. يتميز بنافذة سياق بـ 256 ألف رمز، ووضع تفكير/استدلال قابل للتكوين، ودعم الدوال الأصلي...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يشير GPT Chat Latest إلى الاسم المستعار المستقر لواجهة برمجة التطبيقات من أوبن إي آي `chat-latest` الذي يحل دائماً لأحدث نموذج دردشة فوري المستخدم في ChatGPT. مع طرح أوبن إي آي تحديثات نموذج فورية جديدة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-3.5 تيربو هو أسرع نماذج أوبن إي آي. يمكنه فهم وتوليد اللغة الطبيعية أو الكود، ومحسّن للدردشة ومهام الإكمال التقليدية. بيانات التدريب حتى سبتمبر 2021.",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يوفر هذا النموذج أربعة أضعاف طول السياق من gpt-3.5-turbo، مما يسمح له بدعم حوالي 20 صفحة من النص في طلب واحد بتكلفة أعلى. بيانات التدريب: حتى...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "هذا النموذج هو متغير من GPT-3.5 تيربو مضبوط لمطالبات التعليمات وحذف التحسينات المتعلقة بالدردشة. بيانات التدريب: حتى سبتمبر 2021.",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "النموذج الرائد من أوبن إي آي، GPT-4 هو نموذج لغوي متعدد الوسائط واسع النطاق قادر على حل المشكلات الصعبة بدقة أكبر من النماذج السابقة بسبب معرفته العامة الأوسع وقدرات الاستدلال المتقدمة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أحدث نموذج GPT-4 تيربو مع قدرات الرؤية. يمكن لطلبات الرؤية الآن استخدام وضع JSON واستدعاء الدوال. بيانات التدريب: حتى ديسمبر 2023.",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "النموذج المعاين GPT-4 مع تحسين اتباع التعليمات، ووضع JSON، والمخرجات القابلة للتكرار، واستدعاء الدوال المتوازي، وأكثر من ذلك. بيانات التدريب: حتى ديسمبر 2023. **ملاحظة:** محدود بشدة بواسطة أوبن إي آي بينما...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-4.1 هو نموذج لغوي كبير رائد محسّن لاتباع التعليمات المتقدمة، والهندسة البرمجية الواقعية، والاستدلال طويل السياق. يدعم نافذة سياق بـ 1 مليون رمز ويتفوق على GPT-4o و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-4.1 ميني هو نموذج متوسط الحجم يوفر أداءً تنافسياً مع GPT-4o بكمون وتكلفة أقل بكثير. يحتفظ بنافذة سياق بـ 1 مليون رمز ويسجل 45.1% في المهام الصعبة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-4.1 Nano",
    description:
      "For tasks that demand low latency, GPTâ€‘4.1 nano is the fastest and cheapest model in the GPT-4.1 series. It delivers exceptional performance at a small size with its 1 million...",
    descriptionAr:
      "بالنسبة للمهام التي تتطلب كمون منخفض، يعد GPT-4.1 نانو أسرع وأرخص نموذج في سلسلة GPT-4.1. يقدم أداءً استثنائياً بحجم صغير مع نافذة سياقه بـ 1 مليون...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      'GPT-4o (الحرف "o" لـ "omni" أي شامل) هو أحدث نموذج ذكاء اصطناعي من أوبن إي آي، يدعم مدخلات النص والصورة مع مخرجات نصية. يحافظ على مستوى ذكاء [GPT-4 تيربو](/models/openai/gpt-4-turbo) بينما يكون أسرع بمرتين...',
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      'تقدم نسخة 2024-08-06 من GPT-4o أداءً محسناً في المخرجات المنظمة، مع القدرة على توفير مخطط JSON في تنسيق الاستجابة. اقرأ المزيد [هنا](https://openai.com/index/introducing-structured-outputs-in-the-api/). GPT-4o (الحرف "o" لـ "omni") هو...',
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "The 2024-11-20 version of GPT-4o offers a leveled-up creative writing ability with more natural, engaging, and tailored writing to improve relevance & readability. Itâ€™s also better at working with uploaded...",
    descriptionAr:
      "تقدم نسخة 2024-11-20 من GPT-4o قدرة كتابة إبداعية محسنة مع كتابة أكثر طبيعية وجاذبية وتصميماً لتحسين الصلة وقابلية القراءة. كما أنها أفضل في العمل مع الملفات المرفوعة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-4o ميني هو أحدث نموذج من أوبن إي آي بعد [GPT-4 أومني](/models/openai/gpt-4o)، يدعم مدخلات النص والصورة مع مخرجات نصية. كنموذج صغير متقدم لديهم، فهو بأسعار معقولة بمضاعفات كثيرة أكثر...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5",
    description:
      "GPT-5 is OpenAIâ€™s most advanced model, offering major improvements in reasoning, code quality, and user experience. It is optimized for complex tasks that require step-by-step reasoning, instruction following, and accuracy...",
    descriptionAr:
      "GPT-5 هو أكثر نماذج أوبن إي آي تقدماً، ويقدم تحسينات كبيرة في الاستدلال وجودة الكود وتجربة المستخدم. محسّن للمهام المعقدة التي تتطلب استدلالاً خطوة بخطوة، واتباع التعليمات، والدقة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5 ميني هو نسخة مضغوطة من GPT-5، مصممة للتعامل مع مهام الاستدلال الأخف وزناً. يوفر نفس فوائد اتباع التعليمات والضبط الآمني مثل GPT-5، ولكن مع كمون وتكلفة مخفضين....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5-نانو هو أصغر وأسرع متغير في نظام GPT-5، محسّن لأدوات المطورين والتفاعلات السريعة وبيئات الكمون فائق الانخفاض. بينما محدود في عمق الاستدلال مقارنة بنظيره الأكبر...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5 Pro",
    description:
      "GPT-5 Pro is OpenAIâ€™s most advanced model, offering major improvements in reasoning, code quality, and user experience. It is optimized for complex tasks that require step-by-step reasoning, instruction following, and...",
    descriptionAr:
      "GPT-5 برو هو أكثر نماذج أوبن إي آي تقدماً، ويقدم تحسينات كبيرة في الاستدلال وجودة الكود وتجربة المستخدم. محسّن للمهام المعقدة التي تتطلب استدلالاً خطوة بخطوة، واتباع التعليمات، و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.1 هو أحدث نموذج من الدرجة الحدودية في سلسلة GPT-5، ويقدم استدلالاً عاماً أقوى، وتقيداً محسناً بالتعليمات، وأسلوب محادثة أكثر طبيعية مقارنة بـ GPT-5. يستخدم استدلالاً تكيفياً...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.1-كوديكس هو نسخة متخصصة من GPT-5.1 محسّنة للهندسة البرمجية وسير عمل البرمجة. مصمم لكل من جلسات التطوير التفاعلية والتنفيذ الطويل المستقل لمهام الهندسة المعقدة....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    description:
      "GPT-5.1-Codex-Max is OpenAIâ€™s latest agentic coding model, designed for long-running, high-context software development tasks. It is based on an updated version of the 5.1 reasoning stack and trained on agentic...",
    descriptionAr:
      "GPT-5.1-كوديكس-ماكس هو أحدث نموذج برمجة وكيل من أوبن إي آي، مصمم لمهام تطوير البرمجيات طويلة التشغيل عالية السياق. مبني على نسخة محدثة من مكدس الاستدلال 5.1 ومدرب على سير العمل الوكيل...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.2 هو أحدث نموذج من الدرجة الحدودية في سلسلة GPT-5، ويقدم أداءً وكيلاً وسياقاً طويلاً أقوى مقارنة بـ GPT-5.1. يستخدم استدلالاً تكيفياً لتخصيص الحساب ديناميكياً، مستجيباً بسرعة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.2 Chat",
    description:
      "GPT-5.2 Chat (AKA Instant) is the fast, lightweight member of the 5.2 family, optimized for low-latency chat while retaining strong general intelligence. It uses adaptive reasoning to selectively â€œthinkâ€ on...",
    descriptionAr:
      'GPT-5.2 دردشة (المعروف أيضاً باسم إنستانت) هو العضو السريع الخفيف في عائلة 5.2، محسّن للدردشة ذات الكمون المنخفض مع الحفاظ على الذكاء العام القوي. يستخدم استدلالاً تكيفياً لـ "التفكير" بشكل انتقائي على...',
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.2 Pro",
    description:
      "GPT-5.2 Pro is OpenAIâ€™s most advanced model, offering major improvements in agentic coding and long context performance over GPT-5 Pro. It is optimized for complex tasks that require step-by-step reasoning,...",
    descriptionAr:
      "GPT-5.2 برو هو أكثر نماذج أوبن إي آي تقدماً، ويقدم تحسينات كبيرة في البرمجة الوكيلة وأداء السياق الطويل مقارنة بـ GPT-5 برو. محسّن للمهام المعقدة التي تتطلب استدلالاً خطوة بخطوة، ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.2-كوديكس هو نسخة محدثة من GPT-5.1-كوديكس محسّنة للهندسة البرمجية وسير عمل البرمجة. مصمم لكل من جلسات التطوير التفاعلية والتنفيذ الطويل المستقل لمهام الهندسة المعقدة....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.3-Codex",
    description:
      "GPT-5.3-Codex is OpenAIâ€™s most advanced agentic coding model, combining the frontier software engineering performance of GPT-5.2-Codex with the broader reasoning and professional knowledge capabilities of GPT-5.2. It achieves state-of-the-art results...",
    descriptionAr:
      "GPT-5.3-كوديكس هو أكثر نماذج البرمجة الوكيل تقدماً من أوبن إي آي، يجمع أداء الهندسة البرمجية الحدودي لـ GPT-5.2-كوديكس مع قدرات الاستدلال الأوسع والمعرفة المهنية لـ GPT-5.2. يحقق نتائج متطورة في...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.4",
    description:
      "GPT-5.4 is OpenAIâ€™s latest frontier model, unifying the Codex and GPT lines into a single system. It features a 1M+ token context window (922K input, 128K output) with support for...",
    descriptionAr:
      "GPT-5.4 هو أحدث نموذج حدودي من أوبن إي آي، يوحد خطوط كوديكس وGPT في نظام واحد. يتميز بنافذة سياق بـ 1 مليون+ رمز (922 ألف إدخال، 128 ألف إخراج) مع دعم لـ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يُجلب GPT-5.4 ميني القدرات الأساسية لـ GPT-5.4 إلى نموذج أسرع وأكثر كفاءة محسّن لأحمال العمل عالية الإنتاجية. يدعم مدخلات النص والصورة مع أداء قوي عبر الاستدلال، والبرمجة،...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.4 نانو هو أكثر متغيرات عائلة GPT-5.4 خفةً وكفاءةً من حيث التكلفة، محسّن للمهام الحرجة من حيث السرعة وعالية الحجم. يدعم مدخلات النص والصورة ومصمم لبيئات الكمون المنخفض...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.4 برو هو أكثر نماذج أوبن إي آي تقدماً، يبنى على بنية GPT-5.4 الموحدة بقدرات استدلال محسنة للمهام المعقدة عالية المخاطر. يتميز بنافذة سياق بـ 1 مليون+ رمز (922 ألف إدخال، 128 ألف...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.5",
    description:
      "GPT-5.5 is OpenAIâ€™s frontier model designed for complex professional workloads, building on GPT-5.4 with stronger reasoning, higher reliability, and improved token efficiency on hard tasks. It features a 1M+ token...",
    descriptionAr:
      "GPT-5.5 هو النموذج الحدودي من أوبن إي آي مصمم لأحمال العمل المهنية المعقدة، يبنى على GPT-5.4 باستدلال أقوى، وموثوقية أعلى، وكفاءة رموز محسنة على المهام الصعبة. يتميز بـ 1 مليون+ رمز...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    name: "GPT-5.5 Pro",
    description:
      "GPT-5.5 Pro is OpenAIâ€™s high-capability model optimized for deep reasoning and accuracy on complex, high-stakes workloads. It features a 1M+ token context window (922K input, 128K output) with support for...",
    descriptionAr:
      "GPT-5.5 برو هو النموذج عالي القدرة من أوبن إي آي، محسّن للاستدلال العميق والدقة على أحمال العمل المعقدة عالية المخاطر. يتميز بنافذة سياق بـ 1 مليون+ رمز (922 ألف إدخال، 128 ألف إخراج) مع دعم لـ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 لونا هو نموذج سريع فعال من حيث التكلفة في سلسلة GPT-5.6 من أوبن إي آي. مناسب للمهام عالية الحجم والحساسة للكمون مثل الدردشة والتصنيف وسير العمل الوكيل الخفيف، ويوفر استدلالاً قادراً لـ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 لونا برو هو نفس النموذج الأساسي مثل [GPT-5.6 لونا](https://openrouter.ai/openai/gpt-5.6-luna)، يُخدم مع ضبط `reasoning.mode` على `pro` للحصول على استجابات عالية الجودة في المهام المعقدة. تعلم المزيد في وثائق أوبن إي آي: https://developers.openai.com/api/docs/guides/reasoning#reasoning-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 سول هو النموذج الرائد في سلسلة GPT-5.6 من أوبن إي آي. مناسب للاستدلال المعقد، والبرمجة، وسير العمل الوكيل، وقوي بشكل خاص في سطر الأوامر ومهام البرمجة متعددة الخطوات...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 سول برو هو نفس النموذج الأساسي مثل [GPT-5.6 سول](https://openrouter.ai/openai/gpt-5.6-sol)، يُخدم مع ضبط `reasoning.mode` على `pro` للحصول على استجابات عالية الجودة في المهام المعقدة. تعلم المزيد في وثائق أوبن إي آي: https://developers.openai.com/api/docs/guides/reasoning#reasoning-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 تيرا هو نموذج متوازن في سلسلة GPT-5.6 من أوبن إي آي، يضع نفسه بين مستوى سول الرائد ومستوى لونا الفعال من حيث التكلفة. مناسب للبرمجة اليومية، والاستدلال، والوكيل...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "GPT-5.6 تيرا برو هو نفس النموذج الأساسي مثل [GPT-5.6 تيرا](https://openrouter.ai/openai/gpt-5.6-terra)، يُخدم مع ضبط `reasoning.mode` على `pro` للحصول على استجابات عالية الجودة في المهام المعقدة. تعلم المزيد في وثائق أوبن إي آي: https://developers.openai.com/api/docs/guides/reasoning#reasoning-mode",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "gpt-oss-120b هو نموذج لغوي مزيج الخبراء (MoE) مفتوح الوزن بـ 117 مليار معامل من أوبن إي آي، مصمم لحالات الاستخدام الإنتاجية عالية الاستدلال والوكيلية والعامة. ينشط 5.1 مليار معامل لكل تمرير أمامي ومحسّن...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "gpt-oss-20b هو نموذج مفتوح الوزن بـ 21 مليار معامل أصدره أوبن إي آي بموجب ترخيص أباتشي 2.0. يستخدم بنية مزيج الخبراء (MoE) بـ 3.6 مليار معامل نشط لكل تمرير أمامي، محسّن لـ...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "gpt-oss-safeguard-20b هو نموذج استدلال أمان من أوبن إي آي مبني على gpt-oss-20b. هذا النموذج المفتوح الوزن، المكون من 21 مليار معامل ومزيج الخبراء (MoE) يوفر كموناً أقل لمهام الأمان مثل تصنيف المحتوى، وتصفية LLM، والثقة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أحدث وأقوى عائلة نماذج من أوبن إي آي، o1 مصمم لقضاء وقت أطول في التفكير قبل الرد. يتم تدريب سلسلة نماذج o1 باستخدام التعلم المعزز واسع النطاق للاستدلال...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يتم تدريب سلسلة نماذج o1 باستخدام التعلم المعزز للتفكير قبل الإجابة وأداء الاستدلال المعقد. يستخدم نموذج o1-pro المزيد من القوة الحسابية للتفكير بشكل أعمق وتقديم...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "o3 هو نموذج شامل وقوي عبر المجالات. يضع معياراً جديداً للرياضيات والعلوم والبرمجة ومهام الاستدلال البصري. كما يمتاز في الكتابة التقنية واتباع التعليمات....",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أوبن إي آي o3-mini هو نموذج لغوي فعال من حيث التكلفة محسّن لمهام الاستدلال في العلوم والتكنولوجيا والهندسة والرياضيات (STEM)، ويتفوق بشكل خاص في العلوم والرياضيات والبرمجة. يدعم هذا النموذج معلمة `reasoning_effort`، التي يمكن ضبطها على...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أوبن إي آي o3-mini-high هو نفس النموذج مثل [o3-mini](/openai/o3-mini) مع ضبط reasoning_effort على high. o3-mini هو نموذج لغوي فعال من حيث التكلفة محسّن لمهام الاستدلال في العلوم والتكنولوجيا والهندسة والرياضيات (STEM)، ويتفوق بشكل خاص في العلوم والرياضيات و...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "يتم تدريب سلسلة النماذج o باستخدام التعلم المعزز للتفكير قبل الإجابة وأداء الاستدلال المعقد. يستخدم نموذج o3-pro المزيد من القوة الحسابية للتفكير بشكل أعمق وتقديم استجابات متسقة...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أوبن إي آي o4-mini هو نموذج استدلال مضغوط في السلسلة o، محسّن لأداء سريع فعال من حيث التكلفة مع الحفاظ على قدرات متعددة الوسائط ووكيلية قوية. يدعم استخدام الأدوات ويُظهر استدلالاً تنافسياً...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  },
  {
    ...
    descriptionAr:
      "أوبن إي آي o4-mini-high هو نفس النموذج مثل [o4-mini](/openai/o4-mini) مع ضبط reasoning_effort على high. أوبن إي آي o4-mini هو نموذج استدلال مضغوط في السلسلة o، محسّن لأداء سريع فعال من حيث التكلفة مع الحفاظ على...",
    modelType: AiModelType.TEXT,
    tier: null,
  ...
  for (const provider of AI_PROVIDER_SEEDS) {
    ...
    if (existing) {
      await repository.update(existing.id, {
        ...
        tenantIntegrationAllowed: provider.tenantIntegrationAllowed,
