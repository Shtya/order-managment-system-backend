...
export class AccountingController {
  ...
  async getStats(@Req() req: any, @Query() query: AccountingStatsDto) {
  ...
  async exportShipmentsCityReport(
    ...
    const buffer = await this.accountingService.exportShipmentsCityReport(
      req.user,
      query,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=shipments-city-report-${Date.now()}.xlsx`,
    );
  ...
  async getFinancialStats(@Req() req: any) {
    return await this.accountingService.getSupplierPeriodPreview(
      req.user,
      null,
      null,
      null,
