...
export class AccountingService {
  ...
  async getShipmentsCityReport(
    ...
    if (filters.search) {
      qb.andWhere("(cityDetails.nameAr ILIKE :search)", {
        search: `%${filters.search}%`,
      });
    }

    qb.groupBy("COALESCE(cityDetails.nameAr, order.city)").orderBy(
      '"totalShipments"',
      "DESC",
    );
  ...
  async exportShipmentsCityReport(me: any, q?: any) {
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.accounting.shipments_city_report"),
    );

    // 5. Define English Columns
    worksheet.columns = [
      {
        header: this.translations.t("domains.accounting.city"),
        key: "city",
        width: 25,
      },
      {
        header: this.translations.t("domains.accounting.total_shipments"),
        key: "totalShipments",
        width: 15,
      },
      {
        header: this.translations.t("domains.accounting.actual_deliveries"),
        key: "actualDeliveries",
        width: 15,
      },
      {
        header: this.translations.t("domains.accounting.failed_cancelled"),
        key: "failedShipments",
        width: 15,
      },
      {
        header: this.translations.t("domains.accounting.success_rate"),
        key: "successRate",
        width: 15,
      },
      {
        header: this.translations.t("domains.accounting.failure_rate"),
        key: "failureRate",
        width: 15,
      },
    ];
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!supplier) {
        throw new NotFoundException(
          this.translations.t("domains.accounting.supplier_not_found"),
        );
      }
      ...
      if (supplier.lastClosingEndDate) {
        ...
        if (newStartDate <= lastEnd) {
          ...
          throw new BadRequestException(
            this.translations.t(
              "domains.accounting.closing_period_start_after_last",
              { args: { formattedDate } },
            ),
          );
        }
      ...
      if (newEndDate <= newStartDate) {
        throw new BadRequestException(
          this.translations.t("domains.accounting.end_date_must_be_later"),
        );
      }
      ...
      const {
        ...
        totalPaid,
        totalPurchases,
      } = await this.getSupplierPeriodPreview(
        me,
        supplierId,
        startDate,
        endDate,
      ...
      if (pCount === 0 && rCount === 0) {
        throw new BadRequestException(
          this.translations.t("domains.accounting.no_unclosed_invoices"),
        );
      }
  ...
  async getSupplierClosing(me: any, id: string) {
    ...
    if (!closing) {
      throw new NotFoundException(
        this.translations.t("domains.accounting.closing_not_found"),
      );
    }
    return closing;
  }
  ...
  async listSupplierClosings(me: any, q?: any) {
    ...
    if (q?.supplierId) {
      qb.andWhere("closing.supplierId = :supplierId", {
        supplierId: q.supplierId,
      });
    }
    if (q?.year) {
      qb.andWhere("EXTRACT(YEAR FROM closing.endDate) = :year", {
        year: q.year,
      });
    }
  ...
  async getAccountingStats(me: any, q?: any) {
    ...
    const [totalSuppliers, totalReturns, purchaseTotal] = await Promise.all([
      ...
      this.returnRepo
        ...
        .where("exp.adminId = :adminId", { adminId })
        .andWhere("exp.statusUpdateDate BETWEEN :start AND :end", {
          start: startDate,
          end: endDate,
        })
        .getRawOne(),
      ...
      this.purchaseRepo
        ...
        .andWhere("p.statusUpdateDate BETWEEN :start AND :end", {
          start: startDate,
          end: endDate,
        })
        .getRawOne(),
  ...
  async getSupplierPeriodPreview(
    me: any,
    ...
    const purchaseRepo = manager
      ...
      : this.purchaseRepo;
    const returnRepo = manager
      ? manager.getRepository(PurchaseReturnInvoiceEntity)
      : this.returnRepo;
    const paymentRepo = manager
      ? manager.getRepository(SupplierPaymentEntity)
      : this.dataSource.getRepository(SupplierPaymentEntity);
    ...
    purchaseQb.andWhere("p.status = :status", {
      status: ApprovalStatus.ACCEPTED,
    });
    ...
    returnQb.andWhere("r.status = :status", {
      status: ApprovalStatus.ACCEPTED,
    });
    // .andWhere("r.closingId IS NULL");

    const paymentQb = paymentRepo
    ...
    const supplierRepo = manager
      ? manager.getRepository(SupplierEntity)
      : this.supplierRepo;
    const dueBalanceQb = supplierRepo
