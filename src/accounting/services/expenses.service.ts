...
export class ExpensesService {
  ...
  async getExpense(me: any, id: string) {
    ...
    if (!expense) {
      throw new NotFoundException(
        this.translations.t("domains.expenses.expense_not_found"),
      );
    }
    return expense;
  }
  async exportExpenses(me: any, q?: any) {
    ...
    const exportData = records.map((expense) => ({
      ...
      collectionDate: expense.collectionDate
        ? new Date(expense.collectionDate).toLocaleDateString()
        : "N/A",
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.expenses.excel_worksheet_title"),
    );

    worksheet.columns = [
      { header: this.translations.t("common.id"), key: "id", width: 10 },
      {
        header: this.translations.t("domains.expenses.excel_header_category"),
        key: "category",
        width: 20,
      },
      {
      ...
      {
        header: this.translations.t("domains.expenses.excel_header_safe"),
        key: "safe",
        width: 20,
      },
      {
        header: this.translations.t(
          "domains.expenses.excel_header_collection_date",
        ),
        key: "collectionDate",
        width: 20,
      },
      ...
      {
        header: this.translations.t("domains.expenses.excel_header_created_at"),
        key: "createdAt",
        width: 20,
      },
    ];
  ...
  async createExpense(me: any, dto: CreateManualExpenseDto) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!category) {
        throw new NotFoundException(
          this.translations.t("domains.expenses.category_not_found_with_id", {
            args: { categoryId: dto.categoryId },
          }),
        );
      }
      ...
      if (dto.safeId) {
        ...
        if (!safe) {
          throw new BadRequestException(
            this.translations.t("domains.expenses.safe_account_not_found"),
          );
        }
        if (safe.status !== AccountStatus.ACTIVE) {
          throw new BadRequestException(
            this.translations.t("domains.expenses.safe_account_not_active"),
          );
        }
      }
      ...
      if (Number(savedExpense.amount) > 0 && savedExpense.safeId) {
        ...
        await this.safesService.withdraw(
          me,
          {
            ...
            notes: await this.requestTranslationsService.tAsync(
              "domains.expenses.log_expense_accepted",
              adminId,
              { args: { amount: savedExpense.amount, categoryName } },
            ),
          },
          manager,
        );
  ...
  async updateExpense(me: any, id: string, dto: UpdateManualExpenseDto) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!expense) {
        throw new NotFoundException(
          this.translations.t("domains.expenses.expense_record_not_found"),
        );
      }

      if (expense.monthlyClosingId) {
        throw new BadRequestException(
          this.translations.t("domains.expenses.cannot_update_closed"),
        );
      }
      ...
      if (oldAmount > 0 && oldSafeId) {
        ...
        await this.safesService.deposit(
          me,
          {
            ...
            notes: await this.requestTranslationsService.tAsync(
              "domains.expenses.log_reversing_old_expense",
              adminId,
              { args: { oldAmount } },
            ),
          },
      ...
      if (
        dto.attachment &&
        expense.attachment &&
        dto.attachment !== expense.attachment
      ) {
      ...
      if (Number(savedExpense.amount) > 0 && savedExpense.safeId) {
        ...
        await this.safesService.withdraw(
          me,
          {
            ...
            notes: await this.requestTranslationsService.tAsync(
              "domains.expenses.log_expense_updated",
              adminId,
              { args: { amount: savedExpense.amount, categoryName } },
            ),
          },
  ...
  async deleteExpense(me: any, id: string) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (expense.monthlyClosingId) {
        throw new BadRequestException(
          this.translations.t("domains.expenses.cannot_delete_closed"),
        );
      }
      ...
      if (amount > 0 && safeId) {
        ...
        await this.safesService.deposit(
          me,
          {
            ...
            notes: await this.requestTranslationsService.tAsync(
              "domains.expenses.log_expense_deleted_refunding",
              adminId,
              { args: { amount } },
            ),
          },
      ...
      return {
        ...
        message: this.translations.t(
          "domains.expenses.expense_deleted_successfully",
        ),
      };
    });
  }
