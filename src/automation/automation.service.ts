...
export class AutomationService {
  ...
  async create(me: any, dto: CreateAutomationDto) {
    ...
    if (!isSuperAdminFlag && !adminId) {
      throw new BadRequestException(
        this.translations.t("common.admin_id_not_found"),
      );
    }
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (existing) {
        throw new BadRequestException(
          this.translations.t("domains.automation.name_already_exists"),
        );
      }
  ...
  async update(me: any, id: string, dto: UpdateAutomationDto) {
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (!automation) {
        throw new BadRequestException(
          this.translations.t("domains.automation.not_found"),
        );
      }

      if (dto.flow) {
        const triggerNode = dto.flow.nodes.find((n) => n.type === "trigger");

        if (triggerNode && triggerNode.data?.type !== automation.triggerType) {
          throw new BadRequestException(
      ...
      if (dto.flow) {
        const parentVersion = automation.latestVersion;
        // let isPatch = false;
        ...
        if (parentVersion && this.isFlowEqual(dto.flow, parentVersion.flow)) {
          return {
            ...
            message: this.translations.t(
              "domains.automation.flow_identical_skipped",
            ),
          };
        }
  ...
  async retryRun(me: any, runId: string, useLatestVersion?: boolean) {
    ...
    if (!run) {
      throw new NotFoundException(
        this.translations.t("domains.automation.run_not_found"),
      );
    }

    if (run.status !== RunStatus.FAILED) {
      throw new BadRequestException(
        this.translations.t("domains.automation.only_failed_runs_retriable"),
      );
    }
  ...
  async findAll(me: any, q?: any) {
    ...
    if (isSuperAdminFlag) qb.where("automation.adminId Is NULL");
    else qb.where("automation.adminId = :adminId", { adminId });
    ...
    if (q?.triggerType) {
      qb.andWhere("automation.triggerType = :triggerType", {
  ...
  async findOne(me: any, id: string, version?: string) {
    ...
    if (version) {
      if (!automation.versions || automation.versions.length === 0) {
        throw new BadRequestException(
          this.translations.t("domains.automation.version_not_found", {
            args: { version },
          }),
        );
      }
      ...
      if (!automation.latestVersion) {
        throw new BadRequestException(
          this.translations.t("domains.automation.no_active_version"),
        );
      }
  ...
  private async generateNextVersion(automationFlowId: string): Promise<string> {
    const latestVersion = await this.versionRepo
      .createQueryBuilder("version")
      .where("version.automationFlowId = :automationFlowId", {
        automationFlowId,
      })
      .orderBy(
        "CAST(SPLIT_PART(version.versionString, '.', 1) AS INTEGER)",
        "DESC",
      )
      .getOne();

    const currentMajor = latestVersion
      ? parseInt(latestVersion.versionString.split(".")[0], 10)
      : 0;
    return `${currentMajor + 1}.0`;
  }

  async generateNextPatchVersion(
    ...
    const latestPatch = await this.versionRepo
      .createQueryBuilder("version")
      .where("version.automationFlowId = :automationFlowId", {
        automationFlowId,
      })
      .andWhere("version.versionString LIKE :pattern", {
        pattern: `${majorVersion}.%`,
      })
      .orderBy(
        "CAST(SPLIT_PART(version.versionString, '.', 2) AS INTEGER)",
        "DESC",
      ) // ترتيب بناءً على رقم الـ Minor كـ Integer
      .getOne();

    if (!latestPatch) {
      throw new NotFoundException(
        this.translations.t("domains.automation.base_version_not_found"),
      );
    }
  ...
  async delete(me: any, id: string) {
    ...
    return await this.automationRepo.manager.transaction(
      async (transactionalManager) => {
        const automation = await transactionalManager.findOne(
          AutomationFlowEntity,
          {
            where: { id, adminId },
          },
        ...
        return {
          message: this.translations.t(
            "domains.automation.deleted_successfully",
          ),
        };
      },
    );
  ...
  async findAllRuns(me: any, q?: any) {
    ...
    if (q?.automationFlowId) {
      qb.andWhere('run."automationFlowId" = :automationFlowId', {
        automationFlowId: q.automationFlowId,
      });
    }

    if (q?.triggerType) {
      qb.andWhere('automation_flow."triggerType" = :triggerType', {
        triggerType: q.triggerType,
      });
    }

    if (q?.triggerEntityType) {
      qb.andWhere('run."triggerEntityType" = :triggerEntityType', {
        triggerEntityType: q.triggerEntityType,
      });
    }

    if (q?.entityId) {
      qb.andWhere('run."triggerEntityId" = :entityId', {
        entityId: q.entityId,
      });
    }
  ...
  async findOneRun(me: any, id: string) {
    ...
    if (!flow) {
      throw new BadRequestException(
        this.translations.t("domains.automation.access_denied_or_not_found"),
      );
    }
  ...
  async export(me: any, q: any) {
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.automation.export_sheet"),
    );

    worksheet.columns = [
      {
        header: this.translations.t("domains.automation.export_name"),
        key: "name",
        width: 25,
      },
      {
        header: this.translations.t("domains.automation.export_trigger_type"),
        key: "triggerType",
        width: 25,
      },
      {
        header: this.translations.t("domains.automation.export_status"),
        key: "status",
        width: 15,
      },
      {
        header: this.translations.t("domains.automation.export_created_at"),
        key: "createdAt",
        width: 25,
      },
    ];
  ...
  async exportRuns(me: any, q: any) {
    const { records } = await this.findAllRuns(me, {
      ...q,
      limit: 1000,
      page: 1,
    });
    ...
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.automation.export_runs_sheet"),
    );

    worksheet.columns = [
      {
        header: this.translations.t("domains.automation.export_automation"),
        key: "automationName",
        width: 25,
      },
      {
      ...
      {
        header: this.translations.t("domains.automation.export_version"),
        key: "version",
        width: 10,
      },
      {
      ...
      },
      {
        header: this.translations.t(
          "domains.automation.export_steps_completed",
        ),
        key: "steps",
        width: 15,
      },
      {
        header: this.translations.t("domains.automation.export_started_at"),
        key: "startedAt",
        width: 25,
      },
      {
        header: this.translations.t("domains.automation.export_completed_at"),
        key: "completedAt",
        width: 25,
      },
      {
        header: this.translations.t("domains.automation.export_error"),
        key: "error",
        width: 40,
      },
    ];
