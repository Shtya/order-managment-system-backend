...
import {
  ...
  CreatePreviewInput,
  PreviewResumeInput,
} from "./engine/automation-preview.service";
...
export class AutomationController {
  ...
  @Post("preview")
  @Permissions("automation.read")
  async createPreview(@Req() req: any, @Body() dto: CreatePreviewInput) {
    const adminId = tenantId(req.user);
    return this.automationPreviewService.createPreview(req.user, {
      ...dto,
      adminId,
    });
  }
  ...
  @Permissions("automation.read")
  async heartbeatPreview(@Param("previewId") previewId: string) {
    return this.automationPreviewService.touchPreview(previewId);
  }

  @Delete("preview/:previewId")
  @Permissions("automation.read")
  async deletePreview(@Param("previewId") previewId: string) {
  ...
  async export(@Req() req: any, @Query() q: any, @Res() res: Response) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=automations_export_${Date.now()}.xlsx`,
    );
  ...
  async exportRuns(@Req() req: any, @Query() q: any, @Res() res: Response) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=automation_runs_export_${Date.now()}.xlsx`,
    );
    return res.send(buffer);
  }
  ...
  retryRun(
    @Req() req: any,
    @Param("id") id: string,
    @Body() body?: { useLatestVersion?: boolean },
  ) {
    return this.automationService.retryRun(
      req.user,
      id,
      body?.useLatestVersion,
    );
  }
