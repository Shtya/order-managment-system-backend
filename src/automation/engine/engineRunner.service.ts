...
export class EngineRunnerService {
  ...
  async startExecution(runId: string): Promise<{
    ...
    if (!allowedStatuses.includes(run.status)) {
      this.logger.log(
        `Run ${runId} has status ${run.status}, skipping execution.`,
      );
      return {
        success: false,
        message: `Run status ${run.status} not allowed`,
        runId,
        status: run.status,
      };
    }
    ...
    try {
      ...
      if (!version) {
        this.logger.error(
          `Version ${run.versionId} not found for run ${runId}`,
        );
        await this.failRun(
          run,
          "This automation version is no longer available. The execution was stopped before starting.",
        );
        return {
          success: false,
      ...
      if (run.currentNodeId) {
        ...
        const isCurrentNodeCompleted = run.completedNodeIds?.includes(
          run.currentNodeId,
        );
        if (isCurrentNodeCompleted) {
          this.logger.log(
            `Node ${run.currentNodeId} is already completed, finding next node`,
          );
          // Find the next node after current node
          const currentNode = version.flow.nodes.find(
      ...
      if (!startNodeId) {
        ...
        startNodeId = findNextNodeId(
          version.flow.edges,
          run.executionState.trigger.nodeId,
        );
      }
      ...
      if (!startNodeId) {
        ...
        return {
          success: true,
          message: "Run completed (no start node found)",
          runId,
      ...
      result = {
        success: false,
        message: `Error in startExecution: ${error.message}`,
        runId,
        status: RunStatus.FAILED,
      };
    } finally {
  ...
  async resumeFromWhatsappInteraction(
    ...
    const step = await this.stepRepo
      ...
      .where(`step."outputData"->>'messageId' = :messageId`, {
    ...
    try {
      ...
      if (
        ...
        if (result.success) {
          ...
          if (result.code === "UPSELL_EXPIRED") {
            ...
            feedbackText =
              "❌ عذراً، لا يمكن إضافة العرض حالياً لأن الطلب تم توصيله.";
          } else if (result.code === "INVALID_ORDER_STATUS") {
            feedbackText =
              "❌ عذراً، لا يمكن إضافة العرض حالياً لأن الطلب قيد التجهيز أو الشحن.";
      ...
      this.logger.error(
        `=== ERROR in resumeFromWhatsappInteraction(${step.runId}):`,
        error,
      );
    ...
    if (!version) {
      return { success: false, message: "Version not found", runId: run.id };
    }
    ...
    if (node?.data?.type === ActionType.SEND_UPSELL) {
      if (buttonId?.endsWith("_btn_0")) {
        ...
        if (upsellApplyResultCode === "SUCCESS") {
          chosenBranch = branches.find(
            (b) => b.id === "accept" || b.label === "accept",
          );
        } else {
          chosenBranch = branches.find(
            (b) => b.id === "reject" || b.label === "reject",
          );
        }
      } else if (buttonId?.endsWith("_btn_1")) {
        // If clicked Reject
        chosenBranch = branches.find(
          (b) => b.id === "client_reject" || b.label === "client_reject",
        );
      }
    }
    ...
    if (!chosenBranch) {
      this.logger.error(
        `No matching branch found for button "${buttonText}" in node ${node.id}`,
      );
      await this.failRun(
        run,
        `User clicked an invalid button that has no configured branch: ${buttonText}`,
      );
      return {
        success: false,
  ...
  async resumeFromWait(
    ...
    if (run.status !== RunStatus.PAUSED || run.currentNodeId !== waitNodeId) {
      this.logger.warn(
        `Skipping wait-resume for run ${runId} (status=${run.status}, node=${run.currentNodeId})`,
      );
      return {
        success: false,
        message: "Run is not paused at the expected wait node",
        runId,
        status: run.status,
      };
    }
    ...
    try {
      ...
      const noResponseBranch = branches.find(
        (b: any) => b.isNoResponse === true,
      );
      if (noResponseBranch) {
      ...
      this.logger.warn(
        `Could not inspect node ${waitNodeId} for a no-response branch: ${error.message}`,
      );
    }
  ...
  private async runLoop(
    ...
    while (currentNodeId) {
      ...
      try {
        ...
        if (!result.success) {
          this.logger.error(
            `Handler failed for node ${currentNodeId}: ${result.error}`,
          );
          await this.failRun(
            run,
            result.error ||
        ...
        this.logger.log(
          `Finding next node after ${currentNodeId} with branch ${result.chosenBranch}`,
        );
        currentNodeId = findNextNodeId(
          flow.edges,
          currentNodeId,
        ...
        this.logger.error(
          `=== ERROR in runLoop for node ${currentNodeId}:`,
          error,
        );
        await this.failRun(
          run,
  ...
  private async sendAutomationNotification(
    ...
    try {
      ...
      this.logger.error(
        `Failed to send automation notification: ${error.message}`,
      );
    }
  }
