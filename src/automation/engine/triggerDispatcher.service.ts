...
export class TriggerDispatcherService {
  ...
  async dispatch(trigger: {
    ...
    console.log(
      `[TriggerDispatcher] Received dispatch request for ${trigger.type} on ${trigger.entityType} ${trigger.entityId} (admin: ${trigger.adminId})`,
    );
    if (!trigger.adminId) {
    ...
    console.log(
      `[TriggerDispatcher] Found ${automations.length} published automations for ${trigger.type}`,
    );
    ...
    for (const automation of automations) {
      console.log(
        `[TriggerDispatcher] Checking automation ${automation.id} (${automation.name})`,
      );
      const shouldRun = this.shouldStartAutomation(automation, trigger);
      console.log(
        `[TriggerDispatcher] Automation ${automation.id} should run: ${shouldRun}`,
      );

      if (!shouldRun) {
        console.log(
          `[TriggerDispatcher] Automation ${automation.id} skipped (shouldRun is false)`,
        );
        continue;
      }

      console.log(
        `[TriggerDispatcher] Starting createRunAndQueue for automation ${automation.id}`,
      );
      await this.createRunAndQueue(automation, trigger);
      console.log(
        `[TriggerDispatcher] Finished createRunAndQueue for automation ${automation.id}`,
      );
    }
  }
  ...
  async autoRetryFailedRuns(adminId: string, automationFlowId: string) {
    ...
    const strategy =
      settings?.automationMigrationStrategy ||
      AutomationMigrationStrategy.LATEST_MAJOR;
    ...
    const newVersion = automation.latestVersion;
  ...
  private async createRunAndQueue(
    ...
    try {
      const savedRun = await this.dataSource.transaction(async (manager) => {
        ...
        console.log(
          `[TriggerDispatcher] Using automation version ${version?.id}`,
        );
        ...
        const triggerNode = version.flow.nodes.find(
          (node) => node.type === FlowNodeType.TRIGGER,
        );
        if (!triggerNode) {
          console.log(
            `[TriggerDispatcher] No trigger node found in flow, skipping`,
          );
          return;
        }
  ...
  private shouldStartAutomation(
    ...
    // Find trigger node
    const triggerNode = flow.nodes.find((n) => n.type === FlowNodeType.TRIGGER);
