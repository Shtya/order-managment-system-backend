...
export class PlansService {
  ...
  async get(me: User, id: string) {
    ...
    if (!plan) {
      throw new NotFoundException(
        this.translations.t("domains.plans.not_found"),
      );
    }
    ...
    if (this.isSuperAdmin(me)) {
      ...
      throw new ForbiddenException(
        this.translations.t("domains.plans.not_allowed"),
      );
    }
    ...
    if (this.isAdmin(me)) {
      ...
      throw new ForbiddenException(
        this.translations.t("domains.plans.not_your_plan"),
      );
    }
  ...
  async create(me: User, dto: CreatePlanDto) {
    ...
    if (!this.isSuperAdmin(me)) {
      throw new ForbiddenException(
        this.translations.t(
          "domains.plans.only_super_admins_can_create_global_plans",
        ),
      );
    }
    ...
    if (exists) {
      throw new BadRequestException(
        this.translations.t("domains.plans.plan_name_already_exists"),
      );
    }
    ...
    const plan = this.plansRepo.create({
      ...
      //
      durationIndays:
        dto.durationIndays !== undefined ? dto.durationIndays : null,

      price: dto.price,
      extraOrderFee: dto.extraOrderFee !== undefined ? dto.extraOrderFee : null,

      includedOrders:
        dto.includedOrders !== undefined ? dto.includedOrders : null,
      ...
      shippingCompaniesLimit:
        dto.shippingCompaniesLimit !== undefined
          ? dto.shippingCompaniesLimit
          : 0,
  ...
  async update(me: User, id: string, dto: UpdatePlanDto) {
    ...
    if (dto.includedOrders !== undefined) {
      plan.includedOrders = dto.includedOrders;
    }
    ...
    if (dto.descriptionEn !== undefined) {
      plan.descriptionEn = dto.descriptionEn.trim();
    }
  ...
  async remove(me: User, id: string) {
    ...
    if (activeSubscriptionCount > 0) {
      throw new BadRequestException(
        this.translations.t("domains.plans.cannot_delete_plan", {
          args: { count: activeSubscriptionCount },
        }),
      );
    }
    ...
    return {
      message: this.translations.t("domains.plans.plan_deleted_successfully"),
    };
  }
