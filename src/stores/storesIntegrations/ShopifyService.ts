...
const ShopifyTopicToGraphQL: Record<ShopifyTopic, string> = {
  ...
  [ShopifyTopic.ORDERS_RISK_ASSESSMENT_CHANGED]:
...
export class ShopifyService extends BaseStoreProvider {
  ...
  baseUrl: string =
    process.env.SHOPIFY_BASE_URL || "https://api.easy-orders.net/api/v1";
  ...
  private async getStoreByExternalStoreId(
  ...
  private async getOrderIdFromFulfillmentOrderId(
  ...
  public async Init(query: Record<string, any>, adminId: string) {
    this.logger.log(
      `[Shopify] Init started | adminId: ${adminId} | query keys: ${Object.keys(query).join(", ")}`,
    );
    ...
    const normalizedShopQuery = rawShop
      ?.replace(/^https?:\/\/(www\.)?|\/$/g, "")
      .trim();
    const frontendBaseUrl = process.env.FRONTEND_URL?.trim();

    this.logger.log(
      `[Shopify] Init params extracted | rawShop: ${rawShop} | shop: ${shop} | normalizedShop: ${normalizedShopQuery} | hmac present: ${!!hmac} | param keys: ${Object.keys(params).join(", ")}`,
    );

    if (!shop || !hmac || !adminId) {
      this.logger.warn(
        `[Shopify] Init validation failed | missing: ${!shop ? "shop" : ""} ${!hmac ? "hmac" : ""} ${!adminId ? "adminId" : ""}`,
      );
      return {
        url: `${frontendBaseUrl}/store-integration?error=shopify_invalid_session`,
      };
    }

    this.logger.log(
      `[Shopify] Init looking up store | adminId: ${adminId} | normalizedStoreUrl: ${normalizedShopQuery}`,
    );
    ...
    if (!store) {
      this.logger.warn(
        `[Shopify] Init store not found | adminId: ${adminId} | normalizedStoreUrl: ${normalizedShopQuery}`,
      );
      return {
    ...
    this.logger.log(
      `[Shopify] Init store found | storeId: ${store.id} | storeUrl: ${store.storeUrl} | isIntegrated: ${store.isIntegrated} | isActive: ${store.isActive}`,
    );
    ...
    this.logger.log(
      `[Shopify] Init HMAC message built | param count: ${Object.keys(params).length} | message length: ${message.length}`,
    );
    ...
    if (!isValid) {
      this.logger.warn(
        `[Shopify] Init HMAC verification failed | adminId: ${adminId} | storeId: ${store.id}`,
      );
      return {
        url: `${frontendBaseUrl}/store-integration?error=shopify_security_verification_failed`,
      };
    }

    this.logger.log(
      `[Shopify] Init HMAC verified successfully | adminId: ${adminId} | storeId: ${store.id}`,
    );

    try {
      const oldIsIntegrated = store.isIntegrated;
      this.logger.log(
        `[Shopify] Init previous integration state | wasIntegrated: ${oldIsIntegrated} | syncRemoteProducts: ${store.syncRemoteProducts}`,
      );

      if (!oldIsIntegrated) {
        this.logger.log(
          `[Shopify] Init first integration - subscribing to webhooks | storeId: ${store.id}`,
        );
        await this.subscribeAllWebhooks(store);
        this.logger.log(
          `[Shopify] Init webhooks subscribed successfully | storeId: ${store.id}`,
        );
      } else {
        this.logger.log(
          `[Shopify] Init skipping webhook subscription - already integrated | storeId: ${store.id}`,
        );
      }
      ...
      this.logger.log(
        `[Shopify] Init updating store | storeId: ${store.id} | setting isActive=true, isIntegrated=true, externalStoreId=${rawShop}`,
      );

      await this.storesRepo.save(store);
      this.logger.log(
        `[Shopify] Init store saved successfully | storeId: ${store.id}`,
      );
      ...
      this.logger.log(
        `[Shopify] Init onboarding achievement enqueued | adminId: ${adminId} | type: STORE_CONNECTED`,
      );

      if (!oldIsIntegrated && store.syncRemoteProducts) {
        this.productSyncQueueService.enqueueFullProductSyncLocally(store);
        this.logger.log(
          `[Shopify] Init full product sync enqueued | storeId: ${store.id} | adminId: ${adminId}`,
        );
      } else {
        this.logger.log(
          `[Shopify] Init skipping product sync | wasIntegrated: ${oldIsIntegrated} | syncRemoteProducts: ${store.syncRemoteProducts}`,
        );
      }

      this.logger.log(
        `[Shopify] Init completed successfully | storeId: ${store.id} | adminId: ${adminId} | redirect: ${redirectUrl}`,
      );
      return { url: redirectUrl };
    } catch (error) {
      this.logger.error(
        `[Shopify] Init error | adminId: ${adminId} | storeId: ${store?.id} | error: ${error.message}`,
        error.stack,
      );
      const errorMessage = this.getErrorMessage(error);
      return {
        url: `${frontendBaseUrl}/store-integration?errorMessage=${encodeURIComponent(errorMessage)}`,
      };
    }
  }
  ...
  private async getAccessToken(store: StoreEntity): Promise<string> {
    ...
    this.logCtx(
      `[Shopify] No access token found. Generating new one...`,
      store,
    );
    ...
    return this.executeWithLimiter(
      ...
      async () => {
        try {
          ...
          if (error) {
            const systemCode =
              (error as any).code || (error as any).networkError?.code;
            ...
            if (isNetworkError) {
              this.logCtxWarn(
                `[Shopify] Network Error (${systemCode}) for store: ${store.storeUrl}. Triggering retry...`,
                store,
              );

              // Throw the error with the code intact so executeWithLimiter recognizes it
              const networkErr: any = new Error(
                `Network ${systemCode}: ${error?.message}`,
              );
            ...
            if (gqlErrors.length > 0) {
              ...
              if (isThrottled) {
                this.logCtxWarn(
                  `[Shopify] GraphQL Rate limit hit for store: ${store.storeUrl}`,
                  store,
                );
              ...
              this.logCtxError(
                `GraphQL Errors: ${JSON.stringify(gqlErrors)}`,
                store,
              );
              ...
              if (typeof firstError === "string") {
                try {
                  ...
                  errorMessage =
                    parsed?.message || parsed?.errors || firstError;
                } catch {
  ...
  private async getCollections(
    ...
    if (cleanHandles.length > 0) {
      queryParts.push(...cleanHandles.map((handle) => `handle:${handle}`));
    }
    ...
    if (cleanNames.length > 0) {
      queryParts.push(...cleanNames.map((name) => `title:"${name}"`));
    }

    // Combine into a single query: (handle:... OR title:"...")
    const searchQuery =
      queryParts.length === 1 ? queryParts[0] : `(${queryParts.join(" OR ")})`;
    ...
    const collections =
      response?.collections?.nodes ?? response?.data?.collections?.nodes ?? [];
  ...
  `;
    ...
    if (userErrors && userErrors.length > 0) {
      throw new Error(`Shopify webhook delete error: ${userErrors[0].message}`);
    }
  ...
  public async subscribeAllWebhooks(store: StoreEntity) {
    ...
    const topics: { topic: ShopifyTopic; url?: string; oldUrl?: string }[] = [
      {
        topic: ShopifyTopic.ORDERS_CREATE,
        url: createUrl,
        oldUrl: oldCreateUrl,
      },
      { topic: ShopifyTopic.ORDERS_CANCELLED },
      { topic: ShopifyTopic.ORDERS_DELETE },
      { topic: ShopifyTopic.ORDERS_UPDATED },
      { topic: ShopifyTopic.ORDERS_PAID },
      { topic: ShopifyTopic.ORDERS_RISK_ASSESSMENT_CHANGED },
    ...
    for (let i = 0; i < topics.length; i += chunkSize) {
      ...
      await Promise.all(
        chunk.map(async ({ topic, url, oldUrl }) => {
          ...
          if (oldUrl) {
            const existingOld = await this.ensureWebhookForTopic(
              store,
              topic,
              oldUrl,
            );
  ...
  public async unsubscribeAllWebhooks(store: StoreEntity) {
    ...
    const identifiers = Array.from(
      new Set([store.id, "shopify"].filter(Boolean)),
    );
  ...
  public mapWebhookUpdate(
  ...
  ): WebhookOrderUpdatePayload | null {
    ...
    const postponedDate: string | null = null;
    switch (topic) {
      ...
      case ShopifyTopic.ORDERS_UPDATED:
        const { orderStatus, paymentStatus } =
          this.mapShopifyWebhookStatusToInternal(body);
        if (orderStatus) {
  ...
  `;
    ...
    try {
      ...
      if (userErrors && userErrors.length > 0) {
        this.logger.error(
          `[Shopify] appUninstall failed: ${JSON.stringify(userErrors)}`,
        );
      }
      ...
      try {
        ...
        this.logger.error(
          `[Shopify] Failed to mark store as inactive after appUninstall: ${saveErrMsg}`,
        );
      }
    }
  }
  ...
  private mapShopifyWebhookStatusToInternal(body: any): {
    ...
    const paymentStatus =
      this.mapShopifyFinancialStatusToPaymentStatus(financialStatus);
    const orderStatus = this.mapShopifyStatusesToOrderStatus(fulfillmentStatus);
  ...
  private async buildProductSetInput(
    ...
    if (hasOnlyDefaultVariant) {
      ...
      productOptions = Array.from(optionsMap.entries()).map(
        ([name, values]) => ({
          name,
  ...
  private async getShopifyUpsellMetafield(
    product: ProductEntity,
    store: StoreEntity,
  ): Promise<any> {
    ...
    for (const upsell of product.upsellingProducts) {
      ...
      const remoteProduct = await this.syncProduct({
  ...
  private async removeProductFromCategoryCollection(
    store: StoreEntity,
  ...
  `;
    ...
    const resp = await this.runGraphQL(store, false, query, {});
    const id =
      resp?.locations?.edges?.find((edge) => edge.node?.shipsInventory)?.node
        ?.id || resp?.locations?.edges?.[0]?.node?.id;
  ...
  private async updateProductWithProductSet(
    ...
    const identifier = remoteProduct?.id
  ...
  private async syncCategoriesCursor(store: StoreEntity): Promise<{
    ...
    while (hasMore) {
      ...
      const remoteCollections = await this.getCollections(
        store,
        handles,
        names,
      );
      ...
      for (const cat of localBatch) {
        ...
        const extId = remoteMap.get(localHandle);
  ...
  private async syncProductsCursor(
    ...
    while (hasMore) {
      ...
      const remoteItems =
        remoteIds.length > 0
          ? await this.getProductsByIds(store, remoteIds)
          : [];
      ...
      for (const bundle of localBatch) {
        ...
        try {
          ...
          if (remote) {
            ...
            syncedBundle = await this.updateBundleWithProductSet(store, bundle);
          }
  ...
  public async syncCategory({
    ...
    const existingCollection = await this.getCollection(
      store,
      checkHandle,
      checkName,
    );
  ...
  private async disableRemoteBundleVariant(
    ...
    if (syncState && syncState.remoteProductId) {
      ...
      if (
        remoteVariant &&
        remoteVariant.id &&
        remoteVariant.requiresComponents
      ) {
  ...
  public async syncProduct({ productId }: { productId: string }) {
    ...
    try {
      ...
      if (product.category) {
        externalCategory = await this.syncCategory({
          category: product.category,
          slug: product.category.slug,
          relatedAdminId: product.adminId,
        });
      ...
      if (externalId) {
        const isPublished = await this.publishProductToOnlineStore(
          activeStore,
          syncedProduct.id,
        );
  ...
  public async updateOrderStatus(
    ...
    if (!order.externalId) return { actions };
    ...
    const startAction = this.mapOldStatusToShopifyAction(
      internalStatus,
      internalOldStatus,
    );
    if (startAction !== "NONE") {
  ...
  `;
    ...
    if (!orderNode) {
      throw new Error(`Could not fetch order ${orderGid} fulfillments`);
    }
  ...
  `;
    ...
    if (!orderNode) {
      throw new Error(`Could not fetch order ${orderGid} fulfillment orders`);
    }
  ...
  `;
    ...
    if (!orderNode) {
      throw new Error(`Could not fetch order ${orderGid} fulfillment data`);
    }

    return orderNode;
  }

  private async cancelSingleFulfillment(
    store: StoreEntity,
    order: OrderEntity,
  ...
  `;
    ...
    const payload =
      response?.fulfillmentCancel ?? response?.data?.fulfillmentCancel ?? null;
    if (!payload) {
      throw new Error(
        `fulfillmentCancel returned empty payload for fulfillment ${fulfillmentId}`,
      );
    }
    ...
    if (userErrors && userErrors.length > 0) {
      throw new Error(
        `Failed to cancel fulfillment ${fulfillmentId} for order ${order.id}: ${userErrors[0].message}`,
      );
    }

    this.logger.log(
      `[Shopify] Successfully canceled fulfillment ${fulfillmentId} for order ${order.id}`,
    );
  }
  ...
  private async createFulfillment(
    ...
    const supportsCreateFulfillment = (fo: any): boolean => {
      ...
      return actions.some((a: any) => a?.action === "CREATE_FULFILLMENT");
    };

    const hasRemainingQuantity = (fo: any): boolean => {
      const lineItems = fo.lineItems?.edges?.map((e: any) => e.node) ?? [];
      return lineItems.some((li: any) => (li?.remainingQuantity ?? 0) > 0);
    };
  ...
  private async cancelOrder(
    ...
    for (const fulfillment of fulfillments) {
      ...
      if (status === "CANCELLED" || status === "FAILURE") {
        this.logger.log(
          `[Shopify] Skipping fulfillment ${fulfillment.id} for order ${order.id} with status ${status}`,
        );
        continue;
      }
    ...
    for (const fo of fulfillmentOrders) {
      ...
      if (
        ...
        this.logger.log(
          `[Shopify] Skipping fulfillment order ${fo.id} for order ${order.id} with status ${status}`,
        );
        continue;
      }
    ...
    `;
        ...
        const payload =
          response?.fulfillmentOrderCancel ??
          response?.data?.fulfillmentOrderCancel ??
          null;

        if (payload?.userErrors?.length > 0) {
          this.logger.warn(
            `[Shopify] Could not cancel fulfillment order ${fo.id} for order ${order.id}: ${payload.userErrors[0].message}`,
          );
        } else {
          this.logger.log(
            `[Shopify] Successfully canceled fulfillment order ${fo.id} for order ${order.id}`,
          );
        }
    ...
    `;
        ...
        const payload =
          response?.fulfillmentOrderSubmitCancellationRequest ??
          response?.data?.fulfillmentOrderSubmitCancellationRequest ??
          null;

        if (payload?.userErrors?.length > 0) {
          this.logger.warn(
            `[Shopify] Could not submit cancellation request for fulfillment order ${fo.id} for order ${order.id}: ${payload.userErrors[0].message}`,
          );
        } else {
          this.logger.log(
            `[Shopify] Successfully submitted cancellation request for fulfillment order ${fo.id} for order ${order.id}`,
          );
        }
      }
    }
  ...
  private async markFulfillmentInProgress(
    ...
    const canReportProgress = (fo: any): boolean => {
      const actions = fo.supportedActions ?? [];
      return actions.some((a: any) => a?.action === "REPORT_PROGRESS");
    };
  ...
  private async markOrderDelivered(
    ...
    if (!fulfillments.length) {
      this.logger.log(`[Shopify] No fulfillments found for order ${order.id}`);
      return;
    }
  ...
  `;
    ...
    const variables = {
      fulfillmentEvent: {
        fulfillmentId,
        status: "DELIVERED",
        message: message ?? "Order marked as delivered via integration.",
  ...
  private async createReturnRequest(
    ...
    if (!returnRequest) {
      throw new Error(
        `Return request ${order.lastReturnId} not found for order ${order.id}`,
      );
    }
  ...
  `;
    ...
    const returnableResponse = await this.runGraphQL(
      store,
      false,
      returnableQuery,
      {
    ...
    if (!returnableNode) {
      throw new Error(
        `Failed to fetch returnable fulfillments for order ${order.id}`,
      );
    }

    // 3. Flatten returnable line items and map by SKU
    const allReturnableLineItems = (returnableNode.edges || []).flatMap(
      (edge: any) =>
        (edge.node?.returnableFulfillmentLineItems?.edges || []).map(
          (liEdge: any) => liEdge.node,
        ),
    );
    ...
    for (const returnItem of returnRequest.items) {
      ...
      if (!returnableItem) {
        throw new Error(
          `No returnable fulfillment line item found for SKU ${variant.sku}`,
        );
      }
  ...
  `;
    ...
    const response = await this.runGraphQL(store, true, mutation, variables);
    const payload =
      response?.returnRequest ?? response?.data?.returnRequest ?? null;

    if (!payload) {
      throw new Error(
        `returnRequest returned empty payload for order ${order.id} (${order.externalId})`,
      );
    }

    if (payload.userErrors?.length > 0) {
      throw new Error(
        `Failed to create return request for order ${order.id}: ${payload.userErrors[0].message}`,
      );
    }
    ...
    if (!returnId) {
      throw new Error(
        `returnRequest did not return a return ID for order ${order.id}`,
      );
    }

    this.logger.log(
      `[Shopify] Created return request ${returnId} for order ${order.id} with ${returnLineItems.length} items`,
    );
    return returnId;
  }
  private async getShopifyReturnDetails(
    order: OrderEntity,
    store: StoreEntity,
  ): Promise<{
    id: string;
  ...
  `;
    ...
    const returnLineItems = (returnData.returnLineItems?.edges || []).map(
      (edge: any) => {
  ...
  private async approveReturnRequest(
    ...
    if (shopifyReturn.status !== "REQUESTED") {
      throw new Error(
        `Return request ${shopifyReturn.id} is not in REQUESTED status`,
      );
    }
  ...
  `;
    ...
    const approveResponse = await this.runGraphQL(
      store,
      true,
      approveMutation,
      approveVariables,
    ...
    if (!approvePayload) {
      throw new Error(
        `returnApproveRequest returned empty payload for return ${shopifyReturn.id}`,
      );
    }

    if (approvePayload.userErrors?.length > 0) {
      throw new Error(
        `Failed to approve return request ${shopifyReturn.id}: ${approvePayload.userErrors[0].message}`,
      );
    }

    this.logger.log(
      `[Shopify] Approved return request ${shopifyReturn.id} (status=${approvePayload.return?.status})`,
    );
    ...
    for (const localItem of returnRequest.items) {
      ...
      if (!shopifyReturnLineItem) {
        throw new Error(
          `Could not find Shopify return line item for SKU ${localItem.returnedVariant?.sku}`,
        );
      }

      if (!shopifyReturnLineItem.reverseFulfillmentOrderLineItem?.id) {
        throw new Error(
          `Shopify return line item ${shopifyReturnLineItem.id} has no reverse fulfillment order line item`,
        );
      }
      ...
      dispositions.push({
        ...
        reverseFulfillmentOrderLineItemId:
          shopifyReturnLineItem.reverseFulfillmentOrderLineItem.id,
        locationId,
        dispositionType:
          localItem.condition === "Damaged" ? "DISCARDED" : "RESTOCKED",
      });
    }
  ...
  `;
    ...
    for (const disp of options.dispositions) {
      ...
      groupedByReturnLineItem[disp.returnLineItemId].push({
        reverseFulfillmentOrderLineItemId:
          disp.reverseFulfillmentOrderLineItemId,
  ...
  private async declineReturnRequest(
    ...
    const { id: returnId, status } = await this.getShopifyReturnDetails(
      order,
      store,
    );
    if (status !== "REQUESTED") {
  ...
  `;
    ...
    if (!payload) {
      throw new Error(
        `returnDeclineRequest returned empty payload for return ${returnId}`,
      );
    }

    if (payload.userErrors?.length > 0) {
      throw new Error(
        `Failed to decline return request ${returnId}: ${payload.userErrors[0].message}`,
      );
    }

    this.logger.log(
      `[Shopify] Declined return request ${returnId} (status=${payload.return?.status})`,
    );
  }
  ...
  public mapOldStatusToShopifyAction(
    status: OrderStatus,
    ...
    if (
      oldStatus === OrderStatus.RETURN_PREPARING &&
  ...
  public async fetchRemoteProducts(
    store: StoreEntity,
    ids: string[],
  ): Promise<any[]> {
  ...
  public async mapWebhookCreate(
    ...
    const paymentMethod =
      body.payment_gateway_names?.length > 0
        ? this.mapPaymentMethod(body.payment_gateway_names?.[0] || "")
    ...
    const remoteProducts = await this.getProductsByIds(
      store,
      uniqueIds as string[],
    );
    ...
    const variantIdToOptionsMap = new Map<
      string,
    ...
    const address =
      `${billing.address1 || ""} ${billing.address2 || ""}`.trim();
    return {
      externalOrderId: String(
        String(body.id).startsWith("gid://")
          ? body.id
          : `gid://shopify/Order/${body.id}`,
      ),
      ...
      cartItems: lineItems.map((item: any) => {
        ...
        const gidId = prodId.startsWith("gid://")
          ? prodId
          : `gid://shopify/Product/${prodId}`;
        return {
  ...
  public async getProductsByIds(
    ...
    try {
      ...
      const products =
        nodes.filter((n: any) => n && n.__typename === "Product") || nodes;
  ...
  private mapRemoteProductToDto(remote: any): MappedProductDto {
    ...
    const variants = (remote.variants?.nodes || []).map((v: any) => {
      ...
      if (!hasOnlyDefaultVariant) {
        baseVariant.variation_props = (v.selectedOptions || []).map(
          (o: any) => ({
    ...
    if (!hasOnlyDefaultVariant) {
      ...
      variations = Array.from(variationMap.entries()).map(([name, values]) => ({
        id: name,
        name: name?.trim(),
    ...
    const productExpense = firstVariant?.inventoryItem?.unitCost?.amount
  ...
  async validateProviderConnection(store: StoreEntity): Promise<boolean> {
    ...
    if (!storeUrl || !apiKey || !accessToken) {
      this.logger.error(
        `[Shopify] Validation failed: Missing storeUrl, apiKey, or accessToken`,
      );
      return false;
    }
  ...
  public async getAllMappedProducts(
    ...
    while (hasNextPage) {
      try {
        ...
        const message = this.getErrorMessage(error);
        this.logger.error(
          `[Shopify] Failed to fetch products batch: ${message}`,
        );
  ...
  public override normalizeOrderId(externalOrderId: string): string {
    return String(externalOrderId).startsWith("gid://")
      ? externalOrderId
  ...
  public async processExternalOrderId(
    ...
    const store = externalStoreId
      ? await this.getStoreByExternalStoreId(externalStoreId)
      : null;
    ...
    const orderIdResolvers: Record<ShopifyTopic, OrderIdResolver> = {
      ...
      [ShopifyTopic.ORDERS_RISK_ASSESSMENT_CHANGED]: (body) =>
        Promise.resolve(body?.order_id),
  ...
  `;
    ...
    const input = await this.buildBundleSetInput(bundle, locationId, store);
    const variables: any = {
      input,
      synchronous: true,
    };
    ...
    if (userErrors && userErrors.length > 0) {
      throw new Error(
        `Shopify bundle productSet Error: ${userErrors[0].message}`,
      );
    }
  ...
  public async syncBundle(bundle: BundleEntity): Promise<any> {
    ...
    try {
      ...
      if (externalId) {
        const remoteProduct = await this.getProduct(activeStore, externalId);
        if (remoteProduct) {
          syncedProduct = await this.updateBundleWithProductSet(
            activeStore,
            loadedBundle,
            remoteProduct,
          );
