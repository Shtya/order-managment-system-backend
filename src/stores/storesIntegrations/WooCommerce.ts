...
export default class WooCommerceService
  extends BaseStoreProvider
  implements ISkuFetch
{
  ...
  baseUrl: string =
    process.env.WOOCOMMERCE_BASE_URL || "https://api.easy-orders.net/api/v1";
  ...
  constructor(
    ...
    protected readonly bundleRepo: Repository<BundleEntity>,
  ...
  private async mapWooCommerceProductToDto(
    remote: any,
    remoteVariations: any,
    store: StoreEntity,
  ): Promise<MappedProductDto> {
    ...
    return {
      ...
      price: Number(remote.price) || 0,
      type:
        remote.type === "simple" ? ProductType.SINGLE : ProductType.VARIABLE,
      expense: 0,
  ...
  private buildBasicAuthHeader(clientKey: string, clientSecret: string) {
    ...
    const token = Buffer.from(`${clientKey}:${clientSecret}`).toString(
      "base64",
    );
    return `Basic ${token}`;
  }
  ...
  private async syncWooVariations(
    ...
    const mappedVariants = await this.mapWooVariationsPayload(
      variants,
      attrMap || new Map(),
    );
    ...
    if (createVariant.length || updateVariant.length || deleteVariant.length) {
      ...
      await this.syncWooVariantsBySku(variants, responseData, store);
    } else {
    }
  }
  ...
  private async getOrCreateWooAttribute(
    store: StoreEntity,
    ...
    const match = attributes.find(
      (a: any) => a.name.toLowerCase() === attributeName.toLowerCase(),
    );
   ...
   */
  private async ensureAttributesForVariants(
    store: StoreEntity,
    variants: ProductVariantEntity[],
   ...
   */
  private async syncAttributeTermsInBatch(
    store: StoreEntity,
    attributeId: string,
    neededOptions: string[],
  ) {
    ...
    const existingNames = new Set(
      existingTerms.map((t: any) => t.name.toLowerCase().trim()),
    );

    // Find options that don't exist yet
    const missingOptions = neededOptions.filter(
      (opt) => !existingNames.has(opt.toLowerCase().trim()),
    );
  ...
  private async getCategory(store: StoreEntity, slug?: string, name?: string) {
    ...
    const [slugResult, nameResult] = await Promise.allSettled([
      slugPromise,
      namePromise,
    ]);
  ...
  private async createProduct(
    ...
    const attrMap = await this.ensureAttributesForVariants(
      store,
      activeVariants,
    );
  ...
  private async updateProduct(
    ...
    const updated = response?.data ?? response;
    await this.syncWooVariations(
      updated?.id || externalId,
      activeVariants,
      store,
      attrMap,
    );
  ...
  protected async getUpsellIds(
    ...
    for (const upsell of product.upsellingProducts) {
      ...
      const syncedProduct = await this.syncProduct({
  ...
  private async mapWooVariationsPayload(
    ...
    return Promise.all(
      variants.map(async (v) => ({
        ...
        sku: String(v.sku ?? ""),
        manage_stock: true,
        stock_quantity: await this.ordersService.calculateAvailableStock(
          v.stockOnHand,
          v.reserved,
          v.adminId,
        ),
  ...
  public async syncCategory({
    ...
    const remoteCategory = await this.getCategory(
      activeStore,
      checkSlug,
      category.name,
    );
  ...
  private async syncCategoriesCursor(store: StoreEntity): Promise<{
    ...
    this.logCtx(
      `[Sync] ✓ Category sync completed | Total: ${stats.processed} | Created: ${stats.created} | Updated: ${stats.updated}`,
      store,
    );
    return {
      categoryMap,
  ...
  private async syncProductsCursor(
    ...
    this.logCtx(
      `[Sync] Starting product synchronization (Individual API calls)`,
      store,
    );
    ...
    this.logCtx(
      `[Sync] ✓ Product sync completed | Total: ${totalProcessed} | Errors: ${totalErrors}`,
      store,
    );
    return {
  ...
  private async syncBundleCursor(
    ...
    this.logCtx(
      `[Sync] Starting bundle synchronization (Individual API calls)`,
      store,
    );
      ...
      for (const bundle of localBatch) {
        try {
          ...
          if (remote) {
            ...
            syncedBundle = await this.createBundle(bundle, store, extCatId);
  ...
  public async mapWebhookCreate(
    ...
    const uniqueIds = [
      ...new Set(lineItems.map((item: any) => String(item.product_id))),
    ];

    const remoteProducts = await this.fetchRemoteProducts(
      store,
      uniqueIds as string[],
    );
    const { orderStatus, paymentStatus } = this.mapExternalStatusToInternal(
  ...
  async validateProviderConnection(store: StoreEntity): Promise<boolean> {
    ...
    if (!storeUrl || !consumerKey || !consumerSecret) {
      this.logger.error(
        `[WooCommerce] Validation failed: Missing credentials or URL`,
      );
      return false;
    }
    ...
    try {
      ...
      if (status === 401) {
        this.logger.warn(
          `[WooCommerce] 401 Unauthorized: Invalid Key or Secret for ${storeUrl}`,
        );
      } else if (status === 404) {
        this.logger.warn(
          `[WooCommerce] 404 Not Found: Check if WooCommerce is installed at ${storeUrl}`,
        );
      } else {
        const message = this.getErrorMessage(error);
  ...
  private async mapBundlePayload(
    bundle: BundleEntity,
    externalCategoryId?: string,
  ) {
    ...
    return {
      ...
      images: [
        ...(bundle.mainImage
          ? [{ src: this.getImageUrl(bundle.mainImage) }]
          : []),
