...
export class StoreWebhooksController {
  ...
  async handleEasyOrdersAuth(
    ...
    this.logger.log(
      `Received EasyOrders credentials for store: ${body.store_id}${
        internalStoreId ? ` (internal store ${internalStoreId})` : ""
      }`,
    );
  ...
  async handleOrderCreate(
    ...
    return await this.storesService.handleWebhookOrderCreate(
      target,
      body,
      headers,
      adminId,
  ...
  async handleOrderStatusUpdate(
    ...
    return await this.storesService.handleWebhookOrderUpdate(
      target,
      body,
      headers,
      adminId,
