...
export class SuppliersService {
  ...
  async list(me: any, q?: any) {
    ...
    if (q?.phone) {
      query
        .andWhere("suppliers.phone LIKE :phone", { phone: `%${q.phone}%` })
        .andWhere("suppliers.secondPhone LIKE :phone", {
          phone: `%${q.phone}%`,
        });
    }
      ...
      query.andWhere(
        new Brackets((qb) => {
          qb.where("suppliers.name ILIKE :search", { search: `%${q.search}%` })
            .orWhere("suppliers.phone ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("suppliers.secondPhone ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("suppliers.email ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("suppliers.address ILIKE :search", {
              search: `%${q.search}%`,
            });
        }),
      );
  ...
  async get(me: any, id: string) {
    const adminId = tenantId(me);
    const entity = await CRUD.findOne(this.supplierRepo, "suppliers", id, [
      "categories",
    ]);
    return entity;
  }
  ...
  private async validateCategories(me: any, categoryIds: string[]) {
    if (!categoryIds || categoryIds.length === 0) {
      throw new BadRequestException(
        this.translations.t("domains.suppliers.at_least_one_category_required"),
      );
    }
    const adminId = tenantId(me);
    ...
    if (categories.length !== categoryIds.length) {
      throw new BadRequestException(
        this.translations.t("domains.suppliers.some_categories_not_found"),
      );
    }
  ...
  async update(me: any, id: string, dto: UpdateSupplierDto) {
    ...
    if (dto.description !== undefined) {
      (supplier as any).description = dto.description;
    }
    if (dto.phone !== undefined) (supplier as any).phone = dto.phone;
    if (dto.phoneCountry !== undefined) {
      (supplier as any).phoneCountry = dto.phoneCountry;
    }
    if (dto.secondPhone !== undefined) {
      (supplier as any).secondPhone = dto.secondPhone;
    }
    if (dto.secondPhoneCountry !== undefined) {
      (supplier as any).secondPhoneCountry = dto.secondPhoneCountry;
    }
  ...
  async updateFinancials(
    me: any,
    id: string,
    dto: UpdateSupplierFinancialsDto,
  ) {
    const supplier = await this.get(me, id);

    if (dto.dueBalance !== undefined) {
      (supplier as any).dueBalance = dto.dueBalance;
    }
    if (dto.purchaseValue !== undefined) {
      (supplier as any).purchaseValue = dto.purchaseValue;
    }
