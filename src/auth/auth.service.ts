...
export class AuthService {
  ...
  async validatePayload(payload: { sub: number }) {
    ...
    if (!user || !user.isActive) {
      throw new UnauthorizedException(
        this.translations.t("domains.auth.invalid_user"),
      );
    }
  ...
  async register(registerDto: RegisterDto) {
    const { name, email, password, phone, companyName, businessType } =
      registerDto;
    ...
    if (exists) {
      throw new BadRequestException(
        this.translations.t("domains.auth.email_already_used"),
      );
    }
    ...
    if (!userRole) {
      throw new BadRequestException(
        this.translations.t("domains.auth.role_not_seeded"),
      );
    }

    // 3. Pending User & Cooldown Logic
    let pendingUser = await this.pendingUserRepository.findOne({
      where: { email },
    });
    ...
    if (pendingUser?.lastSentAt) {
      const timeElapsedSeconds =
        (Date.now() - Number(pendingUser.lastSentAt)) / 1000;

      // 2. Check if we are still within the cooldown period
      if (timeElapsedSeconds < this.RESEND_COOLDOWN_SECONDS) {
        const remainingSeconds = Math.ceil(
          this.RESEND_COOLDOWN_SECONDS - timeElapsedSeconds,
        );

        throw new ForbiddenException(
          this.translations.t("domains.auth.cooldown_wait", {
  ...
  async verifyRegisterOtp(email: string, otp: string) {
    ...
    if (!pendingUser) {
      throw new BadRequestException(
        this.translations.t("domains.auth.verification_session_not_found"),
      );
    }
    ...
    if (!pendingUser.otpCodeHash || Date.now() > exp) {
      throw new BadRequestException(
        this.translations.t("domains.auth.otp_expired"),
      );
    }
    ...
    if (pendingUser.otpAttempts > 5) {
      ...
      await this.pendingUserRepository.save(pendingUser);
      throw new BadRequestException(
        this.translations.t("domains.auth.too_many_attempts"),
      );
    }
    ...
    if (!isMatch) {
      await this.pendingUserRepository.save(pendingUser);
      throw new BadRequestException(
        this.translations.t("domains.auth.invalid_otp"),
      );
    }
  ...
  async resendRegisterOtp(email: string) {
    ...
    if (!pendingUser) {
      throw new BadRequestException(
        this.translations.t("domains.auth.no_pending_email_change"),
      );
    }
  ...
  async login(email: string, password: string) {
    ...
    if (!user || !user.isActive) {
      throw new UnauthorizedException(
        this.translations.t("domains.auth.invalid_credentials"),
      );
    }
  ...
  async verifyResetOtp(email: string, otp: string) {
    ...
    if (!user) {
      throw new BadRequestException(
        this.translations.t("domains.auth.invalid_request"),
      );
    }
  ...
  async resetPasswordByOtp(email: string, newPassword: string) {
    ...
    if (!user.otpVerified || !user.otpCodeHash || Date.now() > exp) {
      throw new BadRequestException(
        this.translations.t("domains.auth.otp_verification_required"),
      );
    }
  ...
  async googleLogin(idToken: string, fallbackName?: string) {
    ...
    if (!email) {
      throw new UnauthorizedException(
        this.translations.t("domains.auth.google_no_email"),
      );
    }
    ...
    if (!user) {
      ...
      user = this.usersRepo.create({
        name:
          decoded.name ||
          fallbackName ||
          this.translations.t("domains.auth.fallback_google_user"),
        email,
        passwordHash: null,
        roleId: userRole.id,
        adminId: null,
    ...
    if (!user.isActive) {
      throw new UnauthorizedException(
        this.translations.t("domains.auth.account_inactive"),
      );
    }
  ...
  async changePasswordByOldPassword(
    ...
    if (!isMatch) {
      throw new BadRequestException(
        this.translations.t("domains.auth.invalid_current_password"),
      );
    }
    ...
    return {
      message: this.translations.t("domains.auth.password_already_exists"),
    };
  }
  ...
  async requestEmailChange(userId: string, newEmail: string) {
    ...
    if (emailExists) {
      throw new ConflictException(
        this.translations.t("domains.auth.email_in_use"),
      );
    }
    ...
    if (user.email === newEmail) {
      throw new BadRequestException(
        this.translations.t("domains.auth.already_current_email"),
      );
    }
    ...
    return {
      message: this.translations.t("domains.auth.email_change_code_sent"),
    };
  }
