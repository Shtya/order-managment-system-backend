...
export class AssetController {
  ...
  async upload(
    ...
    if (!user) {
      throw new NotFoundException(
        this.translations.t("common.authenticated_user_not_found"),
      );
    }
  ...
  async uploadMultiple(
    ...
    const assets = await Promise.all(
      files.map((file) => this.assetService.create(dto, file, user)),
    );

    return {
      message: this.translations.t(
        "domains.assets.assets_uploaded_successfully",
      ),
      assets,
    };
  }
