...
export class SystemErorrsService {
  ...
  async list(me: any, q?: any) {
    ...
    const query = this.systemErrorRepo
    ...
    // Filter by endpoint
    if (q?.endpoint) {
      query.andWhere("errors.endpoint LIKE :endpoint", {
        endpoint: `%${q.endpoint}%`,
      });
    }
    ...
    if (q?.httpStatus) {
      if (q.httpStatus === "all_400") {
        ...
        query.andWhere("errors.httpStatus = :httpStatus", {
    ...
    if (q?.exceptionName) {
      query.andWhere("errors.exceptionName = :exceptionName", {
        exceptionName: q.exceptionName,
      });
    }

    if (q?.routePath) {
      query.andWhere("errors.routePath = :routePath", {
        routePath: q.routePath,
      });
    }

    if (q?.environment) {
      query.andWhere("errors.environment = :environment", {
    ...
    // Filter by errorCode
    if (q?.errorCode) {
      query.andWhere("errors.errorCode LIKE :errorCode", {
    ...
    // Filter by ipAddress
    if (q?.ipAddress) {
      query.andWhere("errors.ipAddress LIKE :ipAddress", {
    ...
    if (q?.search) {
      query.andWhere(
        new Brackets((qb) => {
          qb.where("errors.errorMessage ILIKE :search", {
            search: `%${q.search}%`,
          })
            .orWhere("errors.endpoint ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("errors.routePath ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("errors.originalUrl ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("errors.exceptionName ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("errors.errorCode ILIKE :search", {
              search: `%${q.search}%`,
            })
            .orWhere("errors.referer ILIKE :search", {
              search: `%${q.search}%`,
            });
        }),
      );
  ...
  async exportErrors(me: any, q: any) {
    ...
    const exportData = records.map((r) => ({
      ...
      validationErrors: r.validationErrors
        ? JSON.stringify(r.validationErrors)
        : "",
      dbContext: r.dbContext ? JSON.stringify(r.dbContext) : "",
      externalContext: r.externalContext
        ? JSON.stringify(r.externalContext)
        : "",
      additionalDetails: r.additionalDetails
        ? JSON.stringify(r.additionalDetails)
        : "",
  ...
  async getStats(me: any) {
    ...
    const mostFrequentRouteQuery = this.systemErrorRepo
      .createQueryBuilder("errors")
  ...
  async delete(me: any, id: string) {
    ...
    const deleteQuery = this.systemErrorRepo
      .createQueryBuilder()
      .delete()
      .where("id = :id", { id });
    ...
    if (result.affected === 0) {
      throw new Error(
        "System error not found or you do not have permission to delete it",
      );
    }
