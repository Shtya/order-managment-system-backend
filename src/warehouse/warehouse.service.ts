...
export class WarehousesService {
  constructor(
    ...
    private warehousesRepo: Repository<WarehouseEntity>,
    @InjectRepository(StorageLocationEntity)
    private locationsRepo: Repository<StorageLocationEntity>,
  ...
  async list(me: any, q?: any) {
    ...
    if (q?.isActive !== undefined) {
      qb.andWhere(`w."isActive" = :isActive`, { isActive: q.isActive });
    }
  ...
  async export(me: any, q: any) {
    ...
    records.forEach((w) => {
      worksheet.addRow({
        name: w.name,
        address:
          w.address || this.translations.t("common.not_available_symbol"),

        description:
          w.description || this.translations.t("common.not_available_symbol"),
  ...
  async get(me: any, id: string) {
    ...
    if ((entity as any).adminId !== adminId) {
  ...
  async create(me: any, dto: CreateWarehouseDto) {
    ...
    const exists = await this.warehousesRepo.findOne({
      where: { adminId, name: dto.name } as any,
    });
    if (exists) {
      throw new BadRequestException(
        this.translations.t("domains.warehouses.name_already_exists"),
      );
    }
  ...
  async update(me: any, id: string, dto: UpdateWarehouseDto) {
    ...
    if (dto.name !== undefined && dto.name !== (wh as any).name) {
      ...
      if (exists && (exists as any).id !== (wh as any).id) {
        throw new BadRequestException(
          this.translations.t("domains.warehouses.name_already_exists"),
        );
      }
    ...
    if (dto.description !== undefined) {
      (wh as any).description = dto.description;
    }
  ...
  private async getLocationForWarehouse(
    me: any,
    warehouseId: string,
    locationId: string,
  ) {
    ...
    if (!location) {
      throw new BadRequestException(
        this.translations.t("domains.warehouses.location_not_found"),
      );
    }
    return location;
  }
  ...
  private async assertValidParent(
    ...
    if (!parent) {
      throw new BadRequestException(
        this.translations.t("domains.warehouses.parent_location_not_found"),
      );
    }
    ...
    if (!allowed.includes(childType)) {
      throw new BadRequestException(
        this.translations.t(
          "domains.warehouses.invalid_location_type_for_parent",
        ),
      );
    }
  ...
  async listLocations(me: any, q?: any) {
    ...
    if (q?.types) {
      const typesArr = String(q.types)
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      if (typesArr.length > 0) {
        qb.andWhere("l.type IN (:...types)", { types: typesArr });
      }
    } else if (q?.type) {
    ...
    if (q?.parentId) {
      qb.andWhere("l.parentId = :parentId", { parentId: q.parentId });
    }

    if (q?.isActive) {
      qb.andWhere("l.isActive = :isActive", { isActive: q.isActive });
    }

    if (q?.warehouseId && q.warehouseId !== "none") {
      qb.andWhere("l.warehouseId = :warehouseId", {
  ...
  async exportLocations(me: any, q: any) {
    const { records } = await this.listLocations(me, {
      ...q,
      limit: 1000,
      page: 1,
    });
  ...
  async createLocation(
    ...
    if (existing) {
      throw new BadRequestException(
        this.translations.t("domains.warehouses.location_name_already_exists"),
      );
    }

    let parent: StorageLocationEntity | null = null;
    if (dto.parentId) {
      parent = await this.assertValidParent(
        me,
        warehouseId,
        dto.parentId,
        dto.type,
    ...
    if (dto.name !== undefined) {
      ...
      if (existing) {
        throw new BadRequestException(
          this.translations.t(
            "domains.warehouses.location_name_already_exists",
          ),
        );
      }
    }
  ...
  async updateLocation(
    ...
    if (dto.description !== undefined) {
      (row as any).description = dto.description;
    }
  ...
  async removeLocation(me: any, warehouseId: string, locationId: string) {
    const location = await this.getLocationForWarehouse(
      me,
      warehouseId,
      locationId,
    );
    if (!location) {
      throw new BadRequestException(
        this.translations.t("domains.warehouses.location_has_children"),
      );
    }

    await this.locationsRepo.delete({ id: locationId } as any);
    return {
      message: this.translations.t("domains.warehouses.location_deleted"),
    };
  }
  ...
  async locationStats(me: any, warehouseId?: string) {
    ...
    const qb = this.locationsRepo
      .createQueryBuilder("l")
      .select(`SUM(CASE WHEN l.type = :zone THEN 1 ELSE 0 END)`, "zones")
      .addSelect(`SUM(CASE WHEN l.type = :rack THEN 1 ELSE 0 END)`, "racks")
      .addSelect(`SUM(CASE WHEN l.type = :shelf THEN 1 ELSE 0 END)`, "shelves")
      .addSelect(`SUM(CASE WHEN l.type = :bin THEN 1 ELSE 0 END)`, "bins")
      .where(`l."adminId" = :adminId`, { adminId })
      .setParameters({
