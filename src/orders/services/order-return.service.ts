...
import {
  getEffectiveDeductedQuantity,
  resolveRestockQuantity,
} from "../utils/stock-deduction";
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!order) {
        throw new NotFoundException(
          this.translations.t("domains.orders.return.order_not_found", {
      ...
      if (
        ...
        throw new BadRequestException(
          this.translations.t("domains.orders.return.order_status_wrong", {
      ...
      for (const returnItem of dto.items) {
        ...
        if (!originalOrderItem) {
          errorDetail = await this.requestTranslations.tAsync(
            "domains.orders.return.item_not_found",
            adminId,
            {
              args: { itemId: returnItem.originalItemId, orderId: dto.orderId },
            },
          );
        } else if (returnItem.quantity > originalOrderItem.quantity) {
          errorDetail = await this.requestTranslations.tAsync(
            "domains.orders.return.qty_mismatch",
            adminId,
            {
              args: {
                requested: returnItem.quantity,
                purchased: originalOrderItem.quantity,
              },
            },
          );
        } else {
          const restockQty = resolveRestockQuantity(
            returnItem.quantity,
            returnItem.restockQuantity,
          );
          const damagedQty = returnItem.quantity - restockQty;
          const effectiveDeducted = getEffectiveDeductedQuantity(
            originalOrderItem as any,
          );
          if (restockQty < 0 || restockQty > returnItem.quantity) {
            errorDetail = await this.requestTranslations.tAsync(
              "domains.orders.return.restock_qty_mismatch",
              adminId,
              {
                args: { requested: restockQty, returned: returnItem.quantity },
              },
            );
          } else if (restockQty > effectiveDeducted) {
            errorDetail = await this.requestTranslations.tAsync(
              "domains.orders.return.restock_qty_exceeds_deducted",
              adminId,
              { args: { requested: restockQty, deducted: effectiveDeducted } },
            );
          }

          if (damagedQty > 0 && !returnItem.damageResponsibility) {
            errorDetail = await this.requestTranslations.tAsync(
        ...
        items: dto.items.map((item) => {
          ...
          return {
            ...
            damageResponsibility:
              damagedQty > 0 ? item.damageResponsibility : null,
      ...
      if (preparingStatus) {
        ...
        await this.ordersService.logStatusChange({
          ...
          notes: await this.requestTranslations.tAsync(
            "domains.orders.return.moved_to_return_preparing",
            adminId,
            {},
          ),
          manager,
      ...
      await this.ordersService.logOrderAction({
        ...
        details: await this.requestTranslations.tAsync(
          "domains.orders.return.return_request_created_log",
          adminId,
          {
            args: {
              orderNumberOrId: order.orderNumber || dto.orderId,
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.orders.return.return_request_created_title",
          adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.orders.return.return_request_created_message",
          adminId,
          { fromSettings: true, args: { orderNumber: order.orderNumber } },
        ),
