...
export class OrderReplacementService {
  ...
  async listReplacements(me: any, q?: any) {
    ...
    const qb = this.replacementRepo
      ...
      .addSelect([
        ...
        "originalOrder.shippingCost",
        "originalOrder.finalTotal",
      ])
      ...
      qb.andWhere(
        new Brackets((sq) => {
          sq.where("originalOrder.orderNumber ILIKE :s", { s: `%${search}%` })
            .orWhere("replacementOrder.orderNumber ILIKE :s", {
              s: `%${search}%`,
            })
            .orWhere("originalOrder.customerName ILIKE :s", {
              s: `%${search}%`,
            })
            .orWhere("originalOrder.phoneNumber ILIKE :s", {
              s: `%${search}%`,
            });
        }),
      );
    ...
    if (q?.status) {
      ...
      if (!isNaN(Number(statusParam))) {
        qb.andWhere("replacementOrder.statusId = :statusId", {
          statusId: Number(statusParam),
        });
      } else {
        qb.andWhere("replacementStatus.code = :statusCode", {
  ...
  async exportReplacements(me: any, q?: any) {
    ...
    const exportData = replacements.map((rep) => {
      ...
      const diff =
        rep.originalOrder.finalTotal -
        rep.originalOrder.shippingCost -
        rep.replacementOrder.finalTotal;

      return {
        replacementOrderNumber: rep.replacementOrder?.orderNumber || "—",
        originalOrderNumber: rep.originalOrder?.orderNumber || "—",
        customerName:
          rep.replacementOrder?.customerName ||
          rep.originalOrder?.customerName ||
          "—",
        phoneNumber:
          rep.replacementOrder?.phoneNumber ||
          rep.originalOrder?.phoneNumber ||
          "—",
        ...
        status:
          rep.replacementOrder?.status?.name ||
          rep.replacementOrder?.status?.code ||
          "—",
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.orders.replacement.export.worksheet_name"),
    );

    worksheet.columns = [
      {
        header: this.translations.t(
          "domains.orders.replacement.export.replacement_order",
        ),
      ...
      {
        header: this.translations.t(
          "domains.orders.replacement.export.original_order",
        ),
      ...
      {
        header: this.translations.t(
          "domains.orders.replacement.export.customer",
        ),
        key: "customerName",
        width: 20,
      },
      {
        header: this.translations.t("domains.orders.replacement.export.phone"),
        key: "phoneNumber",
        width: 15,
      },
      {
        header: this.translations.t(
          "domains.orders.replacement.export.original_items",
        ),
      ...
      {
        header: this.translations.t(
          "domains.orders.replacement.export.replacement_items",
        ),
        key: "newProducts",
        width: 35,
      },
      {
        header: this.translations.t(
          "domains.orders.replacement.export.cost_diff",
        ),
        key: "costDiff",
        width: 12,
      },
      {
        header: this.translations.t("domains.orders.replacement.export.status"),
        key: "status",
        width: 15,
      },
      {
        header: this.translations.t("domains.orders.replacement.export.date"),
        key: "createdAt",
        width: 20,
      },
    ];
  ...
  async replaceOrder(me: any, dto: CreateReplacementDto, ipAddress?: string) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (originalOrder.replacementRequest) {
        throw new BadRequestException(
          this.translations.t(
            "domains.orders.replacement.already_has_replacement",
          ),
        );
      }
      ...
      if (!originalOrder) {
        throw new BadRequestException(
          this.translations.t(
            "domains.orders.replacement.original_order_not_found",
          ),
        );
      }
      ...
      const duplicates = requestedItemIds.filter(
        (id, idx) => requestedItemIds.indexOf(id) !== idx,
      );
      if (duplicates.length > 0) {
        throw new BadRequestException(
          this.translations.t("domains.orders.replacement.duplicate_products"),
        );
      }

      // Check all exist in original order
      const invalidIds = requestedItemIds.filter(
        (id) => !originalItemIds.includes(id),
      );
      if (invalidIds.length > 0) {
        throw new BadRequestException(
          this.translations.t("domains.orders.replacement.invalid_products"),
        );
      }
      ...
      const createOrderDto: CreateOrderDto = {
        ...
        allowOpenPackage: originalOrder.allowOpenPackage,
      ...
      const replacement = manager.create(OrderReplacementEntity, {
        ...
        items: dto.items.map((it) =>
          manager.create(OrderReplacementItemEntity, {
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.orders.replacement.notification_title",
          adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.orders.replacement.notification_message",
          adminId,
          {
            args: {
              replacementOrderNumber: newOrder.orderNumber,
              originalOrderNumber: originalOrder.orderNumber,
            },
          },
        ),
  ...
  async getStats(me: any) {
    ...
    const result = await this.orderRepo
      ...
      .addSelect("COUNT(replacement.id)", "replaced")
