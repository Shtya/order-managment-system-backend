...
export class ExtraFeaturesService {
  ...
  async purchaseFeature(user: User, featureId: string) {
    return await this.dataSource.transaction(async (manager) => {
      ...
      const feature = await manager.findOne(Feature, {
        where: { id: featureId, isActive: true },
      });
      if (!feature) {
        throw new NotFoundException(
          this.translations.t(
            "domains.extra_features.feature_not_found_or_inactive",
          ),
        );
      }
      ...
      if (existing) {
        throw new BadRequestException(
          this.translations.t("domains.extra_features.feature_already_active"),
        );
      }
  ...
  async list(me: User, q?: any) {
    ...
    if (q?.featureId) {
      qb.andWhere("uf.featureId = :featureId", { featureId: q.featureId });
    }
  ...
  async exportExtraFeatures(me: User, q?: any) {
    ...
    const worksheet = workbook.addWorksheet(
      this.translations.t("domains.extra_features.export_sheet"),
    );

    // 1. تحديد الأعمدة
    worksheet.columns = [
      {
        header: this.translations.t("domains.extra_features.export_user_name"),
        key: "userName",
        width: 25,
      },
      {
        header: this.translations.t("domains.extra_features.export_user_email"),
        key: "userEmail",
        width: 30,
      },
      {
        header: this.translations.t(
          "domains.extra_features.export_feature_name",
        ),
        key: "featureName",
        width: 20,
      },
      {
        header: this.translations.t(
          "domains.extra_features.export_feature_type",
        ),
        key: "featureType",
        width: 20,
      },
      {
        header: this.translations.t("domains.extra_features.export_paid_price"),
        key: "price",
        width: 15,
      },
      {
        header: this.translations.t("domains.extra_features.export_status"),
        key: "status",
        width: 15,
      },
      {
        header: this.translations.t(
          "domains.extra_features.export_availability",
        ),
        key: "availability",
        width: 15,
      },
      {
        header: this.translations.t(
          "domains.extra_features.export_purchase_date",
        ),
        key: "startDate",
        width: 20,
      },
    ...
    const rows = records.map((uf) => ({
      ...
      featureName:
        uf.feature?.name?.trim() ||
        this.translations.t("domains.extra_features.deleted_feature"),
      ...
      startDate: uf.startDate
        ? new Date(uf.startDate).toISOString().split("T")[0]
        : "—",
    }));
  ...
  async updateFeature(me: any, featureId: string, dto: UpdateFeatureDto) {
    ...
    if (!feature) {
      throw new NotFoundException(
        this.translations.t(
          "domains.extra_features.feature_not_found_with_id",
          { args: { featureId } },
        ),
      );
    }
  ...
  async assignFeatureToUser(me: User, dto: AssignUserFeatureDto) {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!feature) {
        throw new NotFoundException(
          this.translations.t(
            "domains.extra_features.feature_definition_not_found",
          ),
        );
      }
      ...
      if (!feature.isActive) {
        throw new BadRequestException(
          this.translations.t("domains.extra_features.feature_deactivated"),
        );
      }
      ...
      if (existing) {
        throw new BadRequestException(
          this.translations.t(
            "domains.extra_features.user_feature_already_active",
          ),
        );
      }
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.extra_features.extra_feature_assigned",
          user.adminId || me.id,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.extra_features.extra_feature_assigned_message",
          user.adminId || me.id,
          {
            args: {
