...
export class ExtraFeaturesController {
  ...
  async export(@Req() req, @Query() q, @Res() res) {
    const buffer = await this.extraFeaturesService.exportExtraFeatures(
      req.user,
      q,
    );
    res.set({
  ...
  async purchaseAddon(@Req() req: any, @Body("featureId") featureId: string) {
  ...
  @Permissions("extra-features.update")
  @Post("assign")
  async assignFeature(@Req() req, @Body() dto: AssignUserFeatureDto) {
    return await this.extraFeaturesService.assignFeatureToUser(req.user, dto);
  }

  @Permissions("extra-features.read")
  @Get("user")
  async getUserFeatures(@Req() req: any) {
