...
export class PurchasesService {
  ...
  async stats(me: any) {
    ...
    const rejected = await this.invRepo.count({
  ...
  async list(me: any, q?: any) {
    ...
    const hasReceipt =
      q?.hasReceipt && q.hasReceipt !== "all" ? String(q.hasReceipt) : null; // yes/no
  ...
  async get(me: any, id: string) {
    ...
    if (!inv) {
      throw new BadRequestException(
        this.translations.t("domains.purchase_invoice.not_found"),
      );
    }
    return { ...inv, supplier };
  }
  ...
  async acceptPreview(me: any, id: string) {
    ...
    const rows = variantIds.map((variantId) => {
      ...
      if (willApply) {
        if (oldPrice == null) {
          newPrice = Number.isFinite(incomingAvgCost)
            ? Math.round(incomingAvgCost)
            : null;
        } else {
          ...
          if (denom > 0) {
            ...
            newPrice = Number.isFinite(weighted)
              ? Math.round(weighted)
              : oldPrice;
          }
        }
  ...
  async create(
    ...
    if (!dto.items?.length) {
      throw new BadRequestException(
        this.translations.t("domains.purchase_invoice.items_required"),
      );
    }
    ...
    const runWithManager = async (manager: EntityManager) => {
      ...
      const exists = await repo.findOne({
        where: { adminId, receiptNumber: dto.receiptNumber } as any,
      });
      if (exists) {
        throw new BadRequestException(
          this.translations.t("domains.purchase_invoice.receipt_number_exists"),
        );
      }
      ...
      await this.log({
        ...
        description: await this.requestTranslations.tAsync(
          "domains.purchase_invoice.purchase_invoice_created_description",
          adminId,
          { args: { status: saved.status } },
        ),
        ipAddress,
        manager,
      });
  ...
  async update(
    ...
    if (inv.closingId) {
      throw new BadRequestException(
        this.translations.t("domains.purchase_invoice.cannot_update_closed"),
      );
    }
    ...
    if (inv.status === ApprovalStatus.ACCEPTED && dto.items) {
      throw new BadRequestException(
        this.translations.t(
          "domains.purchase_invoice.cannot_modify_accepted_items",
        ),
      );
    }
    ...
    if (dto.safeId) {
      const safe = await this.accountRepo.findOne({
        where: { id: dto.safeId, adminId } as any,
      });
      if (!safe) {
        throw new BadRequestException(
    ...
    if (dto.items) {
      ...
      if (typeof dto.paidAmount === "number") {
        (inv as any).remainingAmount =
          ((inv as any).total ?? 0) - dto.paidAmount;
      }
      saved = await this.invRepo.save(inv as any);
    }
  ...
  async updatePaidAmount(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!inv) {
        throw new BadRequestException(
          this.translations.t("domains.purchase_invoice.invoice_not_found"),
        );
      }
      if (inv.closingId) {
        throw new BadRequestException(
        ...
        if (delta > 0) {
          await this.safesService.withdraw(
            me,
            {
              ...
              notes: await this.requestTranslations.tAsync(
                "domains.purchase_invoice.paid_amount_adjustment_positive_notes",
                adminId,
                { args: { receiptNumber: inv.receiptNumber, delta } },
              ),
            },
            manager,
          );
          ...
          await this.safesService.deposit(
            me,
            {
              ...
              notes: await this.requestTranslations.tAsync(
                "domains.purchase_invoice.paid_amount_adjustment_notes",
                adminId,
                { args: { receiptNumber: inv.receiptNumber, delta } },
              ),
            },
    ...
    const runWithManager = async (manager: EntityManager) => {
      ...
      if (
        ...
        const byVariant = new Map<
          string,
          { qty: number; incomingCostTotal: number }
        >();
        ...
        if (!variantIds.length) {
          throw new BadRequestException(
            this.translations.t("domains.purchase_invoice.no_items_to_apply"),
          );
        }
        ...
        for (const variantId of variantIds) {
          if (!byId.get(variantId)) {
            throw new BadRequestException(
              this.translations.t(
                "domains.purchase_invoice.variant_not_found",
                { args: { variantId } },
              ),
            );
          }
        ...
        for (const variantId of variantIds) {
          ...
          if (oldPrice == null) {
            newPrice = Number.isFinite(incomingAvg)
              ? Math.round(incomingAvg)
              : null;
          } else {
            const denom = oldStock + addQty;
            if (denom > 0) {
              const weighted =
                (oldPrice * oldStock + incomingAvg * addQty) / denom;
              newPrice = Number.isFinite(weighted)
                ? Math.round(weighted)
                : oldPrice;
            }
        ...
        if (Number(inv.paidAmount) > 0 && inv.safeId) {
          await this.safesService.withdraw(
            me,
            {
              ...
              notes: await this.requestTranslations.tAsync(
                "domains.purchase_invoice.log_purchase_invoice_accepted",
                adminId,
                { args: { purchaseNumber: inv.receiptNumber || null } },
              ),
            },
            manager,
          );
        ...
        if (variantIds.length) {
          ...
          for (const variantId of variantIds) {
            ...
            if (next < 0) {
              throw new BadRequestException(
                this.translations.t(
                  "domains.purchase_invoice.cannot_remove_stock_below_zero",
                  { args: { sku: v.sku, oldStock, removeQty } },
                ),
              );
            }
          ...
          if (
            ...
            if (Array.isArray(priceChanges) && priceChanges.length) {
              const priceVariantIds = [
                ...new Set(
                  priceChanges.map((x: any) => x.variantId).filter(Boolean),
                ),
              ];
        ...
        if (Number(inv.paidAmount) > 0 && inv.safeId) {
          await this.safesService.deposit(
            me,
            {
              ...
              notes: await this.requestTranslations.tAsync(
                "domains.purchase_invoice.status_changed_refunded_notes",
                adminId,
                { args: { receiptNumber: inv.receiptNumber, status } },
              ),
            },
            manager,
          );
  ...
  async remove(
    ...
    const runWithManager = async (manager: EntityManager) => {
      ...
      if (inv.closingId) {
        throw new BadRequestException(
          this.translations.t("domains.purchase_invoice.cannot_delete_closed"),
        );
      }
        ...
        if (Number(inv.paidAmount) > 0 && inv.safeId) {
          await this.safesService.deposit(
            me,
            {
              ...
              notes: await this.requestTranslations.tAsync(
                "domains.purchase_invoice.purchase_invoice_deleted_refunded_notes",
                adminId,
                { args: { receiptNumber: inv.receiptNumber, status } },
              ),
            },
  ...
  async exportPurchases(me: any, q?: any) {
    ...
    if (search) {
      qb.andWhere("(inv.receiptNumber ILIKE :s OR supplier.name ILIKE :s)", {
        s: `%${search}%`,
      });
    }
    ...
    worksheet.columns = [
      ...
      {
        header: this.translations.t("domains.purchase_invoice.supplier"),
        key: "supplier",
        width: 25,
      },
      {
        header: this.translations.t("domains.purchase_invoice.receipt_number"),
        key: "receiptNumber",
        width: 20,
      },
      {
      ...
      {
        header: this.translations.t("domains.purchase_invoice.subtotal"),
        key: "subtotal",
        width: 15,
      },
      {
        header: this.translations.t("domains.purchase_invoice.total"),
        key: "total",
        width: 15,
      },
      {
        header: this.translations.t("domains.purchase_invoice.paid_amount"),
        key: "paidAmount",
        width: 15,
      },
      {
        header: this.translations.t(
          "domains.purchase_invoice.remaining_amount",
        ),
        key: "remainingAmount",
        width: 15,
      },
