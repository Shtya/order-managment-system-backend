...
export async function syncProviderLocationsLogic(
  ...
  let turboRecords = [];
  try {
    const turboResp = await axios.get(
      process.env.TURBO_GEO_API_URL + "/external-api/get-government",
    );
  ...
  const processRecords = async (provider: string, records: any[]) => {
    for (const record of records) {
      const matchedCity =
        unifiedCitiesMap.get(normalizeEnglish(record.nameEn)) ||
      ...
      if (!exists) {
        await providerLocationRepo.save(
    ...
    logger.log(
      `✅ Synced ${provider} provider locations for ${records.length} cities`,
    );
  };
