...
export class KashierProvider extends PaymentProvider {
  ...
  constructor(
    ...
    private sessionRepo: Repository<PaymentSessionEntity>,
  ...
  async checkout(options: CheckoutOptions): Promise<CheckoutResponse> {
    this.logger.log(
      `[KashierCheckout] Starting checkout for userId: ${options.userId}, purpose: ${options.purpose}, amount: ${options.amount} ${options.currency}`,
    );
    ...
    this.logger.log(
      `[KashierCheckout] Creating payment session with expireAt: ${expireAtDate.toISOString()}`,
    );
    ...
    this.logger.log(
      `[KashierCheckout] Payment session created: ${savedSession.id}`,
    );
    ...
    this.logger.debug(
      `[KashierCheckout] Calling Kashier API with payload: ${JSON.stringify(payload)}`,
    );
    try {
      // 3. Call Kashier API
      ...
      this.logger.debug(
        `[KashierCheckout] Kashier API response: ${JSON.stringify(data)}`,
      );
      ...
      if (checkoutUrl) {
        this.logger.log(
          `[KashierCheckout] Updating session ${savedSession.id} with checkoutUrl: ${checkoutUrl} and externalSessionId: ${sessionId}`,
        );
      ...
      this.logger.log(
        `[KashierCheckout] Checkout successful, returning checkoutUrl: ${checkoutUrl}`,
      );
      return { checkoutUrl, sessionId };
    } catch (error: any) {
      this.logger.error(
        `[KashierCheckout] Error calling Kashier API: ${error.message}`,
        error.stack,
      );
      this.logger.error(
        `[KashierCheckout] Error details: ${JSON.stringify(error.response?.data || error)}`,
      );
      // Throw user-friendly error
      throw new InternalServerErrorException(
        this.translations.t("domains.payments.initial_payment_session_failed"),
      );
    }
  }
  ...
  mapProviderStatus(rawStatus: any): PaymentSessionStatusEnum {
    switch (rawStatus) {
      case "SUCCESS":
        return PaymentSessionStatusEnum.SUCCESS;
      case "FAILED":
        return PaymentSessionStatusEnum.FAILED;
      case "CANCELLED":
        return PaymentSessionStatusEnum.CANCELLED;
      case "EXPIRED":
        return PaymentSessionStatusEnum.EXPIRED;
      default:
        return PaymentSessionStatusEnum.FAILED;
    }
  }
