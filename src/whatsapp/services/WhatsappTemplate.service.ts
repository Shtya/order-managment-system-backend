...
export class WhatsappTemplateService {
  ...
  async list(me: any, q?: any, superAdmin?: boolean) {
    ...
    if (accountId) {
      ...
      if (selectedAccount?.wabaId) {
        qb.andWhere("account.wabaId = :wabaId", {
    ...
    if (search) {
      qb.andWhere(
        new Brackets((sq) => {
          sq.where("tpl.name ILIKE :s", { s: `%${search}%` }).orWhere(
            "tpl.metaId ILIKE :s",
            { s: `%${search}%` },
          );
        }),
  ...
  async metaLibrary(me: any, q: MetaTemplateLibraryQueryDto) {
    ...
    const accountId = await this.whatsappService.getDefaultAccountId(
      adminId,
      q.accountId,
    );
    ...
    const filteredTemplates = templates.filter(
      (tpl) => tpl.category !== "AUTHENTICATION",
    );

    return filteredTemplates.map((tpl: any) =>
      this.mapMetaLibraryTemplate(tpl),
    );
  }
  ...
  private mapMetaLibraryTemplate(tpl: any): MetaTemplateLibraryItemDto {
    ...
    return {
      ...
      topic: tpl.topic,
      usecase: tpl.usecase,
      industry: Array.isArray(tpl.industry)
        ? tpl.industry
        : tpl.industry
          ? [tpl.industry]
          : [],
   ...
   */
  private mapMetaDtoButtons(
    buttons?: any[],
  ): MetaTemplateLibraryButtonDto[] | undefined {
    if (!buttons?.length) return undefined;

    return buttons.map((btn) => ({
      type: ["PHONE_NUMBER", "URL", "WHATSAPP_CALL"].includes(btn.type)
        ? btn.type
        : "CUSTOM",
      text: btn.text ?? "",
      url: btn.url,
  ...
  async create(me: any, dto: CreateWhatsappTemplateDto) {
    ...
    if (!isSuperAdmin && !dto.accountId) {
      throw new BadRequestException(
        this.translations.t("common.missing_account_id"),
      );
    }
    ...
    if (existingTemplate) {
      throw new BadRequestException(
        this.translations.t("domains.whatsapp.template_already_exists"),
      );
    }
  ...
  async update(me: any, id: string, dto: UpdateWhatsappTemplateDto) {
    ...
    if (!isSuperAdmin && template.status === TemplateStatus.LOCKED) {
      throw new BadRequestException(
        this.translations.t(
          "domains.whatsapp.locked_templates_cannot_be_edited",
        ),
      );
    }
    ...
    if (!isSuperAdmin && !editableStatuses.includes(template.status)) {
      throw new BadRequestException(
        this.translations.t(
          "domains.whatsapp.only_specific_templates_can_be_edited",
        ),
      );
    }
    ...
    if (!isSuperAdmin && template.status === TemplateStatus.DISABLED) {
      throw new BadRequestException(
        await this.requestTranslations.tAsync(
          "domains.whatsapp.disabled_templates_cannot_be_deleted",
          adminId,
        ),
      );
    }
  ...
  async export(me: any, q: any) {
    ...
    const exportData = records.map((t) => ({
      ...
      category: t.category,
      subCategory:
        t.subCategory || this.translations.t("common.not_available_symbol"),
      language: t.language,
      status: t.status,
      quality: t.quality,
  ...
  private async resolveHeaderHandle(
    data: WhatsappTemplateEntity,
    accountId: string,
  ): Promise<string | null> {
  ...
  private async mapToMetaPayload(
    ...
    //body
    if (cfg?.bodyText) {
      const body: BodyComponentDto = { type: "BODY", text: cfg.bodyText };
      if (cfg?.examples) {
        if (parameterFormat === "positional") {
    ...
    //footer
    if (cfg?.footerText) {
      const footer: FooterComponentDto = {
        type: "FOOTER",
        text: cfg.footerText,
      };
      ...
      const footer: FooterComponentDto = {
        type: "FOOTER",
        code_expiration_minutes: cfg.expirationMinutes,
        text: cfg.footerText,
      };
      components.push(footer);
    }
    ...
    return {
      ...
      parameter_format:
        parameterFormat === "positional" ? "POSITIONAL" : "NAMED",
  ...
  private mapButtons(
    ...
    const encoded = (value: string) => encodeURIComponent(value); // 🔥 important for URL params
    ...
    for (const btn of buttons) {
      switch (btn.type) {
        ...
        case "COPY_CODE": {
          ...
          const staticText =
            language === "ar" ? "نسخ رمز العرض" : "Copy offer code";
          metaButtons.push({
            type: "COPY_CODE",
    ...
    if (phoneNumbers.length > 1) {
      throw new BadRequestException(
        this.translations.t(
          "domains.whatsapp.max_1_phone_number_button_allowed",
        ),
      );
    }
    ...
    if (quickReplies.length > 10) {
      throw new BadRequestException(
        this.translations.t(
          "domains.whatsapp.max_10_quick_reply_buttons_allowed",
        ),
      );
    }
    ...
    return buttons.map((btn) => {
      ...
      if (btn.type === "URL") {
        return {
          ...
          urlExample: btn.example?.[0]
            ? decodeURIComponent(btn.example[0])
            : undefined,
        };
      }
      ...
      if (btn.type === "VOICE_CALL") {
        return {
          type: "WHATSAPP_CALL",
          text: btn.text,
          activeForDays: btn.ttl_minutes
            ? Math.round(btn.ttl_minutes / (24 * 60))
            : 7,
        };
      }
  ...
  private async statusFromMeta(
    ...
    if (metaStatus === "DELETED") {
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.whatsapp.template_deleted",
          template.adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.whatsapp.template_deleted_message",
          template.adminId,
          {
    ...
    if (metaStatus === "FLAGGED") {
      ...
      await this.notificationService.create({
        ...
        title: await this.requestTranslations.tAsync(
          "domains.whatsapp.template_flagged",
          template.adminId,
        ),
        message: await this.requestTranslations.tAsync(
          "domains.whatsapp.template_flagged_message",
          template.adminId,
        ),
        relatedEntityType: "Template",
        relatedEntityId: template.id,
  ...
  public async updateQuality(metaTemplateId: string, metaQuality?: string) {
    ...
    await this.notificationService.create({
      ...
      title: await this.requestTranslations.tAsync(
        "domains.whatsapp.template_quality_updated",
        template.adminId,
      ),
      message: await this.requestTranslations.tAsync(
  ...
  public async updateStatus(metaTemplateId: string, metaStatus?: string) {
    ...
    await this.notificationService.create({
      ...
      title: await this.requestTranslations.tAsync(
        "domains.whatsapp.template_status_updated",
        template.adminId,
      ),
      message: await this.requestTranslations.tAsync(
  ...
  async syncTemplatesFromMeta(
    ...
    const metaTemplates = await this.whatsappApi.fetchWabaTemplates(
      wabaId,
      accessToken,
    );
    const repo = manager
      ? manager.getRepository(WhatsappTemplateEntity)
      : this.templateRepo;
    ...
    for (const metaTpl of metaTemplates) {
      const parameterFormat =
        metaTpl.parameter_format === "POSITIONAL" ? "positional" : "named";
        ...
        if (comp.type === "HEADER") {
          ...
          if (comp.format === "TEXT") {
            ...
            if (comp.example?.header_text?.[0]) {
              ...
              templateConfig.headerExample =
                comp.example.header_text_named_params[0].example;
              templateConfig.headerNamedKey =
                comp.example.header_text_named_params[0].param_name;
            }
          ...
          templateConfig.buttons = this.mapButtonsFromMeta(
            comp.buttons,
            language,
          );
        }
