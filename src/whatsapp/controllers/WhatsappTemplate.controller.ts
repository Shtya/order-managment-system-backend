...
export class WhatsappTemplateController {
  ...
  async export(@Req() req: any, @Query() q: any, @Res() res: Response) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=templates_export_${Date.now()}.xlsx`,
    );
  ...
  async uploadHeaderMedia(
    @UploadedFiles()
    files: {
      headerMedia?: Express.Multer.File[];
    },
  ) {
      ...
      if (headerUrl.startsWith("http://") || headerUrl.startsWith("https://")) {
        try {
          ...
          throw new BadRequestException(
            "Failed to download header media from URL",
          );
        }
      }
      // If it's already a relative path (uploads/... or /uploads/...), use as is
      else if (
        headerUrl.startsWith("uploads/") ||
        headerUrl.startsWith("/uploads/")
      ) {
        templateConfig = {
          ...templateConfig,
          headerUrl: headerUrl.startsWith("/") ? headerUrl.slice(1) : headerUrl,
        };
      }
    }
