...
export class UsersService {
  constructor(
    ...
    private subscriptionsRepo: Repository<Subscription>, // ✅ NEW
    @InjectRepository(GettingStartedItemEntity)
    private itemRepo: Repository<GettingStartedItemEntity>, // ✅ NEW
  ...
  async getFullUser(userId: string, addPass: boolean = false): Promise<User> {
    ...
    // Join Role
    if (addPass) q.addSelect("user.passwordHash");
  ...
  async superAdminList(
    ...
    if (search) {
      qbStats.andWhere("(u.name LIKE :q OR u.email LIKE :q)", {
        q: `%${search}%`,
      });
    }
  ...
  async updateMyAvatar(me: User, avatar?: Express.Multer.File) {
    if (!avatar) {
      throw new BadRequestException(
        this.translations.t("domains.users.avatar_file_required"),
      );
    }
  ...
  async list(
    ...
    if (active) {
      qb.andWhere("user.isActive = :active", { active: active === "true" });
    }
    if (search) {
      qb.andWhere(
  ...
  async listForTable(
    ...
    const pushSearch = (extra: any) => {
      ...
      return [
        ...
        {
          ...extra,
          email: () => `email LIKE '%${search.replace(/'/g, "''")}%'`,
        },
        {
          ...extra,
          phone: () => `phone LIKE '%${search.replace(/'/g, "''")}%'`,
        },
      ];
    };
    ...
    if (!this.isSuperAdmin(me)) {
      if (me.role?.name === SystemRole.ADMIN) {
        qb2.where("u.adminId = :adminId", { adminId: me.id });
    ...
    if (search) {
      qb2.andWhere("(u.name LIKE :q OR u.email LIKE :q OR u.phone LIKE :q)", {
        q: `%${search}%`,
      });
    }
  ...
  private async ensureUsersLimit(me: User) {
    ...
    if (
      ...
      currentCount >= subscription?.usersLimit
    ) {
      throw new BadRequestException(
  ...
  async update(me: User, id: string, patch: UpdateUserDto) {
    ...
    if ((patch as any).phone !== undefined) {
      updateData.phone = (patch as any).phone;
    }
    if ((patch as any).employeeType !== undefined) {
      updateData.employeeType = (patch as any).employeeType;
    }
  ...
  async processNextOnboardingStep(
    me: User,
    userId: string,
    wantedStep: OnboardingStep,
  ) {
    if (me.role?.name !== "admin") {
      throw new ForbiddenException(
        this.translations.t(
          "domains.users.only_admins_can_complete_onboarding",
        ),
      );
    }
      ...
      default:
        return {
          message: this.translations.t(
            "domains.users.onboarding_already_completed_status",
          ),
          step: user.currentOnboardingStep,
        };
    }
