    ...
    try {
      ...
      for (const cat of SEED_DATA.productCategories) {
        try {
          ...
          this.logger.warn(
            `Failed to seed product category ${cat.name}: ${e.message}`,
          );
        }
      }
      ...
      for (const cat of SEED_DATA.supplierCategories) {
        try {
          ...
          this.logger.warn(
            `Failed to seed supplier category ${cat.name}: ${e.message}`,
          );
        }
      }
      ...
      if (productCount === 0) {
        ...
        for (const product of SEED_DATA.products) {
          try {
            ...
            this.logger.warn(
              `Failed to seed product ${product.name}: ${e.message}`,
            );
          }
        }
      } else {
      ...
      this.logger.error(
        `Error during seeding data for user ${user.id}: ${error.message}`,
        error.stack,
      );
    }
  }
