...
export class CategoriesService {
  ...
  async create(me: any, dto: CreateCategoryDto) {
    ...
    const existsName = await this.catRepo.findOne({
      where: { adminId, name: dto.name } as any,
    });
    if (existsName) {
      throw new BadRequestException(
        this.translations.t("domains.category.name_already_exists"),
      );
    }

    const existsSlug = await this.catRepo.findOne({
      where: { adminId, slug: dto.slug } as any,
    });
    if (existsSlug) {
      throw new BadRequestException(
        this.translations.t("domains.category.slug_already_exists"),
      );
    }
  ...
  async duplicate(me: any, id: string, dto: { name: string; slug: string }) {
    ...
    if (!source) {
      throw new BadRequestException(
        this.translations.t("domains.category.source_not_found"),
      );
    }
